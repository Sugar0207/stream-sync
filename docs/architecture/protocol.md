...
- `AuthResponse` payload decode の最小実装
- `Heartbeat` payload decode の最小実装
- `HeartbeatAck` payload decode の最小実装
...
- `Heartbeat` payload encode の最小実装
...
payload decode は現時点では `AuthRequest`, `AuthResponse`, `Heartbeat`, `HeartbeatAck`, `VideoFrame`, `ClientStats`, `ServerNotice` に対応する。`AuthRequest` payload decode は fixed header decode と protocol_version 期待値チェックが済んだ後に、`client_id`, `run_id`, `app_version`, `shared_token`, `display_name` を docs の byte layout どおりに読む。`AuthResponse` payload decode は同じ前提で、`client_id`, `run_id`, `accepted`, `reason_code`, `message`, `server_time`, `expected_protocol_version` を読む。`Heartbeat` payload decode は同じ前提で、`client_id`, `run_id`, `sent_at`, `local_time`, `short_status` を読む。`HeartbeatAck` payload decode は同じ前提で、`client_id`, `run_id`, `echoed_sent_at`, `server_received_at`, `server_sent_at` を読む。`VideoFrame` payload decode は同じ前提で、`client_id`, `run_id`, 46 byte numeric metadata, H.264 bytes を読む。H.264 bytes は `payload_size` と実際の残り byte 数が一致する場合だけ `Vec<u8>` として復元し、codec decode や映像内容の解釈は行わない。`ClientStats` payload decode は同じ前提で、最小 stats fields と optional heartbeat observation block を読む。`ServerNotice` payload decode は同じ前提で、`run_id`, `notice_type`, `message` を読む。
payload encode は現時点では `AuthRequest`, `AuthResponse`, `Heartbeat`, `HeartbeatAck`, `VideoFrame`, `ClientStats`, `ServerNotice` に対応する。`AuthRequest` encode は fixed header の `message_type = AuthRequest`、`protocol_version`、`payload_length` を書き、payload に `client_id`, `run_id`, `app_version`, `shared_token`, `display_name` を docs の byte layout どおりに書く。`AuthResponse` encode は fixed header の `message_type = AuthResponse`、`protocol_version`、`payload_length` を書き、payload に `client_id`, `run_id`, `accepted`, `reason_code`, `message`, `server_time`, `expected_protocol_version` を docs の byte layout どおりに書く。`Heartbeat` encode は fixed header の `message_type = Heartbeat`、`protocol_version`、`payload_length` を書き、payload に `client_id`, `run_id`, `sent_at`, `local_time`, `short_status` を docs の byte layout どおりに書く。`HeartbeatAck` encode は fixed header の `message_type = HeartbeatAck`、`protocol_version`、`payload_length` を書き、payload に `client_id`, `run_id`, `echoed_sent_at`, `server_received_at`, `server_sent_at` を docs の byte layout どおりに書く。`VideoFrame` encode は fixed header の `message_type = VideoFrame`、`protocol_version`、`payload_length` を書き、payload に `client_id`, `run_id`, 46 byte numeric metadata, H.264 bytes を docs の byte layout どおりに書く。H.264 bytes は protocol crate では変換せず、`payload_size` は実 payload byte 長から決める。`ClientStats` encode は fixed header の `message_type = ClientStats`、`protocol_version`、`payload_length` を書き、payload に最小 stats fields と optional heartbeat observation block を書く。`ServerNotice` encode は fixed header の `message_type = ServerNotice`、`protocol_version`、`payload_length` を書き、payload に `run_id`, `notice_type`, `message` を書く。
...
この段階では fixed header decode、protocol_version 期待値チェック、`AuthRequest` / `AuthResponse` / `Heartbeat` / `HeartbeatAck` / `VideoFrame` / `ClientStats` / `ServerNotice` payload decode と encode の最小実装を置く。継続 loop、retry、fragmentation、encryption、認証状態更新、heartbeat state commit、video buffer 投入は protocol crate には入れない。
...
`FixedHeaderCodec` と `decode_fixed_header` は、16 byte fixed header の byte parsing と payload slice の切り出しだけを行う。`decode_payload_by_message_type` は fixed header の `message_type` に応じて payload decoder を選ぶ dispatch helper とする。`AuthRequestPayloadDecoder` / `decode_auth_request_payload` は `AuthRequest` payload のみを、`AuthResponsePayloadDecoder` / `decode_auth_response_payload` は `AuthResponse` payload のみを、`HeartbeatPayloadDecoder` / `decode_heartbeat_payload` は `Heartbeat` payload のみを、`HeartbeatAckPayloadDecoder` / `decode_heartbeat_ack_payload` は `HeartbeatAck` payload のみを、`VideoFramePayloadDecoder` / `decode_video_frame_payload` は `VideoFrame` payload のみを、`ClientStatsPayloadDecoder` / `decode_client_stats_payload` は `ClientStats` payload のみを、`ServerNoticePayloadDecoder` / `decode_server_notice_payload` は `ServerNotice` payload のみを型へ変換する。`MessageEncoder` は境界名と責務を固定するための入口であり、現時点では `AuthRequest`, `AuthResponse`, `Heartbeat`, `HeartbeatAck`, `VideoFrame`, `ClientStats`, `ServerNotice` の fixed header + payload bytes を書く。
...
とする。現在の code はこの layout の最小 payload encode / decode と
fixed header encode を持つ。
...
- heartbeat observation carrier として optional block を追加する方針とする。
  ただし継続送信 loop、server receive route / handler 接続、RTT / offset
  state commit はまだ行わない。
...
- 現時点
  - `encode_client_stats_payload` / `decode_client_stats_payload` が上記 layout
    の最小 wire 変換を行う。
  - `ClientStatsPayloadDecoder` は fixed header decode 後の payload を
    `ProtocolMessage::ClientStats` へ変換する。
  - `ProtocolMessageEncoderBoundary` は `ProtocolMessage::ClientStats` を
    fixed header + payload bytes へ変換する。
  - 継続 stats 送信、server 側の route / handler 接続、timebase state commit
    はまだ実装しない。
...
7. `ServerHeartbeatHandlerBoundary` returns these inputs next to the ack queue
   handoff.
...
- timebase plan boundary
  - Maps timestamp input into a calculation plan.
  - Does not produce numeric estimates.
...
bridges that input to `crates/timebase::HeartbeatTimebasePlanBoundary` and
stores the result as `ServerHeartbeatTimebasePlan`.

### Heartbeat RTT / Offset Calculation Policy

Heartbeat timestamp fields must keep their clock domains explicit. The current
code records a calculation plan only; final RTT / offset values and smoothing
state remain future work.

Policy:

- RTT
  - A server receive sample cannot complete RTT by itself because the server
    does not know the client's ack receive time.
  - The plan records `RequiresClientAckObservation` with the echoed
    `Heartbeat.sent_at` and `HeartbeatAck.server_sent_at`.
  - A future client-side observation can combine client send time, client ack
    receive time, and server ack timestamps.
- Offset
  - `Heartbeat.local_time` is optional. When present, it becomes an offset
    candidate paired with `server_received_at`.
  - The future estimator must apply delay / RTT compensation before accepting
    the candidate as a client clock offset.
  - When absent, the plan records `MissingClientLocalTime` and no offset update
    should be attempted.
- Smoothing
  - The current plan is `Deferred`.
  - Numeric smoothing factor, outlier handling, warm-up behavior, and
    per-client estimate storage belong to the future timebase calculation layer.

Responsibility split:

- protocol
  - Defines and decodes heartbeat timestamp fields.
- server heartbeat input boundary
  - Provides raw timestamp observations after source authentication.
- timebase plan boundary
  - Chooses the applicable calculation path without mutating state.
...
2. `HeartbeatObservationCarrierBoundary` wraps it as
   ...
   observation }`.
3. The `ClientStats` encoder writes the common stats payload and then the
...
- protocol carrier
  - Defines the typed carrier and payload field set.
  - Encodes/decodes the minimal `ClientStats` payload, including the optional
    heartbeat observation block.
- server carrier
...
encode/decode is implemented in `crates/protocol`; continuous send/receive
wiring and state commit remain future work.
...
heartbeat observation block. These helpers describe the same layout used by the
minimal encoder/decoder and `ProtocolMessageEncoderBoundary` supports
`ProtocolMessage::ClientStats`.
...
- `crates/protocol::ProtocolMessageEncoderBoundary` implements
  ...
  `ProtocolMessage::AuthResponse`, `ProtocolMessage::Heartbeat`,
  `ProtocolMessage::HeartbeatAck`, `ProtocolMessage::VideoFrame`,
  `ProtocolMessage::ClientStats`, and `ProtocolMessage::ServerNotice`.
