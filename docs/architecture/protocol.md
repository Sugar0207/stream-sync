...
この段階では fixed header decode、protocol_version 期待値チェック、`AuthRequest` / `Heartbeat` / `VideoFrame` / `ClientStats` payload decode、`AuthRequest` / `AuthResponse` / `HeartbeatAck` / `VideoFrame` / `ClientStats` payload encode の最小実装だけを置く。その他 payload decode と encode の本実装は行わない。
