...
この実装は fixed header と raw payload slice の境界確認までに限定する。payload の message 別 decode、protocol_version の期待値チェック、encode はまだ本実装しない。
...
この段階では fixed header decode の最小実装だけを置く。payload decode と encode の本実装は行わない。
...
5. app 側、または protocol crate の payload decode 入口は、payload decode 前に `protocol_version` が期待値と一致するか確認する。
...
`FixedHeaderCodec` と `decode_fixed_header` は、16 byte fixed header の byte parsing と payload slice の切り出しだけを行う。`PayloadDecoder` と `MessageEncoder` は境界名と責務を固定するための placeholder であり、現時点では実際の payload parsing / byte writing は実装しない。
