...
この実装は fixed header と raw payload slice の境界確認、および protocol_version の期待値照合までに限定する。payload の message 別 decode と encode はまだ本実装しない。
...
この段階では fixed header decode と protocol_version 期待値チェックの最小実装だけを置く。payload decode と encode の本実装は行わない。
