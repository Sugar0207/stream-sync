...
same-PC smoke の標準設定は上記を採用します。必要な rerun だけ比較条件を変えます。
...
現在は `256MiB receive buffer + cap1024 + summary-only` を same-PC smoke の標準設定とする。
次の比較は cap 探索ではなく、longer-run でこの設定が安定するかを確認する。
