...
- 次の中心は heartbeat timeout notice wakeup 実行本体に進む前の境界整理、RTT / offset metrics snapshot の具体的な export cadence / dashboard refresh 方針、eventual while-loop repeated invocation skeleton / stop flag refresh の最小範囲整理
...
3. eventual while-loop repeated invocation skeleton / stop flag refresh の最小範囲を整理する
