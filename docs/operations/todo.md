...
## 現在位置
- 2026-06-03 Program-first validation mode now also suppresses remaining non-Program Preview one-shot decode after a previous Preview output exists. This keeps defaults unchanged and only affects `--program-first-validation-mode` when ProgramOutput / explicit Program continuous decode can resolve a Program source. The Program continuous source keeps the existing continuous one-shot suppression behavior, while other Preview sources return decode-deferred placeholders instead of spawning one-shot FFmpeg. New diagnostics are `program_first_suppressed_preview_one_shot_decode_count`, `program_first_suppressed_preview_one_shot_decode_slot_counts`, `program_first_program_only_decode_path_enabled`, and `program_first_remaining_one_shot_decode_count`. The next human validation should confirm the previous `one_shot_decode_attempt_slot_counts=slot0:157|slot1:0` pattern collapses to suppressed Preview counts and that Program FPS improves or reveals a non-decode bottleneck
...
13. [ ] rerun longer Program-first validation with OBS capturing `StreamSync Program Output` using `--enable-program-output-window --program-selected-client-id player2 --enable-program-continuous-decode --program-continuous-decode-mode smooth-latest --program-first-validation-mode`; compare `program_first_validation_enabled`, `program_first_program_only_decode_path_enabled`, `program_first_suppressed_preview_one_shot_decode_count`, `program_first_suppressed_preview_one_shot_decode_slot_counts`, `program_first_remaining_one_shot_decode_count`, Preview compose skip/reuse counts, Program render FPS/success/failure counts, continuous latest/stale counts, one-shot fallback count, placeholder/black counters, and perceived stutter
14. [ ] use a larger handoff budget for long Program-first validation, e.g. `max_handoff_requests=64000` or the current no-budget / very-large-budget validation setting, to avoid another `MaxHandoffRequestsReached` stop at 3000 attempts
15. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
16. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
17. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
18. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
19. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
