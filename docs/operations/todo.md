...
- 次の中心は heartbeat timeout notice wakeup 実行本体に進む前の境界整理、RTT / offset metrics snapshot の具体的な export cadence / dashboard refresh 方針、actual timer / retry / cleanup sequencing から future completed loop body の実行順序へ進む最小範囲整理
...
3. actual timer / retry / cleanup sequencing から future completed loop body の実行順序へ進む最小範囲を整理する
