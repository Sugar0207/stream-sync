...
## Next Items
1. next step では `S:\stream-sync\manual-logs\two-client-render-rerun-20260518-111013` を latest comparison run、`S:\stream-sync\manual-logs\two-client-render-rerun-20260517-223121` を previous scaled-pass run として比較し、`one_shot_decode_slow_first_byte_reason_counts` と `one_shot_decode_slow_output_read_reason_counts` の `source_recovered:2` 偏りが再現するかを narrow に切り分ける
2. `S:\stream-sync` の next rerun では `first_render_attempt_index`、`first_render_elapsed_ms`、`no_render_before_first_render`、`no_frame_count`、`handoff_error_count`、server `no_frame_count` を前回比で取り、startup/no-frame availability が after-first FPS と独立した主課題かを確認する
