...
2. one-shot path を残したまま、safer diagnostics 候補として `one_shot_decode_stdin_close_elapsed_ms`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdout_first_byte_elapsed_ms`、derived throughput / per-payload-kb summary を優先度つきで整理する
