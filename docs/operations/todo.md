...
## 直近でやること
1. operator-facing control surface の設計を固める:
   - `AllView` と `Focused(slot_index)` の state model を固定する
   - current fixed-order `--four-view-four-real-handoff-preview-loop ...` を内部 baseline として維持する
   - slot/client/run binding を崩さない薄い control layer の shape を決める
2. `Focused(slot_index)` の最小実装に進む:
   - slot `0..3` を full-window focused view として扱う
   - full hotkey/UI を入れずに state transition と render semantics だけを実装する
3. all-view / focused-view manual validation を行う:
   - guarded `4`-client recipe を再利用する
   - `AllView -> Focused(slot_index) -> AllView` の遷移を確認する
