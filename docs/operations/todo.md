...
- 未完了の中心は targetTime-selected frame -> decode/render connection、4-view sync orchestration、dashboard UI rendering、continuous receive/send loop 本体、実キュー / 実送信 / 継続ログ出力
...
2. targetTime-selected 2-view frame を decode/render へ接続する最小境界を分けて設計する
...
- [ ] 2-view targetTime-selected frame decode/render connection
- [ ] 2 人同期表示
- [ ] 4 人 2x2 表示
- [ ] OBS 取り込み確認
...
## Next Items
1. production H.264 encoder configuration / error logging policy
2. targetTime-selected frame -> decode/render connection boundary
3. continuous acquisition / frame arrived wait boundary
4. 2-view sync PoC after targetTime-selected decode/render is isolated
