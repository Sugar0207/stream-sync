...
## 直近でやること
1. existing `ServerVideoFrameQueueReadBoundary` を使う server 側 single-request handoff handler と、`SwitcherQueuedFrameHandoff` を実装する switcher 側 client adapter を追加する
2. named pipe 実装前提の transport-neutral handoff codec を server/switcher runtime へ接続する境界を整理する
