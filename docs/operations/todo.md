...
## 直近でやること
1. `--four-view-real-handoff-preview-loop [pipe-name] [real-slot-index] [client-id] [run-id] [frames]` の manual actual pass を記録する
2. existing server `--receive-auth-video-queue-and-serve-handoff-many` と新 switcher command を組み合わせて、`1` real slot + `3` placeholder slots の 4-view clean output/OBS downstream 挙動を再手動確認する
