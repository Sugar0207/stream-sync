...
- production H.264 encoder configuration / error logging policy の docs 設計も固定済みで、first production profile は client-owned `ffmpeg + libx264` low-latency profile として `720p30` / `yuv420p` / `bitrate_kbps=4500` / `gop_frames=30` / `preset=ultrafast` / `tune=zerolatency` / `profile=main` / `level=3.1` を基準にする。current PoC の成功 recipe は維持し、次実装 slice は config surfacing / summary fields / FFmpeg availability visibility を優先し、hardware encoder integration は後段へ回す
...
1. production H.264 encoder configuration / error logging policy
   - first slice は client TOML に `[video.encoder]` を追加し、validated な `libx264` low-latency default を explicit にする
   - bounded sender summary に encoder profile / FFmpeg availability-version / last encode error kind を追加する
   - current successful manual recipe を壊さないため、未設定時は current implicit defaults を維持する
