...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260518-022141` では、`--disable-persistent-decoder` による pure one-shot-only rerun 上で two-real preview loop 限定 scaled decode output は runtime PASS 継続だった。`one_shot_decode_output_width=640`、`one_shot_decode_output_height=360`、`one_shot_decode_output_pixel_format=Bgra8`、`one_shot_decode_scaled_output_enabled=true`、`one_shot_decode_scaled_output_reason=two_real_slot_size`、`one_shot_decode_expected_output_bytes_per_frame=921600` を維持しており、latest FPS 悪化は scaled decode failure ではない
- persistent decoder config-disabled toggle も latest rerun で PASS 継続だった。`persistent_decode_config_enabled=false`、`persistent_decode_attempt_count=0`、`persistent_decode_timeout_count=0` を維持しており、request/response persistent decoder は引き続き凍結候補として扱う
- server / client / transport はこの rerun でも成立している。server は `packets_received=38692`、`frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0` で、client1/client2 も `frames_sent=900`、`effective_output_fps=26.863|26.946` を維持した
- latest rerun の switcher は `effective_render_fps_after_first_render=9.469`、`effective_render_fps=7.543` まで悪化し、previous scaled-pass rerun `manual-logs/two-client-render-rerun-20260517-223121` の `16.579` より明確に低かった。ただし noisy comparison として扱い、latest FAIL を scaled decode slice の失敗には帰属させない
- latest rerun では decode-side variance が大きかった。`decode_attempt_count=42`、`decode_success_count=42`、`one_shot_decode_attempt_count=42`、`one_shot_decode_elapsed_ms=8809`、`one_shot_decode_input_write_elapsed_ms=3132`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms=3781`、`one_shot_decode_stdout_first_byte_elapsed_ms=3687`、`one_shot_decode_output_read_elapsed_ms=4655`、`one_shot_decode_output_read_exact_elapsed_ms=3729` で、previous scaled-pass rerun の `decode_attempt_count=26`、prior first-byte diagnostics rerun `manual-logs/two-client-render-rerun-20260517-232111` の `stdin_write_to_stdout_first_byte=1093` / `output_read_elapsed_ms=1333` より重い
- compose / GDI side も latest rerun では軽くなかった。`quad_view_incremental_update_count=66`、`quad_view_full_compose_count=1` で incremental compose 自体は PASS 継続だが、`quad_view_compose_elapsed_ms=1667`、`gdi_paint_wait_elapsed_ms=132`、`placeholder_visual_changed_count=61`、`scheduler_status=PartialSelected` だったため、decode-side だけでなく compose/GDI variance も混ざる noisy run と整理する
- current code-path 調査では、actual `decode_attempt_count` は `apps/switcher/src/main.rs` の two-real preview loop decode cache miss 時にだけ増える。cache key は `width` / `height` / `source_identity(client_id, run_id, frame_id)` に依存し、preview-loop decodes では payload bytes そのものではなく frame identity を見る。一方 `unchanged_frame_reuse_count` / `skipped_decode_unchanged_frame_count` は `apps/switcher/src/lib.rs` の unchanged-frame reuse 判定で、slot が `UseUpdatedFrame` を保ちつつ `client_id/run_id/frame_id` が一致した場合にしか増えない
- したがって latest `decode_attempt_count=42` の増加候補は、scaled output failure ではなく、real slot で `frame_id` churn が増えたこと、`Selected` 継続より `NoFrameAvailable` / `HandoffError` / placeholder / held-previous への遷移が増えて unchanged reuse が減ったこと、または partial selection に伴って real slot の visual identity が落ち着かなかったことに置く。`selected_source_changed_count` は render/placeholder churn の手掛かりにはなるが、source kind そのものは decode cache key ではない
- incremental quad compose は runtime PASS 継続と整理する。ただし latest rerun では `quad_view_incremental_update_count=66` / `quad_view_full_compose_count=1` を維持しつつも `quad_view_compose_elapsed_ms=1667`、`gdi_paint_wait_elapsed_ms=132`、`placeholder_visual_changed_count=61` なので、compose/GDI variance を完全に無視はしない
- current one-shot path の計測境界は維持されている。`decode_input_write_elapsed_ms` は `stdin.write_all(&input.encoded_payload)` だけ、`one_shot_decode_stdin_close_elapsed_ms` は `stdin` close/drop 相当だけ、`one_shot_decode_stdout_first_byte_elapsed_ms` は spawn から first stdout byte まで、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms` は write 完了から first stdout byte まで、`decode_output_read_exact_elapsed_ms` は first byte 後の bounded rawvideo read、`decode_output_read_elapsed_ms` は bounded read + extra-output probe まで、`child.wait()` は別カウンタだと整理できる
- current scaled decode slice は latest rerun でも runtime PASS 継続で、two-real preview loop の expected stdout shape は引き続き `640x360 Bgra8` / `921600 bytes/frame` のまま保たれている。previous scaled-pass rerun では `decode_stdout_expected_bytes_total=23961600` まで下がっており、latest noisy run の FPS 悪化は expected raw output size の後退では説明しない
- latest diagnostics では `stdin close` は `0ms` のままで、主に重かったのは `stdin write`、`write->stdout first byte wait`、`stdout read` だった。`one_shot_decode_input_write_elapsed_ms=3132`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms=3781`、`one_shot_decode_output_read_elapsed_ms=4655` が並んでいるため、first-byte wait 増加が decode attempt 増加だけで説明できるか、per-attemptでも悪化しているかを next candidate にする
- latest rerun の payload shape は `one_shot_decode_input_payload_bytes_min=53416`、`max=115991`、`avg=91042.357` で、previous scaled-pass rerun `avg=195142.192` よりむしろ小さい。したがって今回の first-byte/read 悪化は payload size 増加では説明しにくく、attempt frequency と per-attempt variance の切り分け優先度を上げる
- latest implementation step で追加した safer diagnostics は有効のまま維持する。summary には `one_shot_decode_stdin_close_elapsed_ms`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdout_first_byte_elapsed_ms`、`one_shot_decode_write_throughput_bytes_per_ms`、`one_shot_decode_input_write_elapsed_per_payload_kb`、`one_shot_decode_read_throughput_bytes_per_ms` が出る
- latest diagnostics-only slice では、attempt frequency と per-attempt wait variance を summary 上で分けて見られるようにした。new fields は `one_shot_decode_first_byte_elapsed_ms_max`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms_max`、`one_shot_decode_first_byte_slow_count`、`one_shot_decode_first_byte_slow_threshold_ms`、`one_shot_decode_output_read_slow_count`、`one_shot_decode_output_read_slow_threshold_ms`、`one_shot_decode_attempt_source_counts`、`one_shot_decode_attempt_slot_counts`、`one_shot_decode_attempt_reason_counts`、`decode_cache_miss_slot0_count`、`decode_cache_miss_slot1_count`、`skipped_decode_unchanged_slot0_count`、`skipped_decode_unchanged_slot1_count` で、two-real preview loop の decode decision / cache miss / unchanged reuse 観測だけを足し、one-shot decode 挙動自体は変えていない
- current code-path では preview-loop decode cache key は `width` / `height` / `source_identity(client_id, run_id, frame_id)` を使うが、two-real preview loop の `source_identity` 文字列は現在 `selected` 固定で入れている。したがって `queue` / `retained_keyframe` の new attempt source counts は cache key の直接説明ではなく、「どの handoff source 由来の decode attempt が多かったか」を見る補助 diagnostics として扱う
- current request/response persistent decoder 実装は凍結候補として扱う。continuous-stream decoder rewrite は別設計候補として保留し、今回の baseline 以後も main optimization path へ戻さない。Production Readiness は FAIL 継続
- branch diagnostics / materialization diagnostics / source-only `selected_source` churn fix は維持する。direct compose は current half-scale helper の visual semantics を壊しておらず、Production Readiness は 30fps target 未達のまま FAIL 継続
...
## 直近でやること
1. next step では `apps/switcher/src/lib.rs` の current one-shot FFmpeg decode path を narrow に見直し、`decode_process_spawn_elapsed_ms` / `decode_input_write_elapsed_ms` / `decode_output_read_exact_elapsed_ms` / `decode_output_read_elapsed_ms` の計測境界と支配度を code path ベースで整理する
2. rerun 比較では `manual-logs/two-client-render-rerun-20260517-183552` を compose-pass baseline とし、`one_shot_decode_attempt_count` / `one_shot_decode_elapsed_ms` / `one_shot_decode_input_write_elapsed_ms` / `one_shot_decode_output_read_elapsed_ms` / `one_shot_decode_output_read_exact_elapsed_ms` / `decode_input_payload_bytes_total` / `decode_stdout_expected_bytes_total` を優先して見る
3. keyframe payload size と elapsed の関係、payload bytes と expected stdout bytes の比率、spawn/write/read/wait のどこに safer slice があるかを見たうえで、request/response persistent decoder ではない narrow one-shot follow-up を選ぶ
4. incremental quad compose は PASS として維持し、same-PC rerun 比較では `quad_view_full_compose_count` / `quad_view_incremental_update_count` / `quad_view_incremental_skip_reason_counts` を regression guard として残す
5. request/response persistent decoder / continuous-stream rewrite / shared-memory / GPU backend / distributed-PC widening には進まず、same-PC two-real の next dominant candidate を one-shot decode I/O に絞って閉じる
...
## Next Items
1. next step では `S:\stream-sync\manual-logs\two-client-render-rerun-20260518-022141` を latest noisy run、`S:\stream-sync\manual-logs\two-client-render-rerun-20260517-223121` を previous scaled-pass run として比較し、new summary fields `one_shot_decode_attempt_slot_counts`、`one_shot_decode_attempt_reason_counts`、`decode_cache_miss_slot0_count`、`decode_cache_miss_slot1_count`、`skipped_decode_unchanged_slot0_count`、`skipped_decode_unchanged_slot1_count` と `slot0/slot1 frame_id_changed_count` / `selected_source_changed_count` の相関を narrow に切り分ける
2. `S:\stream-sync` の next rerun では `one_shot_decode_first_byte_elapsed_ms_max`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms_max`、`one_shot_decode_first_byte_slow_count`、`one_shot_decode_output_read_slow_count`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdout_first_byte_elapsed_ms`、`one_shot_decode_output_read_elapsed_ms` を前回比で取り、attempt frequency と per-attempt first-byte/read variance のどちらが支配的かを確認する
3. request/response persistent decoder は凍結候補のまま維持し、continuous-stream decoder rewrite、one-shot decoder I/O の大改造、raw BGRA 以外の intermediate pixel format 比較は次候補としてだけ残す
