...
## 直近でやること
1. queue-backed 2-view targetTime scheduler result を既存 2-view decode/render path へ渡す最小 adapter 範囲を整理する
...
## Next Items
1. Plan how the queue-backed scheduler should feed the existing 2-view decode/render path without late-drop mutation.
2. Define the smallest adapter from queue-backed scheduler per-view results to existing selected-frame decode/render input.
