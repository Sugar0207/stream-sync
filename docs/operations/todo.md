...
## Next Items
1. 次回の same-PC concurrent human rerun では、`frames_rendered=frames_attempted` strict count ではなく `no final HandoffError` / final real slots `Selected` / final `handoff_response_kind=FrameRead` / final `io_error=none` / `render_failures=0` / `clean_output_render_result_kind=Rendered` を主 gate に使う
2. rendered/attempted ratio を改善したい場合だけ、switcher start timing / client start timing / planned frame count / warm-up handling の調整 recipe を固める
3. 54 tick 分の non-render 内訳まで必要になった場合だけ、per-tick render outcome の追加 observability を別 slice として切り出す
