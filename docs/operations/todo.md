...
## 直近でやること
1. fallible single-client targetTime handoff source result を 2-view scheduler へ接続する最小方針を計画する。handoff error は partial/no-frame/waiting に潰さない
2. その方針に基づき、scheduler 側で handoff error を明示的に保持する最小 validation slice を実装する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest 2-view scheduler path for `SwitcherSingleClientTargetTimeHandoffSourceResult` without collapsing handoff errors into partial/no-frame/waiting.
