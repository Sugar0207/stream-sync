...
## 現在位置
- 2026-06-03 latest snapshot-style low-cost Preview validation used `--program-first-preview-refresh-interval 5 --program-first-preview-decode-refresh-interval 90 --operator-preview-snapshot-retention`. OBS setup is correct: OBS captured `StreamSync Program Output`, did not capture `StreamSync 4-view Output`, and Program did not mix 4-view / borders / debug UI / Preview labels. ProgramOutput remains near-MVP / partial PASS: black / placeholder none, perceived stutter small, continuous latest used (`program_render_used_continuous_latest_count=2736`), one-shot fallback `0`, black / placeholder counters `0`, but `program_render_effective_fps=16.201`. Snapshot retention fixed Preview black/flicker (`operator_preview_snapshot_retention_enabled=true`, `operator_preview_snapshot_reuse_count=2743`, `operator_preview_placeholder_avoided_by_snapshot_count=2743`, `operator_preview_slot_black_after_snapshot_count=0`; client1/client2 were not black and did not flicker). However Preview remained too slow for operator monitoring (`operator_preview_render_effective_fps=3.233`, decode refresh success `31`, one-shot decode attempts `31`). Same-loop low-cost Preview refresh tuning is now limited / paused; do not keep lowering the Preview refresh interval in this path. Recommended next direction is to close ProgramOutput near-MVP first, treat current Preview as stable snapshot-only, and design a separate Preview cadence/runtime or lighter renderer later
- 2026-06-03 latest Program-first + `smooth-latest` + Preview one-shot suppression OBS validation is PASS / near-MVP for ProgramOutput: OBS captured `StreamSync Program Output`, did not accidentally capture `StreamSync 4-view Output`, Program had no Preview labels / borders / debug UI mixed in, black / placeholder did not occur, perceived stutter was small, `program_render_effective_fps=21.795`, `continuous_decode_output_throughput_fps=27.102`, `program_render_used_one_shot_fallback_count=0`, `one_shot_decode_attempt_count=0`, and `program_first_remaining_one_shot_decode_count=0`. In the same mode the 4-view Preview did not display (`frames_rendered=0`, `clean_output_render_result_kind=NoRenderableQuadView`, `output_width=none`, `output_height=none`), so `--program-first-validation-mode` is explicitly treated as ProgramOutput validation mode, not final operator mode. Practical operation now has a narrow opt-in validation candidate: `--program-first-preview-refresh-interval <ticks>` attempts low-cost Preview compose/render every N ticks while keeping non-Program Preview one-shot suppression active and ProgramOutput prioritized
...
24. [ ] close ProgramOutput near-MVP first: record remaining Program FPS / smoothness acceptance threshold, black/placeholder guard, OBS target separation, and current static Program selection constraints
25. [ ] design separate Preview cadence/runtime or lighter renderer path for future operator monitoring; treat current 4-view Preview as stable snapshot-only, not the final monitoring Preview
26. [ ] design Program source switching over hotkey or control pipe after ProgramOutput closeout; keep current CLI/static selection as the only supported switch for now
27. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
28. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
29. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
30. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
