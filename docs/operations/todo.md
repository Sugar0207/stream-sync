...
## 現在位置
- 2026-06-03 latest operator low-cost Preview validation with `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30` plus Program-selected Preview reuse made both Preview slots visible sometimes, but it was still not useful: client1 and client2 both flickered, Program perceived stutter became large, and Program render FPS dropped to `program_render_effective_fps=15.581` while one-shot Preview decode cost was high (`one_shot_decode_attempt_count=94`, `one_shot_decode_elapsed_ms=16349`, `avg_decode_elapsed_ms=174.096`). ProgramOutput still used continuous latest (`program_render_used_continuous_latest_count=2799`, one-shot fallback `0`, black / placeholder counters `0`), but low-FPS video Preview is now treated as too expensive / unstable. Current code adds opt-in `--operator-preview-snapshot-retention` for Program-first low-cost Preview mode so Preview holds last-visible per-slot snapshots instead of flickering back to placeholder/deferred output. Next validation should start with `--program-first-preview-refresh-interval 30 --program-first-preview-decode-refresh-interval 90 --operator-preview-snapshot-retention`
...
22. [ ] validate snapshot-style low-cost Preview with `--program-first-preview-refresh-interval 30 --program-first-preview-decode-refresh-interval 90 --operator-preview-snapshot-retention`; compare snapshot reuse/creation/placeholder-avoided/black-after-snapshot diagnostics, Program render FPS, Program black/placeholder counters, one-shot decode elapsed, and operator usefulness
23. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
24. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
25. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
26. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
27. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
