...
- minimal manual sender `--send-control-command [control-pipe-name] [command]` も追加済みで、wrapper 本体をまだ作らずに same-session control pipe を 1 request / 1 response で叩ける。stdin / `--commands` baseline と existing successful handoff recipes は維持している
- same-session bounded server lifecycle については、現状の request-budget formula `render_command_count * max_ticks_per_command * real_slot_count` と extra flush read requirement を docs に残した上で、wrapper 設計の blocker にはしない判断で固定した。bounded handoff summary flush/exit の改善は later narrow polish として扱う
...
## 直近でやること
1. separate local control pipe の guarded manual validation を記録する:
   - `--four-view-controlled-handoff-preview-loop ... --control-pipe streamsync-control-dev` を real guarded `4`-client baseline で回す
   - `--send-control-command streamsync-control-dev "status|focus 0..3|all|quit"` の main success path と rejected path を記録する
...
2. same-session bounded server lifecycle の narrow polish が本当に必要か判断する:
   - request-budget formula は docs 運用で十分か
   - extra flush read を later wrapper automation に持ち込むべきか
