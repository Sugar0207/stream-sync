...
## 現在位置
- latest same-PC `2`-client render reuse rerun では runtime は PASS、client output FPS は `29.612|29.495fps` まで戻り、quad-view composition は主ボトルネックから外れた。一方で switcher は `effective_render_fps_after_first_render=2.675` に留まり、current top suspect は `decode_elapsed_ms=7613` / `avg_decode_elapsed_ms=126.883`、secondary suspect は `render_buffer_copy_elapsed_ms=4935` と `render_prepare_elapsed_ms=1937`。今回の code slice では `--four-view-two-real-handoff-preview-loop` の decode path を分解し、final summary に `decode_process_spawn_elapsed_ms` / `decode_input_write_elapsed_ms` / `decode_output_read_elapsed_ms` / `decode_process_wait_elapsed_ms` / `decode_pixel_convert_elapsed_ms` / `decode_buffer_allocation_count` / `decode_output_bytes_total` / `avg_decode_output_read_elapsed_ms` / `avg_decode_process_spawn_elapsed_ms` を追加した。現行 FFmpeg runtime は decode ごとに process spawn し、H.264 payload を stdin に書き、stdout raw BGRA を読む。pixel format conversion は FFmpeg の `-pix_fmt bgra` に任せるため Rust 側の `decode_pixel_convert_elapsed_ms` は現状 `0`。さらに source identity 付き decoded-frame cache を追加し、`client_id + run_id + frame_id + source_identity` が同じ decoded retained frame は再 decode せず、frame_id が変われば payload が同じでも再 decode する。source-error placeholder semantics、unchanged-frame decode reuse、visual identity / composed-frame reuse、placeholder row cache、incremental composition、render reuse、client encode/capture behavior 非変更、distributed-PC validation docs 非変更は維持
...
## Next Items
1. same-PC `2`-client smoke を rerun し、switcher final summary の decode phase breakdown / decoded-frame cache hit / FPS / render buffer copy diagnostics を確認する
