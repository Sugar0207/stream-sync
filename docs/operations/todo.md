...
- `--receive-send-three` は stateless RTT / offset candidate を `ServerHeartbeatRttOffsetState` へ 1 回 commit し、stdout に `heartbeat_rtt_offset_entries` / `heartbeat_rtt_offset_samples` を表示できる
...
- heartbeat timeout は `ServerHeartbeatTimeoutPolicy` と `evaluate_timeout` で 1 client 分を `Alive` / `TimedOut` / `NoHeartbeat` に分類できる。`TimedOut` 結果から auth registry invalidation command、timeout log event input、`AuthExpired` notice plan を作る最小 action boundary と、registry invalidation / caller-owned timeout log writer / notice queue item handoff までを適用する最小 apply boundary も追加済み
- `ServerHeartbeatTimeoutLoopTickBoundary` は future continuous loop が選んだ 1 client について timeout evaluation -> action plan -> apply を 1 回だけ呼べる。client iteration、cadence、stop condition、notice queue storage、send wakeup は未実装
...
- `ClientStats` receive route / gate / registered handler bridge と、`HeartbeatAckObservation` を既存 timebase plan / stateless calculator へ渡す最小接続、RTT / offset latest estimate state commit、same-run delta threshold の最小 candidate policy 境界は完了。継続送信 loop、metrics state commit、completed smoothing / outlier model は未実装
...
- 次の中心は heartbeat timeout loop tick の notice queue storage / send wakeup 方針、RTT / offset candidate policy を commit 前に接続する方針、continuous heartbeat loop に進む前の境界整理
...
## 直近でやること
1. heartbeat timeout loop tick の notice queue storage / send wakeup 方針を整理する
2. RTT / offset candidate policy を commit 前に接続する方針を整理する
...
- [ ] RTT / offset candidate policy を commit 前に接続する
- [ ] offset 平滑化を実装する
