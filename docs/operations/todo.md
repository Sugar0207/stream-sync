...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260518-111013` では、`--disable-persistent-decoder` による pure one-shot-only rerun 上で two-real preview loop 限定 scaled decode output は runtime PASS 継続だった。`one_shot_decode_output_width=640`、`one_shot_decode_output_height=360`、`one_shot_decode_output_pixel_format=Bgra8`、`one_shot_decode_scaled_output_enabled=true`、`one_shot_decode_expected_output_bytes_per_frame=921600` を維持しており、latest rerun でも scaled decode failure shape は出ていない
...
- server / client / transport はこの rerun でも成立している。server は `packets_received=34059`、`frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0` で、client1/client2 も `frames_sent=900`、`effective_output_fps=26.110|26.070` を維持した
- latest rerun の switcher は `effective_render_fps_after_first_render=17.749`、`effective_render_fps=11.497` まで改善し、previous scaled-pass rerun `manual-logs/two-client-render-rerun-20260517-223121` の `16.579` と、previous correlation rerun `manual-logs/two-client-render-rerun-20260518-080637` の `13.760` を上回った。ただし `first_render_attempt_index=137`、`first_render_elapsed_ms=5025`、`no_render_before_first_render=136` は startup 側の注意点として残る
...
- latest rerun の decode attempt frequency は大きな異常ではない。`decode_attempt_count=20`、`decode_success_count=20`、`one_shot_decode_attempt_count=20` まで下がり、attempt slot も `one_shot_decode_attempt_slot_counts=slot0:10|slot1:10` で均等だった。今回の主犯を attempt frequency や slot bias に置かない
- latest attempt/source diagnostics でも、attempt source は `one_shot_decode_attempt_source_counts=queue:20|retained_keyframe:0|unknown:0` に揃い、attempt reason も `one_shot_decode_attempt_reason_counts=frame_id_changed:18|cache_miss:0|previous_unavailable:0|source_recovered:2|unknown:0` だった。`input_write_outlier` も `one_shot_decode_input_write_outlier_count=0` のため、input-write outlier も今回の主犯ではない
- slow correlation diagnostics では、slow first-byte / slow output-read はどちらも `3` 件だけで、`slot0:1|slot1:2`、`queue:3|retained_keyframe:0|unknown:0`、`frame_id_changed:1|source_recovered:2` だった。したがって slow path は少数で、source bias は queue 側固定だが、reason bias は `source_recovered` に寄っていると整理する
- latest rerun の decode-side metrics も overall では軽かった。`one_shot_decode_elapsed_ms=1615`、`one_shot_decode_elapsed_ms_max=111`、`one_shot_decode_input_write_elapsed_ms=665`、`one_shot_decode_input_write_elapsed_ms_max=52`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms=599`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms_max=88`、`one_shot_decode_output_read_elapsed_ms=732`、`one_shot_decode_output_read_elapsed_ms_max=98` で、previous scaled-pass rerun や `20260518-080637` より改善している
- latest rerun の payload shape は `one_shot_decode_input_payload_bytes_avg=81371.600` で、`one_shot_decode_input_write_outlier_payload_bytes_avg=n/a` だった。今回の evidence では payload size と input-write outlier の相関は読めず、次の主候補を `source_recovered slow path` と `startup/no-frame availability` に移す
- incremental quad compose は runtime PASS 継続で、`quad_view_incremental_update_count=43`、`quad_view_full_compose_count=1`、`quad_view_compose_elapsed_ms=537`、`gdi_paint_wait_elapsed_ms=18` を維持した。compose / GDI variance は regression guard として残すが、latest rerun の first-order culprit とは置かない
...
## Next Items
1. `S:\stream-sync` の next rerun では `first_render_attempt_index`、`first_render_elapsed_ms`、`no_render_before_first_render`、`no_frame_count`、`handoff_error_count`、server `no_frame_count` を前回比で取り、startup/no-frame availability が after-first FPS と独立した主課題かを確認する
2. `source_recovered` slow bias の再現確認では、`one_shot_decode_slow_first_byte_reason_counts`、`one_shot_decode_slow_output_read_reason_counts`、`one_shot_decode_attempt_reason_counts` を基準に、post-no-frame / post-handoff-error recovery と見てよいかを narrow に切り分ける
3. code を触る場合は diagnostics-only に限定し、最小候補を `first_render_wait_reason_counts`、`no_render_before_first_render_reason_counts`、`startup_no_frame_count`、`startup_handoff_error_count`、`startup_selected_but_not_rendered_count`、`source_recovered_after_no_frame_count`、`source_recovered_after_handoff_error_count` に絞る。persistent decoder revive、continuous-stream decoder rewrite、one-shot decoder I/O の大改造には進まない
