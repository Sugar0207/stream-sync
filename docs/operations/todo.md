...
- next minimal diagnostics should focus on backlog shape before behavior changes: pending correspondence frame_id min/max, latest input minus latest output lag, input-to-output lag avg/max, output throughput FPS, reader full-frame elapsed max, output lag to selected, pending age, and queue drop reason counts. Average/max output latency with enqueue timestamps is useful but held behind simpler frame-id/throughput fields
...
## Next Items
1. `docs/operations/continuous-output-lag-plan.md` に沿って、slot0 / two-real / opt-in continuous 限定の diagnostics-only first slice を実装するかを確認する
2. 実装する場合は `continuous_decode_pending_correspondence_frame_id_min/max`、`continuous_decode_latest_input_minus_latest_output_lag`、`continuous_decode_input_to_output_lag_frames_avg/max`、`continuous_decode_output_throughput_fps` を最小優先にする
3. 次点で `continuous_decode_reader_full_frame_elapsed_ms_max`、`continuous_decode_output_lag_to_selected_frames`、`continuous_decode_correspondence_pending_age_ms`、`continuous_decode_queue_drop_reason_counts` を検討する
