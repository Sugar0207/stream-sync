...
8. [ ] rerun longer runtime validation with OBS capturing `StreamSync Program Output` and compare missing-before/after-first-render, Program frame reuse, placeholder/black counters, and perceived stutter
9. [ ] investigate Program stutter seams in the current shared loop: one-shot decode blocking, Preview quad composition cost, CPU scale/copy, and whether Program can be decoupled before hotkey/control-pipe switching
10. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
11. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
12. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
13. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
14. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, suppression, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
