...
## 直近でやること
1. operator-facing control surface の最小設計に進む:
   - current fixed-order `--four-view-four-real-handoff-preview-loop ...` を内部 baseline として維持
   - その上に slot/client/run binding を崩さない薄い operator-facing wrapper をどう置くか決める
2. operator-facing surface に必要な最低限の stdout / state visibility を決める:
   - current selected bindings
   - current scheduler status
   - per-slot result kind
   - last handoff / parse / io / decode error visibility
3. player1 単独 isolation が必要な場合は引き続き `--four-view-real-handoff-preview-loop streamsync-handoff-dev 0 player1 streamsync-dev-session 5` を baseline として使う
