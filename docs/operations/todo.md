...
10. [ ] rerun longer runtime validation with OBS capturing `StreamSync Program Output` using `--enable-program-output-window --program-selected-client-id player2 --enable-program-continuous-decode --program-continuous-decode-mode smooth-latest`; compare `program_render_used_continuous_latest_count`, stale-but-accepted count, one-shot fallback count, frame reuse, placeholder/black counters, and perceived stutter
11. [ ] investigate remaining Program stutter seams after the smooth-latest rerun: continuous output throughput, remaining first-frame/disabled fallback, Preview quad composition cost, CPU scale/copy, and whether Program can be further decoupled before hotkey/control-pipe switching
12. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
13. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
14. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
15. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
16. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, suppression, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
