...
## 直近でやること
1. fallible scheduler -> decode/render adapter output を次段へどう接続するかを計画する。handoff error を既存 decode/render input に潰せないケースは明示的に扱う
2. fallible adapter output の source-error skip を display policy / placeholder path へ渡す最小 validation slice を検討する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest follow-up connection for `SwitcherTwoViewHandoffSchedulerDecodeRenderAdapterOutput`, especially how source-error skip instructions should surface before display policy without fake frames.
