...
## 現在位置
- latest same-PC `4`-client all-real concurrent validation は `manual-logs/four-client-20260513-151543` を latest evidence として FAIL 判定にした。server ready / stopped summary、client1..4 auth/send、server queue participation、named-pipe handoff transport は PASS しており、primary failure bucket は `Switcher Selection / Decode / Render Failure` とする。switcher final state は全 real slot が `WaitingForFrameAtOrBeforeTarget`、`clean_output_render_result_kind=NoRenderableQuadView`、final slot diagnostics は `handoff_response_kind=FrameRead` / `parse_error=none` / `io_error=none` / `retained_keyframe_available=true` / `selected_frame_available=false` / `decode_attempted=false` を示した
- 4-client failure の主因候補は switcher 4-real path の read mode / targetTime 差分として固定した。2-client PASS path は `--four-view-two-real-handoff-preview-loop ... preview-latest-decodable` により `PreviewLatestDecodableIfAtOrBefore` / `InspectLatestDecodable` を使い、retained-keyframe fallback を使える。一方、current `--four-view-four-real-handoff-preview-loop` は preview-mode 引数を持たず、固定で `PreviewLatestIfAtOrBefore` / `InspectLatest` を使うため retained keyframe を選択に使わない。さらに 2-real loop は targetTime を tick ごとに再計算するが、4-real loop は開始時に 1 回計算した targetTime を全 tick で再利用している
...
## Next Items
1. `--four-view-four-real-handoff-preview-loop` に 2-real path と同等の preview-mode wiring を追加する。候補は optional `[preview-oldest|preview-latest|preview-latest-decodable]` 引数で、既存 2-client behavior は維持する
2. 4-real preview loop の targetTime を 2-real path と同じく tick ごとに再計算する最小 slice を実装し、same-PC `4`-client all-real validation を `preview-latest-decodable` で rerun する
3. rerun 後も final state が Waiting / NoRenderable の場合だけ、same-PC saturation、persistent decoder context、retry/backoff、distributed-PC、OBS capture follow-up の順に再分類する
