...
## Next Items
1. continuous `output_pending` の追加 diagnostics を slot0 限定で設計または実装する。優先候補は input frame_id gap、input SPS/PPS/IDR/non-IDR NAL kind、FFmpeg stderr summary、stdout reader blocked / no-output-after-input counters
2. `apps/switcher/src/main.rs` の current render-demand feed を前提に、slot0 per-client continuous feed/drain policy の最小設計を切る。selected frame exact lookup / one-shot fallback は残し、latest decoded fallback や targetTime-aware lookup にはまだ進めない
3. 人間側 `S:\stream-sync` の次回 rerun では `--disable-persistent-decoder --enable-continuous-stream-decoder` を維持し、追加 diagnostics で sparse input / FFmpeg stderr / stdout pending のどれが主に出るか確認する。slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode はまだ進めない
