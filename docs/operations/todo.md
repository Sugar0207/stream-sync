...
## Next Items
1. `S:\stream-sync\manual-logs\two-client-render-rerun-20260520-005310` を latest evidence として、continuous decoder output lag / output pending correspondence / stdout read latency / decoded queue-drop policy を docs-first で整理する
2. 次の設計では `output_pending_correspondence_count` が増える理由、stdout read elapsed / reader blocked の意味、input/output count 差分、`queue_len=30` と `dropped_stale_count` の関係、one-shot fallback による二重負荷の可能性を分けて扱う
3. 追加 diagnostics 候補を最小化する: `continuous_decode_output_latency_frames_avg/max`、`continuous_decode_input_to_output_lag_frames_avg/max`、`continuous_decode_correspondence_pending_age_ms`、`continuous_decode_queue_drop_reason_counts`、`continuous_decode_output_lag_to_selected_frames`、`continuous_decode_reader_full_frame_elapsed_ms_max`、`continuous_decode_output_throughput_fps`
4. 次 step でも Production Readiness FAIL を維持し、allowed lag threshold 変更、slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode、one-shot fallback 削除、無制限 latest decoded fallback、feed max count 変更には広げない
