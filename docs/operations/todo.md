...
## Next Items
1. 人間側 `S:\stream-sync` の次回 rerun で `--disable-persistent-decoder --enable-continuous-stream-decoder` を維持し、追加 diagnostics の `continuous_decode_ffmpeg_args_summary`、stdin write count/bytes/error、process running/exit status、stdout read attempt/in-progress、stderr reader alive/bytes/last time、first-input timing を回収する
2. rerun 結果で stdin write が成功しているか、FFmpeg process が生存しているか、stdout reader が read_exact 中か、stderr が本当に空か、first input から output 0 のまま何 ms 経過しているかを読み、FFmpeg probing/buffering/stdout 側の問題か feed policy 側の問題かを切り分ける
3. selected frame exact lookup / one-shot fallback は残し、latest decoded fallback、targetTime-aware lookup、slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode はまだ進めない
