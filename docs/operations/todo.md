...
## 直近でやること
1. fallible display policy output を composition adapter / placeholder path へ渡す最小方針を計画する。source-error placeholder detail は no-frame / waiting に潰さない
2. その方針に基づき、fallible source-error placeholder を composition-facing skipped side detail として保持する最小 validation slice を検討する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest composition adapter / placeholder connection for `SwitcherTwoViewHandoffDisplayPolicyOutput`, preserving source-error placeholder detail without fake frames.
