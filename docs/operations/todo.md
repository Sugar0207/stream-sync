...
- 今回の rerun で分かった実際の blocker は transport parse/runtime failure ではなく manual receive stop condition で、`expected_reassembled_frames=2` が player1 単独の 2 frame で満たされ、server receive/auth phase が閉じたあとに client2 が `AuthResponse(Receive(ConnectionReset))` になっていた
...
## 直近でやること
1. `2`-real-slot manual procedure を見直し、server receive 完了条件が player1 単独 2 frame で満たされないようにする
2. `configs/manual/server.two-real-slots.toml`、`configs/manual/client.player1.toml`、`configs/manual/client.player2.toml` を使って、player1 と player2 の両方が auth / queue 参加した状態で `--four-view-two-real-handoff-preview-loop ...` を再実行する
3. 再実行時も `queue_len_before_read` / `queue_len_after_read` / `selected_client_id` / `selected_run_id` / `slot_diagnostics` を採取して、初めて本当の 2-client handoff path を記録する
4. player1 単独 isolation が必要な場合は引き続き `--four-view-real-handoff-preview-loop streamsync-handoff-dev 0 player1 streamsync-dev-session 5` を baseline として使う
5. production H.264 encoder configuration / error logging policy
6. Decide later whether `--live-two-view-switcher-once` should be renamed or deprecated after the transport-backed server-mediated path exists
