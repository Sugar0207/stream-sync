...
16. [ ] manually validate Program-first operator low-cost Preview with OBS still capturing `StreamSync Program Output`; compare `operator_low_cost_preview_enabled`, `operator_preview_refresh_interval_ticks`, refresh attempt/success/skipped/stale counts, `operator_preview_forced_one_shot_decode_count`, `operator_preview_render_effective_fps`, Program render FPS, Program continuous latest count, Program one-shot fallback count, black/placeholder counters, and whether the 4-view Preview is visible enough for source selection
17. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
18. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
19. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
20. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
21. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
