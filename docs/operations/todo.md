...
- secret store provider 連携、token hashing、rotation 実行、timeout evaluation / action plan / apply boundary を continuous loop から呼ぶ処理、再認証、実際の packet 破棄、時刻同期本体、映像受信・復号・表示、switcher UI は未実装
...
- 次の中心は timeout evaluation / action plan / apply boundary を continuous loop から呼ぶ方針、RTT / offset estimate の state commit 方針、continuous heartbeat loop に進む前の境界整理
...
## 直近でやること
1. timeout evaluation / action plan / apply boundary を continuous loop から呼ぶ方針を整理する
...
- [ ] timeout evaluation / action plan / apply boundary を continuous loop から呼ぶ
- [ ] RTT 計測を実装する
