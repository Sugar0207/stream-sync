...
## Next Items
1. slot0 per-client continuous feed/drain policy を docs-first で最小設計する。current render-demand selected-frame feed との差分、handoff/source からの連続 frame 取得方法、feed cadence、queue/backpressure/drop policy、diagnostics、first implementation slice を整理する
2. latest decoded fallback は `requested_frame_id=535` / `latest_decoded_frame_id=386` の stale risk があるため保留し、exact selected frame lookup と one-shot fallback を維持したまま feed/drain policy で lag を縮められるかを次候補にする
3. 次 step でも code へ進む前に Production Readiness FAIL を維持し、slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode、targetTime-aware decoded queue lookup 本実装、one-shot fallback 削除には広げない
