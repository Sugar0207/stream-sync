...
- next human rerun の main gate は `retained_keyframe_clients=2`、`per_client_retained_keyframe_frame_id` populated、client summary で `h264_idr_count > 0` かつ `keyframes_sent > 0`、switcher real slots で `decodable_source=queue|retained_keyframe` のいずれかになること。`retained_keyframe_clients=0` が再発する場合は encoder cadence や GOP policy ではなく、client summary / server receive summary / raw handoff diagnostics の counters で metadata propagation mismatch を再確認する
