...
## 現在位置
- same-PC `2`-client quad-view reuse rerun では client output FPS は約 `29.4fps`、server queue / handoff は正常、visual identity reuse と unchanged-frame decode skip も効いたが、switcher は `effective_attempt_fps=4.053` / `effective_render_fps_after_first_render=2.526` に留まり、残る最大ボトルネックは actual composition cost の `avg_quad_view_compose_elapsed_ms=245.549` だった。今回の code slice では `--four-view-two-real-handoff-preview-loop` に persistent quad composition buffer を追加し、visual identity が変わった tick でも canvas 全体の再確保・全体 placeholder fill・全 slot copy を避け、同一 canvas size では changed slot region だけを placeholder fill + source copy する incremental update path にした。さらに placeholder row を cache し、full compose 時は canvas 全体を一度だけ placeholder fill して slot ごとの重複 fill を避ける。source-error placeholder は visual identity change として扱い、該当 slot region を source-error placeholder に更新する。stable no-frame placeholder は persistent buffer allocation / full recomposition を強制しない。既存の whole composed-frame reuse、unchanged-frame decode reuse、source-error placeholder semantics、client encode/capture behavior 非変更、distributed-PC validation docs 非変更は維持。final summary には `quad_view_incremental_update_count` / `quad_view_full_compose_count` / `quad_view_changed_slot_update_count` / `quad_view_reused_slot_count` / `quad_view_allocation_count` / `avg_quad_view_incremental_update_elapsed_ms` を追加済み
...
## Next Items
1. same-PC `2`-client smoke を rerun し、switcher final summary の incremental quad composition / full compose / allocation / FPS / timing diagnostics を確認する
