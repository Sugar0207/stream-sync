...
- 次の中心は heartbeat timeout notice wakeup 実行本体に進む前の境界整理、RTT / offset metrics snapshot の具体的な export cadence / dashboard refresh 方針、continuous heartbeat loop 本体へ進む前の client ack receive / observation return 接続範囲整理
...
3. continuous heartbeat loop 本体へ進む前の client ack receive / observation return 接続範囲を整理する
