...
## 直近でやること
1. `configs/manual/client.player3.toml` と `configs/manual/client.player4.toml` を追加し、player1..player4 が same run id `streamsync-dev-session` で参加する `4`-client manual config set を揃える
2. `4`-client startup recipe を docs に固定する:
   - minimal proof: `expected_reassembled_frames=4` / `expected_reassembled_clients=4` / `expected_reassembled_frames_per_client=1`
   - preferred proof: `expected_reassembled_frames=8` / `expected_reassembled_clients=4` / `expected_reassembled_frames_per_client=2`
3. `4` real slots 全部を読む dedicated switcher preview/output command の最小 shape を決め、existing `1`-real-slot / `2`-real-slot commands は壊さずに追加する
4. `4`-real-slot manual expected logs を固定する:
   - server: `registered_clients=4` / `observed_reassembled_clients=4` / `per_client_reassembled_frames=...`
   - switcher: `slot_result_kinds=Selected|Selected|Selected|Selected` / `scheduler_status=AllSelected` / `clean_output_render_result_kind=Rendered`
5. player1 単独 isolation が必要な場合は引き続き `--four-view-real-handoff-preview-loop streamsync-handoff-dev 0 player1 streamsync-dev-session 5` を baseline として使う
6. production H.264 encoder configuration / error logging policy
7. Decide later whether `--live-two-view-switcher-once` should be renamed or deprecated after the transport-backed server-mediated path exists
