...
1. docs/operations/two-client-handoff-validation.md を使って same-PC 2-client handoff validation を人間が rerun する
   ...
   - switcher real slots では `frame_is_keyframe` / `decode_expected_width` / `decode_expected_height` / `decode_expected_rawvideo_len` / `decoded_stdout_len` / `payload_nal_kinds` を確認する
2. rerun の結果から、`preview-latest-decodable` で `decode_error=none` まで通るか、それとも IDR cadence / keyframe policy の narrow follow-up が必要かを確定する
