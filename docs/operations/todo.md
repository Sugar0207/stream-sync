...
## 現在位置
- same-PC `2`-client latest timing rerun では client output FPS は約 `30fps`、unchanged-frame decode skip も効いていたが、switcher は `effective_render_fps=1.794` まで落ち、最大ボトルネックは `quad_view_compose_elapsed_ms=26880` だった。今回の code slice では `--four-view-two-real-handoff-preview-loop` が handoff / scheduler / decode / display / composition-render 接続の後、BGRA quad composition 直前で slot visual identity を比較できるようにし、全 slot の visual identity が不変かつ前回 composition result がある tick は quad-view recomposition を skip して前回 composition result を再利用する。selected source frame identity は `client_id + run_id + frame_id + decodable_source`、stable `NoFrameAvailable` 系は no-display placeholder、`HandoffError` / source error は source-error placeholder として扱う。final summary には `quad_view_compose_attempt_count` / `quad_view_compose_success_count` / `quad_view_compose_skipped_unchanged_count` / `quad_view_composed_frame_reuse_count` / `quad_view_visual_unchanged_count` / `quad_view_visual_changed_count` / `avg_quad_view_compose_elapsed_ms` を追加済み。unchanged-frame decode reuse、source-error placeholder、client encode/capture behavior 非変更、distributed-PC validation docs 非変更は維持
...
## Next Items
1. same-PC `2`-client smoke を rerun し、switcher final summary の quad-view compose skip / composed-frame reuse / FPS / timing diagnostics を確認する
