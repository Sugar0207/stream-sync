...
## Next Items
1. one-shot FFmpeg decode path の内訳を切り分けて、`decode_process_spawn_elapsed_ms` / `decode_input_write_elapsed_ms` / `decode_output_read_exact_elapsed_ms` / `decoded_buffer_clone_count` を中心に見る
2. `decode_output_buffer_reuse_count=0` と `decode cache ownership` を前提に、persistent decoder はまだ実装せず候補として残す
