...
## Next Items
1. `docs/operations/continuous-feed-drain-plan.md` の first implementation slice を実装前に再確認し、slot0 / two-real preview loop / opt-in continuous decoder / diagnostics-first に限定した最小 code slice へ切る
2. oldest-driven bounded feed で `PreviewOldest -> ConsumeOldest` を使う場合の queue mutation risk を実装直前に確認し、render selection authority / one-shot fallback / exact lookup を崩さない
