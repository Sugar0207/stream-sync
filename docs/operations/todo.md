...
## 直近でやること
1. 今回成功した `2`-real-slot manual recipe を基準に、server manual stop condition を distinct client-aware に広げる必要が本当にあるかを判断する
2. もし stop condition を広げるなら、`expected_reassembled_clients` / `expected_reassembled_frames_per_client` / `stop_after_expected_distinct_clients` のような最小追加案を docs 上で比較し、manual-only knob で済むかを切り分ける
