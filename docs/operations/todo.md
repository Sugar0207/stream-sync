...
- `ClientStats` receive route / gate / registered handler bridge と、`HeartbeatAckObservation` を既存 timebase plan / stateless calculator へ渡す最小接続、RTT / offset latest estimate state commit は完了。継続送信 loop、metrics state commit、smoothing / outlier policy は未実装
...
- 次の中心は heartbeat timeout loop tick の notice queue storage / send wakeup 方針、RTT / offset smoothing / outlier policy の範囲整理、continuous heartbeat loop に進む前の境界整理
...
2. RTT / offset smoothing / outlier policy の範囲を整理する
