...
## 直近でやること
1. thin wrapper MVP を実装する:
   - same-binary wrapper command を `stream-sync-switcher` 側に追加するかたちで進める
   - first interaction は CLI/TUI keyboard loop とし、existing control pipe に command を送るだけに留める
   - wrapper は latest response line 表示と local guarded-quit text だけを持ち、switcher window lifecycle は持たない
2. wrapper MVP の manual validation を実施する:
   - actual validation 前に `cargo build` して stale `target/debug` binary を避ける
   - `1..4` / `0 or A` / `S` / guarded `Q` の manual plan を記録どおり確認する
...
3. same-session bounded server lifecycle の narrow polish が本当に必要か判断する:
   - rebuilt control-pipe rerun の `requests_served=140` / `handoff_errors=0` を踏まえて flush/exit improvement が still needed かを見直す
   - request-budget formula は docs 運用で十分かを確認する
