...
- 次の 4-view slice は scheduler から decode/render/composition へ進める planning とし、generic N-view 化は避けたまま dedicated 4-view adapter、per-slot decode/render connection、per-slot display policy、fixed `QuadView` 2x2 composition を最小形として進める
...
## 直近でやること
1. 4-view orchestration の最小 decode/render/composition slice として、dedicated 4-view adapter / per-slot display policy / fixed `QuadView` composition instruction shape を実装する
