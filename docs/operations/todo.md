...
## 直近でやること
1. 4-view orchestration の次 slice として、`SwitcherFourViewComposedFrame` を受ける dedicated quad render-facing adapter/connection を最小追加し、frame size / BGRA payload length / slot metadata / aggregate scheduler status / placeholder・source-error 情報 / `NoRenderableQuadView` / `InvalidQuadView` を explicit に forwarding する
