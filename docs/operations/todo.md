...
## 直近でやること
1. fallible queued-frame handoff result を switcher のどの境界で最初に消費するかを決める。targetTime selection は引き続き switcher 側に置く
2. `SwitcherQueuedFrameHandoff` を current in-process source で使う最小 validation slice を追加する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Decide and implement the smallest switcher-side consumer for `SwitcherQueuedFrameHandoff` results while keeping targetTime selection in switcher.
