...
## 直近でやること
1. fallible handoff path を server-mediated validation boundary へどう接続するかを検討する。targetTime selection は switcher 側、server は ingest/reassembly/queue owner のまま維持する
...
## Next Items
1. Plan the smallest fallible handoff path connection into the server-mediated validation boundary, preserving source-error detail and keeping targetTime selection in switcher.
2. production H.264 encoder configuration / error logging policy
3. Decide later whether `--live-two-view-switcher-once` should be renamed or deprecated after the server-mediated path exists
