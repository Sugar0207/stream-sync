...
## 直近でやること
1. fallible 2-view targetTime handoff scheduler result を decode/render adapter へ接続する最小方針を計画する。handoff error は fake no-frame / waiting / decoded frame に潰さない
2. その方針に基づき、decode/render adapter 側で handoff error を明示的に保持する最小 validation slice を実装する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest decode/render adapter path for `SwitcherTwoViewTargetTimeHandoffSourceSchedulerResult` without collapsing handoff errors into fake no-frame / waiting / decoded frames.
