...
- latest suppression ON rerun is `S:\stream-sync\manual-logs\two-client-render-rerun-20260522-082451`. It used `C:\streamsync-target\stream-sync-rerun\debug\*.exe`, kept low-latency args active, and makes the suppression runtime evaluation VALID
- suppression flag and diagnostics are PASS in the latest rerun: `continuous_decode_slot0_one_shot_suppression_enabled=true`, `continuous_decode_slot0_one_shot_suppressed_count=216`, suppression reasons `continuous_not_ready:51|stale:165|future:0|unknown:0`, and render safety counts `decode_deferred_placeholder:216|unknown:0`
- continuous feed remains PASS in the latest rerun: `continuous_feed_frame_received_count=347`, `continuous_feed_enqueued_count=345`, `continuous_decode_input_from_feeder_count=345`, `continuous_decode_input_from_render_demand_count=2`, and `continuous_decode_feeder_lag_to_selected=0`
- continuous output remains PASS: `continuous_decode_input_frame_count=347`, `continuous_decode_output_frame_count=304`, `continuous_decode_output_throughput_fps=22.327`, `continuous_decode_output_bytes_per_sec=20576263.220`, reader full-frame avg `44.220ms`, reader full-frame max `2233ms`, output frame interval avg `36.997ms`, and output frame interval max `335ms`
- continuous render consumption is now PARTIAL PASS on suppression ON evidence: `render_used_continuous_decoded_count=3`, `continuous_decode_bounded_lookup_hit_count=3`, `continuous_decode_render_used_bounded_lag_count=3`, while guarded rejects remain visible at stale `165` and continuous-not-ready `51`
- suppression ON reduced visible competing one-shot work to `continuous_decode_competing_one_shot_attempt_count=12` / `continuous_decode_competing_one_shot_decode_elapsed_ms=1414`; prior valid OFF baseline `20260522-075029` showed `34` / `3515ms`
- throughput causality remains INCONCLUSIVE until matched A/B evidence exists: latest ON client output fps was `22.340` / `22.453`, while the prior valid OFF baseline was `28.358` / `28.501`, so the `22.327fps` ON continuous output throughput must not be read as a clean suppression speedup
- continuous output throughput analysis remains tracked in `docs/operations/continuous-output-throughput-plan.md`; suppression boundary and matched A/B rerun policy remain tracked in `docs/operations/continuous-one-shot-double-load-plan.md`
...
## Next Items
1. 人間側 `S:\stream-sync` で同一 build / 同一 `C:\streamsync-target\stream-sync-rerun\debug\*.exe` を使い、suppression OFF と ON の matched A/B rerun を行う
2. OFF は `--disable-persistent-decoder --enable-continuous-stream-decoder --continuous-decoder-low-latency-args`、ON はそこへ `--continuous-decoder-slot0-suppress-one-shot-fallback` を足し、client `effective_output_fps` 差が概ね `2fps` を超える比較は noisy と記録する
3. matched A/B では continuous output throughput、output lag、latest input-output lag、competing one-shot attempts/elapsed、continuous render count、bounded lookup hit、render FPS を比較する
4. pixel format / scale path / stdout reader buffering experiment は matched A/B evidence 後に再評価し、threshold tuning は output lag が guarded range に近づくまで進めない
5. Production Readiness FAIL を維持し、allowed lag threshold 変更、targetTime-aware lookup 実装、latest decoded fallback、slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode、one-shot fallback 削除、feed max count 変更には広げない
