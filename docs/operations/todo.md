...
- heartbeat timeout は `ServerHeartbeatTimeoutPolicy` と `evaluate_timeout` で 1 client 分を `Alive` / `TimedOut` / `NoHeartbeat` に分類できる。`TimedOut` 結果から auth registry invalidation command、timeout log event input、`AuthExpired` notice plan を作る最小 action boundary も追加済み
- `ClientStats` payload encode/decode と heartbeat observation optional block の最小 wire 変換は完了
- secret store provider 連携、token hashing、rotation 実行、timeout action plan を continuous loop から実適用する処理、再認証、実際の packet 破棄、時刻同期本体、映像受信・復号・表示、switcher UI は未実装
...
- 次の中心は timeout action plan の continuous loop 適用方針、RTT / offset estimate の state commit 方針、continuous heartbeat loop に進む前の境界整理
...
## 直近でやること
1. timeout action plan を continuous loop から実適用する方針を整理する
...
- [ ] timeout action plan を continuous loop から実適用する
- [ ] RTT 計測を実装する
