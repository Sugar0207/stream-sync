...
## 直近でやること
1. 4-view orchestration の次 slice として、handoff source / decode runtime / window render hook を caller-owned input に取る dedicated 4-view orchestration/validation boundary を最小追加し、scheduler / adapter / display policy / QuadView composition instruction / BGRA composition / render-facing / window render の各 stage output を explicit に visible のまま返す
