...
- 次の中心は heartbeat timeout notice wakeup 実行本体に進む前の境界整理、RTT / offset metrics snapshot の具体的な export cadence / dashboard refresh 方針、client one-tick runtime から future repeated loop body を呼ぶ最小範囲整理
...
3. client one-tick runtime から future repeated loop body を呼ぶ最小範囲を整理する
