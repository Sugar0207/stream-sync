...
- client bounded real encoded sender の human-validation summary も整理した。`frames_attempted` は loop tick と capture/send attempt を混同しやすかったため human-facing 出力から外し、`configured_max_frames` / `configured_max_ticks` / `runtime_ticks` / `capture_attempts` / `frames_remaining_to_max` / `elapsed_ms` / `effective_capture_fps` / `effective_send_fps` / `total_fragment_pacing_sleep_ms` / `send_elapsed_ms` / `ticks_elapsed_while_sending` を追加した。current loop は synchronous なので `ticks_elapsed_while_sending=0` が期待値であり、`MaxTicksReached` 時は `frames_remaining_to_max > 0` で max-frames 未達が即読できる
