...
- next code candidate should move toward slot0 / two-real / opt-in continuous output pipeline experiment planning, now tracked in `docs/operations/continuous-output-pipeline-experiment-plan.md`: completed correspondence latency diagnostics first, then stdout/raw BGRA pipe throughput, FFmpeg scale path split, and reader blocking phase diagnostics. Threshold branch remains HOLD / candidate, one-shot suppression remains strong contributor evidence, and Production Readiness remains FAIL
...
2. 次 code を切るなら behavior-changing default ではなく、slot0 / two-real / opt-in continuous 限定の completed correspondence latency diagnostics を first candidate にし、その後 stdout/raw BGRA pipe throughput experiment、FFmpeg scale path split experiment、reader blocking phase diagnostics の順で比較する
...
4. compare completed correspondence latency diagnostics, stdout/raw BGRA pipe throughput, FFmpeg scale path split, and reader blocking phase diagnostics next; do not adopt source-size raw output by default
