...
- 次の中心は RTT / offset estimate の state commit 方針、heartbeat timeout loop tick の notice queue storage / send wakeup 方針、continuous heartbeat loop に進む前の境界整理
...
## 直近でやること
1. RTT / offset estimate を server 側 state に commit する最小境界を整理する
2. heartbeat timeout loop tick の notice queue storage / send wakeup 方針を整理する
