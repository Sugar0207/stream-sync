...
## 直近でやること
1. human rerun は `S:\stream-sync` を repo root にして、two-real preview loop の既存 command 末尾に `--disable-persistent-decoder --enable-continuous-stream-decoder` を付け、first configured real source の continuous opt-in path を比較する。first gate は summary の `continuous_decode_config_enabled=true`
...
- latest bounded-lag lookup rerun is `S:\stream-sync\manual-logs\two-client-render-rerun-20260520-005310`. Continuous opt-in, low-latency args, bounded feed helper, and bounded-lag lookup diagnostic wiring are PASS: `continuous_decode_bounded_lookup_enabled=true` and `continuous_decode_bounded_lookup_allowed_lag_frames=5`
- bounded-lag render consumption remains FAIL in that rerun: `continuous_decode_bounded_lookup_hit_count=0`, `continuous_decode_bounded_lookup_rejected_stale_count=17`, `continuous_decode_bounded_lookup_rejected_not_ready_count=2`, `continuous_decode_bounded_lookup_fallback_to_one_shot_count=19`, `continuous_decode_render_used_exact_count=0`, `continuous_decode_render_used_bounded_lag_count=0`, and `render_used_continuous_decoded_count=0`
- the current blocker is not just a too-small `5` frame threshold. Requested/latest decoded lag reached `88` frames, max observed lag reached `163`, `continuous_decode_output_pending_correspondence_count=79`, `continuous_decode_stdout_read_elapsed_ms=20840`, and `continuous_decode_stdout_reader_blocked_count=17`. Simple threshold expansion is held because it would risk stale frame display
- continuous decoder produced output but still lagged badly: `continuous_decode_input_frame_count=378`, `continuous_decode_output_frame_count=297`, `continuous_decode_queue_len=30`, and `continuous_decode_dropped_stale_count=267`. Next priority is output lag / pending correspondence / stdout read latency / decoded queue-drop policy analysis, not slot1 or 4-client rollout
- continuous output lag docs-first analysis is now split into `docs/operations/continuous-output-lag-plan.md`. The code path is: enqueue -> writer input queue -> writer pushes correspondence metadata -> FFmpeg stdin write -> reader waits for one full raw BGRA frame -> reader pops correspondence -> render loop drains decoded event into the bounded decoded cache. `output_pending_correspondence=79` means writer-side accepted inputs are still waiting for matching full stdout frames
- 2026-05-20 output lag diagnostics-only slice is implemented for slot0 / two-real / opt-in continuous. Summary now exposes `continuous_decode_latest_input_minus_latest_output_lag`, pending correspondence frame_id min/max, `continuous_decode_input_to_output_lag_frames_max`, `continuous_decode_output_lag_to_selected_frames`, `continuous_decode_output_throughput_fps`, `continuous_decode_reader_full_frame_elapsed_ms_max`, and `continuous_decode_queue_drop_reason_counts`
- this diagnostics slice does not change continuous decoder behavior, lookup order, bounded-lag threshold, feed max count, FFmpeg defaults, one-shot fallback, slot1, or 4-client rollout. Production Readiness remains FAIL until a human rerun proves continuous render consumption
...
## Next Items
1. 人間側で `S:\stream-sync` から output lag diagnostics rerun を行い、summary の pending correspondence frame_id range、latest input-output lag、input-to-output max lag、output throughput、reader full-frame max、queue drop reason counts を確認する
2. rerun suffix は `--disable-persistent-decoder --enable-continuous-stream-decoder --continuous-decoder-low-latency-args` を維持する
3. rerun evidence を見て、次を threshold tuning ではなく output throughput / stdout read / correspondence backlog / decoded queue-drop policy のどこへ絞るか決める
4. 次 step でも Production Readiness FAIL を維持し、allowed lag threshold 変更、targetTime-aware lookup 実装、latest decoded fallback、slot1 continuous 化、4-client 化、request/response persistent decoder 復活、GPU decode、one-shot fallback 削除、feed max count 変更には広げない
