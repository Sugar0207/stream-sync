...
- continuous receive loop 本体の実装範囲と 1 tick 実接続範囲は整理済み。loop lifecycle / tick placeholder は追加済みだが、実 loop、handler dispatch、packet drop 本体、ログ writer 実接続は未実装
...
- 次の中心は file sink 実 file open 範囲の再確認、ServerNotice trigger の state transition 接続範囲の再確認、secret store provider / token rotation 実行範囲の再確認、continuous receive loop から operational / rejection writer への実接続範囲整理
...
4. continuous receive loop から operational / rejection writer への実接続範囲を必要になった時点で整理する
