...
## 直近でやること
1. `AllView -> Focused(slot_index) -> AllView` の operator flow を同一 session か近接 session で再確認する:
   - current actual validation は `Focused(0..3)` と `AllView` を個別 session で記録済み
   - next pass では mode transition 自体の operator flow を確認する
