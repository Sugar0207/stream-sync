...
## Next Items
1. 人間側 `S:\stream-sync` の次回 rerun で `--disable-persistent-decoder --enable-continuous-stream-decoder` を維持し、追加 diagnostics の `continuous_decode_input_frame_id_gap_*`、NAL count、`continuous_decode_ffmpeg_stderr_summary`、`continuous_decode_stdout_reader_blocked_count`、`continuous_decode_no_output_after_input_count` を回収する
2. rerun 結果で sparse input / missing reference stderr / keyframe後 no-output / stdout reader pending のどれに寄るかを読み、slot0 per-client continuous feed/drain policy に進むべきか判断する
