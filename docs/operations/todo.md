...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260518-080637` では、`--disable-persistent-decoder` による pure one-shot-only rerun 上で two-real preview loop 限定 scaled decode output は runtime PASS 継続だった。`one_shot_decode_output_width=640`、`one_shot_decode_output_height=360`、`one_shot_decode_output_pixel_format=Bgra8`、`one_shot_decode_scaled_output_enabled=true`、`one_shot_decode_scaled_output_reason=two_real_slot_size`、`one_shot_decode_expected_output_bytes_per_frame=921600` を維持しており、latest rerun でも scaled decode failure shape は出ていない
...
- server / client / transport はこの rerun でも成立している。server は `packets_received=37374`、`frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0` で、client1/client2 も `frames_sent=900`、`effective_output_fps=29.157|28.887` を維持した
- latest rerun の switcher は `effective_render_fps_after_first_render=13.760`、`effective_render_fps=10.435` で、previous scaled-pass rerun `manual-logs/two-client-render-rerun-20260517-223121` の `16.579` には届かないが、noisy rerun `manual-logs/two-client-render-rerun-20260518-022141` の `9.469` からは改善した。したがって latest run は scaled decode PASS を保った中間的な comparison evidence として扱う
- latest rerun の decode attempt frequency は大きな異常ではない。`decode_attempt_count=28`、`decode_success_count=28`、`one_shot_decode_attempt_count=28` は previous scaled-pass rerun の `26` に近く、attempt slot も `one_shot_decode_attempt_slot_counts=slot0:14|slot1:14` で均等、attempt reason も `one_shot_decode_attempt_reason_counts=frame_id_changed:26|cache_miss:0|previous_unavailable:0|source_recovered:2|unknown:0` とほぼ `frame_id_changed` に寄っていた
- latest attempt/source diagnostics でも、actual `decode_attempt_count` は two-real preview loop の decode cache miss を見ているだけだと整理できる。`decode_cache_miss_slot0_count=14`、`decode_cache_miss_slot1_count=14` は偏りがなく、`cache_miss` / `previous_unavailable` reason が `0` だったため、今回の主犯を attempt frequency そのものに置かない
- latest rerun で次に重かったのは per-attempt variance 側だった。`one_shot_decode_elapsed_ms=3782`、`one_shot_decode_input_write_elapsed_ms=1648`、`one_shot_decode_input_write_elapsed_ms_max=401`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms=1474`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms_max=186`、`one_shot_decode_stdout_first_byte_elapsed_ms=1472`、`one_shot_decode_first_byte_elapsed_ms_max=186`、`one_shot_decode_first_byte_slow_count=7`、`one_shot_decode_output_read_elapsed_ms=1775`、`one_shot_decode_output_read_elapsed_ms_max=221`、`one_shot_decode_output_read_slow_count=7` なので、next candidate は `first-byte wait variance` / `output-read slow variance` / `input-write outlier` に更新する
- latest rerun の payload shape は `one_shot_decode_input_payload_bytes_min=49036`、`max=104427`、`avg=83475.286` で、previous scaled-pass rerun `avg=195142.192` より小さい。したがって latest の first-byte/read/input-write 悪化は payload size 増加だけでは説明しにくい
- incremental quad compose は runtime PASS 継続で、`quad_view_incremental_update_count=46`、`quad_view_full_compose_count=1` を維持した。ただし `quad_view_compose_elapsed_ms=969`、`gdi_paint_wait_elapsed_ms=80`、`placeholder_visual_changed_count=44`、`scheduler_status=PartialSelected` なので、compose / GDI variance も注意点として残す
...
## 直近でやること
1. next step では `S:\stream-sync\manual-logs/two-client-render-rerun-20260518-080637` を latest comparison run、`S:\stream-sync\manual-logs/two-client-render-rerun-20260517-223121` を previous scaled-pass run として比較し、`decode_attempt_count=28` が主犯ではない前提で `one_shot_decode_first_byte_slow_count` / `one_shot_decode_output_read_slow_count` / `one_shot_decode_input_write_elapsed_ms_max` を優先して読む
2. rerun 比較では `one_shot_decode_attempt_slot_counts` / `one_shot_decode_attempt_reason_counts` / `decode_cache_miss_slot0_count` / `decode_cache_miss_slot1_count` を使い、slot 偏りがないことと `frame_id_changed` 主体であることを再確認しつつ、attempt frequency と per-attempt variance を切り分ける
3. diagnostics-only follow-up を足すなら、`one_shot_decode_slow_first_byte_*` / `one_shot_decode_slow_output_read_*` / `one_shot_decode_input_write_outlier_*` の相関観測に限定し、FFmpeg args・pixel format・one-shot decoder routing は変えない
...
## Next Items
1. next step では `S:\stream-sync\manual-logs\two-client-render-rerun-20260518-080637` を latest comparison run、`S:\stream-sync\manual-logs\two-client-render-rerun-20260517-223121` を previous scaled-pass run として比較し、`one_shot_decode_attempt_slot_counts`、`one_shot_decode_attempt_reason_counts`、`decode_cache_miss_slot0_count`、`decode_cache_miss_slot1_count`、`one_shot_decode_first_byte_slow_count`、`one_shot_decode_output_read_slow_count`、`one_shot_decode_input_write_elapsed_ms_max` の相関を narrow に切り分ける
2. `S:\stream-sync` の next rerun では `one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms_max`、`one_shot_decode_stdout_first_byte_elapsed_ms`、`one_shot_decode_first_byte_elapsed_ms_max`、`one_shot_decode_output_read_elapsed_ms`、`one_shot_decode_output_read_elapsed_ms_max`、`one_shot_decode_input_write_elapsed_ms_max` を前回比で取り、attempt frequency ではなく per-attempt variance が主因かを確認する
3. request/response persistent decoder は凍結候補のまま維持し、continuous-stream decoder rewrite、one-shot decoder I/O の大改造、raw BGRA 以外の intermediate pixel format 比較は次候補としてだけ残す。code を触る場合も slow attempt correlation diagnostics-only に限定する
