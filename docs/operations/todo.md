...
## 現在位置
- 2026-06-03 latest operator low-cost Preview validation with `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30` made the non-Program Preview slot partially useful: client1 / slot0 became visible through low-frequency one-shot decode. ProgramOutput remained selected-only and good (`program_render_effective_fps=19.884`, `program_render_used_continuous_latest_count=2846`, `program_render_used_one_shot_fallback_count=0`, black / placeholder `0`), but the explicit Program source player2 / slot1 was still black / decode-deferred in 4-view Preview even though ProgramOutput rendered it from continuous latest. Current code now adds an opt-in Preview-only reuse path for the Program-selected slot under Program-first low-cost Preview mode, so the next validation should repeat the same refresh knobs and watch `operator_preview_reused_program_frame_count`, `operator_preview_program_slot_visible_count`, `operator_preview_program_slot_reuse_source`, and `operator_preview_program_slot_black_count`
...
20. [ ] rerun Program-first operator Preview validation with `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30`; compare Program slot reuse/visible/black diagnostics, non-Program visible count, decode refresh source counts, Program render FPS, Program one-shot fallback count, and whether 4-view Preview is useful for source selection
21. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
22. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
23. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
24. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
25. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
