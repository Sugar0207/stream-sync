...
## 直近でやること
1. 4-view orchestration の次 slice として、`SwitcherFourViewComposedCanvasWindowRenderBoundary` の上で bounded one-shot preview/manual proof か thin top-level orchestration boundary を最小追加し、既存 4-view render/window result family を崩さず future OBS consumer と分離したまま扱う
