...
## 直近でやること
1. server queue read boundary を switcher / sync 側の targetTime selection source へ接続する最小 integration を検討・実装する
...
## Next Items
1. Connect the server queue read boundary to the next switcher/sync source path without adding late-drop mutation yet.
2. Decide whether targetTime selection should inspect or dequeue frames in the first manual integration.
