...
## 直近でやること
1. `configs/manual/server.two-real-slots.toml`、`configs/manual/client.player1.toml`、`configs/manual/client.player2.toml` を使って `--four-view-two-real-handoff-preview-loop ...` を diagnostic logging 付きで rerun し、server/client/switcher の stdout を採取する
2. player2 未参加や Auth timeout が続く場合は `--four-view-real-handoff-preview-loop streamsync-handoff-dev 0 player1 streamsync-dev-session 5` で player1 単独の handoff `FrameRead -> switcher render` baseline を先に再確認する
3. 新しい `queue_len_before_read` / `queue_len_after_read` と `slot_diagnostics` を見て、switcher 側 `HandoffError` が transport/runtime/response parse 由来なのか、decode/render placeholder 由来なのかを切り分けて記録する
