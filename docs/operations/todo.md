...
## 直近でやること
1. 4-view orchestration の次 slice として、`SwitcherFourViewHandoffValidationBoundary` を呼ぶ bounded one-shot manual preview/proof wrapper を最小追加し、target timestamp / scheduler status / per-slot kind / BGRA composition result / render-facing result / window render result / placeholder・source-error count を stdout へ要約する deterministic な in-process fixture path を作る
