...
- これで lifecycle 修正と fixed OBS-friendly profile 実装は揃ったため、次の焦点は manual rerun で `StreamSync 4-view Output` が OBS Window Capture から選択できるか、黒画面が解消するかの再確認に移る。free-form な size arguments や render-surface/window-style 調整は、この `1280x720` rerun でも capture が失敗した場合の次段とする
...
## 直近でやること
1. fixed `1280x720` OBS validation profile 版 `--four-view-clean-output-window-loop all-renderable <frames>` の manual actual runtime pass を再記録し、`source_width=4` / `source_height=2` / `output_width=1280` / `output_height=720` / `scale_mode=nearest-neighbor` を含む stdout を確認する
2. 上記 rerun のあと、OBS Window Capture で `StreamSync 4-view Output` を stable capture target として選択・preview できるかを再手動確認する
3. それでも OBS capture が失敗した場合だけ、render-surface/window-style 調整の最小 slice を planning する
