...
- latest diagnostics-only implementation では、one-shot decode 挙動を変えずに slow/outlier correlation を summary だけで読めるようにした。new fields は `one_shot_decode_slow_first_byte_slot_counts`、`one_shot_decode_slow_first_byte_source_counts`、`one_shot_decode_slow_first_byte_reason_counts`、`one_shot_decode_slow_output_read_slot_counts`、`one_shot_decode_slow_output_read_source_counts`、`one_shot_decode_slow_output_read_reason_counts`、`one_shot_decode_input_write_outlier_count`、`one_shot_decode_input_write_outlier_threshold_ms`、`one_shot_decode_input_write_outlier_slot_counts`、`one_shot_decode_input_write_outlier_source_counts`、`one_shot_decode_input_write_outlier_reason_counts`、`one_shot_decode_input_write_outlier_payload_bytes_total`、`one_shot_decode_input_write_outlier_payload_bytes_avg` で、slow first-byte / slow output-read / input-write outlier が slot / source / reason / payload size のどれに寄るかを next rerun で見られる
- current code-path では、actual decode miss ごとの per-attempt observation を current decode cache key と同じ source identity から後結合しているため、one-shot decode routing / FFmpeg args / cache key / selection / decode decision の挙動は変えていない
...
3. next rerun では `one_shot_decode_slow_first_byte_*` / `one_shot_decode_slow_output_read_*` / `one_shot_decode_input_write_outlier_*` を優先して見て、slow first-byte / slow output-read / input-write outlier が slot / source / reason / payload size のどれに偏るかを summary だけで判断する
...
## Next Items
1. next step では `S:\stream-sync\manual-logs\two-client-render-rerun-20260518-080637` を latest comparison run、`S:\stream-sync\manual-logs\two-client-render-rerun-20260517-223121` を previous scaled-pass run として比較し、`one_shot_decode_slow_first_byte_slot_counts`、`one_shot_decode_slow_first_byte_source_counts`、`one_shot_decode_slow_first_byte_reason_counts`、`one_shot_decode_slow_output_read_slot_counts`、`one_shot_decode_slow_output_read_source_counts`、`one_shot_decode_slow_output_read_reason_counts` の相関を narrow に切り分ける
2. `S:\stream-sync` の next rerun では `one_shot_decode_input_write_outlier_count`、`one_shot_decode_input_write_outlier_slot_counts`、`one_shot_decode_input_write_outlier_source_counts`、`one_shot_decode_input_write_outlier_reason_counts`、`one_shot_decode_input_write_outlier_payload_bytes_total`、`one_shot_decode_input_write_outlier_payload_bytes_avg` を前回比で取り、input write outlier が payload size と相関しそうかを summary だけで確認する
