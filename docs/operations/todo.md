...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-125737` では、persistent decoder request/response path は runtime で成功しなかった。`persistent_decode_enabled=true`、`persistent_decode_attempt_count=20`、`persistent_decode_success_count=0`、`persistent_decode_failure_count=20`、`persistent_decode_fallback_count=20`、`persistent_decode_process_spawn_count=21`、`persistent_decode_process_restart_count=20`、`persistent_decode_last_error=persistent_decode_stdout_read_timeout`、`one_shot_decode_fallback_count=20` で、全 attempt が timeout 後 one-shot fallback に落ちていた
- latest rerun の primary regression は timeout wait だった。one-shot fallback 自体は成功しているが、persistent timeout を待ってから fallback していたため `effective_render_fps_after_first_render=4.411`、`effective_render_fps=4.060`、`decode_elapsed_ms=42170`、`avg_decode_elapsed_ms=2108.500` まで悪化した。一方で one-shot 側 `decode_output_read_elapsed_ms=755`、`decode_output_read_exact_elapsed_ms=658` は支配的ではなく、server/client/transport も今回の主犯ではない。`persistent_decode_stdout_read_elapsed_ms=0` / `persistent_decode_output_bytes_total=0` なので、current request/response path は raw BGRA frame boundary を成立させられていない疑いが強い
- current persistent decoder slice には fail-fast / circuit breaker を追加した。`persistent_decode_stdout_read_timeout` を 1 回観測した時点で runtime-disabled にし、session を止めたうえで以後の decode call は即 one-shot fallback に流す。timeout ごとの restart storm は止め、`persistent_decode_runtime_disabled`、`persistent_decode_runtime_disabled_reason`、`persistent_decode_consecutive_failure_count`、`persistent_decode_disabled_after_failure_count`、`persistent_decode_skipped_after_disabled_count`、`persistent_decode_timeout_count`、`persistent_decode_timeout_elapsed_ms` を summary line に追加した
- current persistent request/response 実装は runtime FAIL の failed experiment として扱う。continuous-stream decoder rewrite は別 step の候補として保留し、この step では restart storm を止めて one-shot baseline 側へ早く戻すことを優先する
- persistent decoder 最小 slice の build validation は fail-fast 追加後も維持した。`cargo fmt`、`cargo check -p stream-sync-switcher`、focused test `switcher_four_view_two_real_handoff_preview_summary_formats_expected_fields`、`git diff --check` は PASS した
- decode cache / render reuse は今回大きく変えていない。`decoded_buffer_clone_count`、`decode_cache_store_clone_count`、`decode_output_buffer_reuse_count=0` の current ownership model は維持し、今回の目的は one-shot process spawn / pipe lifetime の切り離しに留めた
- current request/response persistent decoder 仮定は runtime FAIL として記録し、continuous-stream decoder rewrite は別 step の候補として保留する。今回の step ではそこへ進まず、direct compose / zero-fill removal を維持したまま regression stop を優先した。Production Readiness は FAIL 継続
...
## Next Items
1. same-PC `2`-client rerun を次 step で回し、`persistent_decode_runtime_disabled`、`persistent_decode_runtime_disabled_reason`、`persistent_decode_skipped_after_disabled_count`、`persistent_decode_timeout_count`、`persistent_decode_timeout_elapsed_ms`、`one_shot_decode_fallback_count` を回収する
2. rerun で timeout 1 回後に restart storm が止まり、`persistent_decode_attempt_count` / `persistent_decode_process_restart_count` が decode ごとに増え続けず、`effective_render_fps_after_first_render` が one-shot baseline 側へ戻るかを確認する
3. direct compose と zero-fill removal は維持しつつ、continuous-stream decoder rewrite や `4`-client all-real / distributed-PC actual run にはまだ広げない
