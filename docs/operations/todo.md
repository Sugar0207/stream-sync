...
## 直近でやること
1. fallible decode/render connection output を display policy / placeholder path へ渡す最小方針を計画する。source-error skip は no-frame / waiting に潰さない
2. その方針に基づき、fallible source-error skip を display policy-facing placeholder detail として保持する最小 validation slice を検討する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest display-policy / placeholder connection for `SwitcherTwoViewHandoffSchedulerDecodeRenderConnectionOutput`, preserving source-error skip detail without fake frames.
