...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-121040` では、direct compose と zero-fill 除去は維持できている。`render_buffer_half_scale_count=0`、`render_buffer_same_size_copy_count=37`、`render_buffer_scale_loop_elapsed_ms=0`、`decode_output_vec_resize_elapsed_ms=0` を維持しつつ、`avg_render_elapsed_ms=2.017`、`render_buffer_cpu_scale_copy_elapsed_ms=6` で render materialization / GDI は主犯ではない
- latest rerun の decoder-side dominant cost は one-shot FFmpeg decode output production/read に寄っている。`decode_elapsed_ms=4364`、`avg_decode_elapsed_ms=117.946` のうち、`decode_output_read_elapsed_ms=3430`、`decode_output_read_exact_elapsed_ms=2771`、`decode_stdout_expected_bytes_total=136396800` が支配的で、direct compose baseline `manual-logs/two-client-render-rerun-20260517-004552` の `effective_render_fps_after_first_render=12.920`、`decode_elapsed_ms=2180` にはまだ戻っていない
- server/client/handoff は引き続き clean で、`server frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0`、`client1/client2 frames_sent=900`、`send_failures=0`、`encode_failures=0` を維持した。今回の rerun では `decode_process_spawn_elapsed_ms=308`、`decode_input_write_elapsed_ms=427`、`avg_decode_input_write_elapsed_ms=11.541`、`decoded_buffer_clone_elapsed_ms=27`、`decode_cache_hit_clone_count=0`、`decode_cache_store_clone_count=37` で、input write と decoded buffer clone は現時点では主犯ではなさそうだと整理する
- current one-shot FFmpeg decode path の code-path review で確認した計測境界は維持されている。`decode_process_spawn_elapsed_ms` は `spawn_start` から `Command::spawn()` 成否まで、`decode_input_write_elapsed_ms` は `stdin.write_all(&input.encoded_payload)` のみ、`decode_output_read_exact_elapsed_ms` は `stdout.take(expected_len).read_to_end(...)` のみを計測しており、`stdin` close/drop、extra stdout probe、`child.wait()`、stderr read/join は別計測または未計測のままなので、current dominant cost は FFmpeg 側 decode / stdout production / OS scheduling block を含む output read path とみなす
- persistent decoder は distant candidate ではなく next design candidate に引き上げる。ただし今回の step では実装せず、`docs/operations/persistent-decoder-plan.md` を source of truth にして、one-shot FFmpeg decode から何を置き換えるか、stdin/stdout 継続利用、access unit 境界、SPS/PPS と keyframe handling、decode cache / render reuse との関係、failure fallback / process restart、最小 diagnostics、最小 implementation slice を docs-first で整理した
...
## Next Items
1. `docs/operations/persistent-decoder-plan.md` を元に、persistent decoder の最小 implementation slice を one-shot FFmpeg decode path へどう差し込むかを narrow に決める
2. next same-PC `2`-client rerun では persistent decoder 実装後に `decode_output_read_exact_elapsed_ms` / `decode_output_read_elapsed_ms` / `decode_process_spawn_elapsed_ms` がどこまで落ちるかを主比較にする
3. direct compose と zero-fill removal は維持しつつ、decoder-side baseline が戻るまで `4`-client all-real や distributed-PC actual run へは広げない
