...
## 現在位置
- 2026-06-03 operator low-cost Preview validation with `--program-first-preview-refresh-interval 30` made `StreamSync 4-view Output` visible, but it was not useful for monitoring: client1 ended black / `DecodeDeferred:ContinuousOneShotSuppressed`, Preview refresh was too sparse (`operator_preview_refresh_success_count=4`, `operator_preview_render_effective_fps=0.031`), and ProgramOutput still used continuous latest with no one-shot fallback (`program_render_used_continuous_latest_count=2819`, `program_render_used_one_shot_fallback_count=0`). The next narrow candidate is opt-in `--program-first-preview-decode-refresh-interval <ticks>`, which allows at most one non-Program Preview one-shot decode on matching Preview refresh ticks while keeping ProgramOutput selected-only and prioritized
...
18. [ ] manually validate Program-first operator Preview with both refresh knobs, e.g. `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30`; compare decode refresh attempt/success/skipped/source/budget diagnostics, non-Program visible count, Program render FPS, Program one-shot fallback count, black/placeholder counters, and whether 4-view Preview is useful for source selection
19. [ ] design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
20. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
21. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
22. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
23. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
