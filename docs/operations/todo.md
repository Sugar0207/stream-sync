...
## 直近でやること
1. 次は `S:\stream-sync\manual-logs\two-client-output-pipeline-ab-rerun-20260528-014200` を latest evidence として、scaled-bgr24 adoption ではなく BGR24 conversion optimization / direct render path の docs-first検討へ進める。valid runtime は引き続き `C:\streamsync-target\stream-sync-rerun\debug\*.exe`
2. raw pipe bytes hypothesis は PARTIAL PASS としつつ、default-bgra を継続する。次点で FFmpeg scale path split opt-in experiment、reader blocking phase diagnostics の順に比較する
