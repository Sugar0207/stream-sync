...
## Next Items
1. 次の最小 code slice は、default continuous FFmpeg args を変えず、opt-in experimental args toggle または diagnostics-only で `continuous_decode_ffmpeg_loglevel`、low-latency/probe args enabled flag、stdout partial bytes / first byte visibility を追加する
2. 人間側 `S:\stream-sync` の次回 rerun では `--disable-persistent-decoder --enable-continuous-stream-decoder` を維持し、experimental args toggle を実装した場合だけその flag も追加して、stdin write success / process alive / stderr none / stdout read in progress / output 0 が変わるかを見る
