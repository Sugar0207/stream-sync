...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-194136` では、`--disable-persistent-decoder` による pure one-shot-only baseline を維持したまま one-shot decode I/O diagnostics を回収できた。`persistent_decode_config_enabled=false`、`persistent_decode_attempt_count=0`、`persistent_decode_timeout_count=0`、`persistent_decode_skipped_by_config_count=26` で、persistent decoder config-disabled toggle は引き続き PASS だった
- server / client / transport はこの rerun でも成立している。server は `packets_received=36466`、`frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`keyframes_queued=60`、`io_error_count=0` で、client1/client2 も `frames_sent=900`、`encode_failures=0`、`send_failures=0`、`effective_output_fps roughly 29.6-29.8` を維持した
- latest rerun の switcher は `effective_render_fps_after_first_render=13.201`、`effective_render_fps=10.521`、`render_elapsed_ms=131`、`avg_render_elapsed_ms=1.016` まで改善したが、まだ 30fps 目標には遠い。persistent decoder を完全に避けた状態でも FPS 未達なので、原因を persistent decoder 単独とは扱わない
- incremental quad compose は runtime PASS 継続と整理する。`quad_view_compose_elapsed_ms=578`、`quad_view_incremental_update_count=27`、`quad_view_full_compose_count=1`、`avg_quad_view_compose_elapsed_ms=15.211`、`gdi_paint_wait_elapsed_ms=8` で、compose/render/GDI wait は current rerun の first-order culprit ではない
- latest rerun の decoder-side では one-shot decode I/O diagnostics を回収できた。`decode_attempt_count=26`、`decode_success_count=26`、`one_shot_decode_attempt_count=26`、`one_shot_decode_elapsed_ms=2287`、`one_shot_decode_elapsed_ms_max=144`、`one_shot_decode_input_write_elapsed_ms=1051`、`one_shot_decode_input_write_elapsed_ms_max=94`、`one_shot_decode_output_read_elapsed_ms=952`、`one_shot_decode_output_read_elapsed_ms_max=118`、`one_shot_decode_output_read_exact_elapsed_ms=815`、`one_shot_decode_output_read_exact_elapsed_ms_max=112`、`decode_process_spawn_elapsed_ms=129`、`decode_process_wait_elapsed_ms=93` を current baseline にする
...
- latest diagnostics では extra-output probe は主犯ではない。`one_shot_decode_extra_output_probe_elapsed_ms=124` に留まり、`one_shot_decode_input_write_elapsed_ms=1051` と `one_shot_decode_output_read_elapsed_ms=952` のほうが支配的だった。spawn/wait も `129ms` / `93ms` に留まり、現時点の主犯扱いはしない
- current one-shot FFmpeg path の dominant candidate は `stdin write` と `stdout raw BGRA read volume` に絞る。latest rerun では `one_shot_decode_expected_output_bytes_per_frame=3686400`、`decode_stdout_expected_bytes_total=95846400` で、raw BGRA output volume 自体が first-order factor になっている
- current code では next safer slice として、two-real preview loop 限定の scaled decode output を additive に追加した。`TimedSwitcherH264DecodeRuntime` は current two-real path だけ `scaled_output_enabled=true` / `scaled_output_reason=two_real_slot_size` で decode input を `640x360` に override し、FFmpeg one-shot process は `scale=640:360:flags=neighbor` を通して raw BGRA stdout を返す。full-size decode path、persistent config-disabled toggle、4-client path は維持したままにしている
- latest compile-fix / build-validation step では、`scaled_output_enabled=true` の decode input は request/response persistent decoder path を試行せず one-shot decode path に直行するように固定した。persistent helper / spawn path を scaled-output 対応へ広げ直さず、frozen 候補の persistent decoder は current slice でも revive していない。`cargo fmt`、`cargo check -p stream-sync-switcher`、focused summary formatting test、related two-real focused tests、`git diff --check` は PASS した
- next rerun では `one_shot_decode_output_width=640`、`one_shot_decode_output_height=360`、`one_shot_decode_scaled_output_enabled=true`、`one_shot_decode_scaled_output_reason=two_real_slot_size`、`one_shot_decode_expected_output_bytes_per_frame=921600`、`decode_stdout_expected_bytes_total` の低下を main gate にする。current step では runtime evidence は未回収で、Production Readiness は引き続き FAIL 扱いに留める
...
- current one-shot FFmpeg path の safer slice 候補は、request/response persistent decoder ではなく、payload bytes / expected stdout bytes / keyframe payload size と elapsed の関係を見たうえで、one-shot process を残したまま削れる read/write 周辺の小改善か、narrow diagnostics 追加に限定する
...
## Next Items
1. next rerun は `S:\stream-sync` で `--disable-persistent-decoder` を付けた one-shot-only baseline を継続し、`one_shot_decode_output_width=640` / `one_shot_decode_output_height=360` / `one_shot_decode_scaled_output_enabled=true` / `one_shot_decode_expected_output_bytes_per_frame=921600` / `decode_stdout_expected_bytes_total` を回収する
2. same rerun で `one_shot_decode_input_write_elapsed_ms` と `one_shot_decode_output_read_elapsed_ms` / `one_shot_decode_output_read_exact_elapsed_ms` が baseline `manual-logs/two-client-render-rerun-20260517-194136` よりどこまで下がるか比較する
