...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-121040` を受けた persistent decoder 最小 additive slice を `apps/switcher` に追加した。current one-shot FFmpeg decode path は温存し、two-real preview loop だけ `SwitcherPersistentFfmpegH264DecodeRuntimeHook` を通す形にした
- persistent decoder は FFmpeg process と stdin/stdout pipe を継続利用し、initial bootstrap では `SPS/PPS + IDR` を要求する安全側 preflight を入れた。persistent path が write/read/request-channel で失敗した場合は process restart を試し、その call は one-shot fallback に流す
- two-real preview summary には persistent path observability を追加した。`persistent_decode_enabled`、`persistent_decode_attempt_count`、`persistent_decode_success_count`、`persistent_decode_failure_count`、`persistent_decode_fallback_count`、`persistent_decode_process_spawn_count`、`persistent_decode_process_restart_count`、`persistent_decode_stdin_write_elapsed_ms`、`persistent_decode_stdout_read_elapsed_ms`、`persistent_decode_stdout_read_exact_elapsed_ms`、`persistent_decode_output_bytes_total`、`persistent_decode_last_error`、`one_shot_decode_fallback_count` で summary line から path/fallback/restart を追える
- persistent decoder 最小 slice の build validation は通った。`cargo fmt` と `cargo check -p stream-sync-switcher` は PASS、focused test `switcher_four_view_two_real_handoff_preview_summary_formats_expected_fields` も PASS した。compile fix は persistent extra-output error formatting の borrow checker 解消と、unused fallback field の削除に留めた
...
- runtime rerun はまだ未実施で、`decode_output_read_exact_elapsed_ms` / `decode_output_read_elapsed_ms` が latest baseline からどこまで下がるかは未回収のまま。direct compose / zero-fill removal は維持し、Production Readiness は FAIL 継続
...
## Next Items
1. same-PC `2`-client rerun を次 step で回し、`persistent_decode_*` diagnostics と既存 `decode_output_read_exact_elapsed_ms` / `decode_output_read_elapsed_ms` / `decode_process_spawn_elapsed_ms` を回収する
2. rerun で persistent path の success/fallback 比率と `persistent_decode_last_error` を確認し、initial bootstrap gate や restart/fallback policy が狭すぎるかを判断する
3. build は通ったので、次 step でも direct compose と zero-fill removal は維持したまま runtime evidence 回収だけに集中し、`4`-client all-real や distributed-PC actual run へは広げない
