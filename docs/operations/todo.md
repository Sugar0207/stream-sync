...
## 直近でやること
1. single-client queue source boundary を targetTime / jitter-buffer selection へ接続する最小 integration を検討・実装する
...
- switcher/sync-facing single-client queue source now wraps the server queue read boundary with explicit `PreviewLatest` and `ConsumeOldest` modes scoped by client/run, returning switcher encoded-frame handoff data without targetTime, decode, rendering, 4-view, or OBS behavior.
...
## Next Items
1. Connect the single-client queue source boundary to targetTime / jitter-buffer selection without adding late-drop mutation yet.
2. Decide whether the first targetTime integration should use `PreviewLatest` or `ConsumeOldest` for manual queue progression.
