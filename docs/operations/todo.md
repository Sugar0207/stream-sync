...
## 直近でやること
1. server-mediated 2-client validation boundary を `SwitcherQueuedFrameSource` 経由に寄せ、既存の targetTime / display / composition / render 経路を維持する
2. server->switcher production transport / manual runtime hook の最小候補を、in-process source interface の結果を基準に計画する
...
## Next Items
1. Route the server-mediated 2-view validation boundary through `SwitcherQueuedFrameSource` while preserving current targetTime / display / composition / render visibility.
