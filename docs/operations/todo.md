...
## 直近でやること
1. fragmented real encoded sender -> server reassembly queue path の手動通し確認
...
- server-side `VideoFrameFragment` reassembly now accepts authenticated fragments into caller-owned state keyed by client / run / frame, rejects inconsistent metadata, ignores duplicates explicitly, reconstructs complete payloads in chunk order, and queues completed frames through the existing server video frame queue storage.
...
## Next Items
1. fragmented real encoded sender -> server reassembly queue path manual verification
2. production H.264 encoder configuration / error logging policy
