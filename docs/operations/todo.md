...
## 直近でやること
1. `SwitcherQueuedFrameSource` の上に transport-neutral / fallible な server->switcher handoff contract を最小追加し、no-frame と source/transport failure を分離して表現する
2. その contract を current in-process `SwitcherInProcessServerQueueFrameSource` でテストし、IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
- production/manual handoff hook planning: the next useful hook should be a transport-neutral, fallible handoff contract around `SwitcherQueuedFrameSource`, not a new manual command and not a local IPC/TCP prototype. Switcher should request one latest/oldest/dequeue read per `client_id + run_id`; queue snapshots remain diagnostic-only, and targetTime-aware selection stays in switcher. Normal no-frame results must remain distinct from source unavailable, timeout, unsupported mode, malformed response, and source shutdown errors.
...
## Next Items
1. Add the minimal transport-neutral / fallible server->switcher handoff contract around `SwitcherQueuedFrameSource`, backed by the current in-process adapter for tests.
