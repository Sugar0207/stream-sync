...
## Next Items
1. `render_buffer_copy_elapsed_ms=2225` / `avg_render_elapsed_ms=20.275` / `render_buffer_bytes_copied_total=169574400` を起点に、render buffer copy と Windows GDI render path を次の narrow bottleneck として整理する
2. `avg_decode_elapsed_ms=72.833` と `decode_output_buffer_reuse_count=0` を起点に、decode output reuse を次の candidate slice として整理する
3. persistent decoder は future follow-up candidate としてだけ残し、今回の rerun evidence と衝突しない最小導入条件を docs 上で先に整理する
