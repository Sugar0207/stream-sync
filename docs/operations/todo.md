...
## 直近でやること
1. 4-view orchestration の次 slice として、`SwitcherFourViewQuadRenderFacingConnectionBoundary` の上に optional isolated OS window render boundary を最小追加し、`RenderReady` / `NoRenderableQuadView` / `InvalidQuadView` を collapse せず future OBS consumer と共有できる render result family を維持する
