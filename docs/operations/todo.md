...
- next code candidate should move toward slot0 / two-real / opt-in continuous output pipeline experiment planning: stdout/raw BGRA pipe throughput, FFmpeg scale path split, completed correspondence latency diagnostics, and reader blocking phase diagnostics. Threshold branch remains HOLD / candidate, one-shot suppression remains strong contributor evidence, and Production Readiness remains FAIL
...
## 直近でやること
1. next docs-first/code planning は `S:\stream-sync\manual-logs\two-client-output-availability-rerun-20260527-173716` を最新 evidence として、threshold tuning ではなく output pipeline / stdout reader / FFmpeg scale path 側に進める。valid runtime は引き続き `C:\streamsync-target\stream-sync-rerun\debug\*.exe`
2. 次 code を切るなら behavior-changing default ではなく、slot0 / two-real / opt-in continuous 限定の stdout/raw BGRA pipe throughput experiment、FFmpeg scale path split experiment、completed correspondence latency diagnostics、reader blocking phase diagnostics の比較から選ぶ
...
4. compare stdout/raw BGRA pipe throughput, FFmpeg scale path split, completed correspondence latency diagnostics, and reader blocking phase diagnostics next; do not adopt source-size raw output by default
