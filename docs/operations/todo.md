...
## Next Items
1. next step では one-shot process を残したまま `stdin write` cost を narrow に再確認し、pipe backpressure か FFmpeg側 consumption wait が主に効いているかを code path ベースで整理する
2. 同時に `stdout raw BGRA read volume` を safer slice 候補として見直し、raw BGRA 以外の intermediate pixel format が switcher 側の最終 pixel format と両立するかを調査する
3. request/response persistent decoder は凍結候補のまま維持し、continuous-stream decoder rewrite、decode cache ownership / buffer reuse follow-up、extra-output probe follow-up は比較対象としてだけ残す
