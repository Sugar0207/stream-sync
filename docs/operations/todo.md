...
## 直近でやること
1. `--four-view-clean-output-window-loop [all-renderable] [frames]` を追加し、bounded frame / fixed 30fps cadence で dedicated clean output window `StreamSync 4-view Output` を維持しながら `frames_attempted` / `frames_rendered` / `render_failures` / `window_title` / `width` / `height` / `bgra_payload_len` を stdout に出す
...
- [ ] dedicated clean output continuous/runtime path を追加する
- [ ] `--four-view-clean-output-window-loop [all-renderable] [frames]` を追加する
- [ ] OBS で映像表示に成功する
- [ ] 720p / 30fps で表示確認する
- [ ] 長時間表示でも安定することを確認する
- [ ] 不要 UI 非表示モードを作る
