...
- 次に追加する switcher command の第一候補は bounded manual preview loop で、server 側は新 command を増やさず既存 `--receive-auth-video-queue-and-serve-handoff-many` を再利用する。switcher 側は `--four-view-real-handoff-preview-loop [pipe-name] [real-slot-index] [client-id] [run-id] [frames]` を planning shape とし、stdout では `real_handoff=true` / `real_slot_count` / `real_slot_index` / per-slot `client_id` / `run_id` / `scheduler_status` / per-slot result kind / clean output render result / `window_title=StreamSync 4-view Output` を少なくとも可視化する方針にした
...
## 直近でやること
1. bounded mixed real preview command を実装する
2. first slice は `1` real handoff slot + `3` deterministic non-real slots で existing named-pipe handoff wrapper と `SwitcherFourViewHandoffValidationBoundary` / clean output family を接続する
3. stdout に `real_handoff=true` / `real_slot_count` / `real_slot_index` / per-slot `client_id` / `run_id` / `scheduler_status` / per-slot result kind / clean output render result / `window_title=StreamSync 4-view Output` を追加する
