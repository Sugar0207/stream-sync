...
- first completed correspondence latency diagnostics slice is implemented for slot0 / two-real / opt-in continuous and tracked in `docs/operations/continuous-output-pipeline-experiment-plan.md`. Summary now separates pending correspondence age from completed correspondence latency using completed count, avg/max/latest latency, slow count/threshold, and completed frame-id min/max. Threshold branch remains HOLD / candidate, one-shot suppression remains strong contributor evidence, and Production Readiness remains FAIL
...
2. 次 human rerun では pending correspondence age/count と completed correspondence latency avg/max/latest/slow を並べて読み、unfinished backlog と completed output delay が同じ形かを確認する。その後 stdout/raw BGRA pipe throughput experiment、FFmpeg scale path split experiment、reader blocking phase diagnostics の順で比較する
...
4. completed correspondence latency diagnostics は実装済み。次は rerun evidence を読んでから stdout/raw BGRA pipe throughput、FFmpeg scale path split、reader blocking phase diagnostics を比較する; do not adopt source-size raw output by default
