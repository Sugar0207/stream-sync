...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-004552` では、direct `1280x720` compose が two-real clean output path に効き、half-scale branch は消えた。`render_buffer_half_scale_count=0`、`render_buffer_same_size_copy_count=32`、`render_buffer_passthrough_count=0`、`render_buffer_generic_scale_count=0`、`render_buffer_scale_loop_elapsed_ms=0`、`render_buffer_cpu_scale_copy_elapsed_ms=3`、`render_buffer_output_copy_elapsed_ms=3` だった
- 同 rerun では render materialization が大幅に軽くなった。`avg_render_elapsed_ms=1.143`、`render_elapsed_ms=120`、`effective_render_fps_after_first_render=12.920`、`effective_render_fps=11.224`、`render_buffer_bytes_copied_total=117964800` で、前回 direct-compose baseline `manual-logs/two-client-render-rerun-20260516-171634` の `avg_render_elapsed_ms=17.694`、`render_elapsed_ms=1911`、`effective_render_fps_after_first_render=10.948`、`effective_render_fps=9.577`、`render_buffer_half_scale_count=29`、`render_buffer_scale_loop_elapsed_ms=1779` から明確に改善した
- branch diagnostics / materialization diagnostics / source-only `selected_source` churn fix は維持する。`selected_source_changed_count` は diagnostics として残し、render-facing 主因としては扱わない一方で、two-real clean output path では direct compose が materialization bottleneck を解消した
- server/client/handoff は clean のままで、`server frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0`、`client1/client2 frames_sent=900`、`send_failures=0`、`encode_failures=0` だった。`decode_elapsed_ms=2180`、`decode_output_read_elapsed_ms=1654`、`decode_output_buffer_reuse_count=0` なので、残る主 bottleneck は decoder-side follow-up
- direct compose は current half-scale helper の visual semantics、つまり `2x2` block 左上 pixel 採用を維持しつつ、branch diagnostics / materialization diagnostics / source-only churn fix / placeholder behavior / `4`-client all-real path を壊していない。Production Readiness は 30fps target 未達のまま FAIL 継続
...
## Next Items
1. decoder-side follow-up の候補を切り出し、`decode_elapsed_ms=2180` / `decode_output_read_elapsed_ms=1654` / `decode_output_buffer_reuse_count=0` の残り cost を整理する
2. direct compose は two-real clean output では有効だったので、`4`-client all-real へ広げる前に decoder-side bottleneck の方を先に詰める
3. source-only `selected_source` churn と branch/materialization diagnostics は維持しつつ、次回 rerun の比較軸を direct-compose 後 baseline に固定する
