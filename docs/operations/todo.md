...
## 直近でやること
1. 4-view orchestration の次 slice として、`SwitcherFourViewQuadRenderFacingConnectionOutput` を consume する dedicated composed-canvas window render boundary を最小追加し、既存 `SwitcherWindowRenderRuntimeHook` を再利用しつつ `RenderReady` / `NoRenderableQuadView` / `InvalidQuadView` と slot metadata / aggregate scheduler status を explicit に保持する
