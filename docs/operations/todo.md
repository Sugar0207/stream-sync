...
## 直近でやること
1. `--four-view-two-real-handoff-preview-loop [pipe-name] [slot0-index] [client0-id] [run0-id] [slot1-index] [client1-id] [run1-id] [frames]` の manual actual pass を記録する
2. `configs/manual/server.two-real-slots.toml`、`configs/manual/client.player1.toml`、`configs/manual/client.player2.toml` を使って existing server bounded handoff service session 1 本・named pipe 1 本・2 distinct real `client_id + run_id` scopes で `2` real slots + `2` placeholders の clean output / OBS downstream 挙動を再手動確認する
