...
## 直近でやること
1. 次は `docs/operations/continuous-pixel-conversion-plan.md` を source of truth として、slot0 / two-real / opt-in continuous 限定の BGR24 conversion optimization を最小 code slice 候補にする。valid runtime は引き続き `C:\streamsync-target\stream-sync-rerun\debug\*.exe`
2. raw pipe bytes hypothesis は PARTIAL PASS、default-bgra 継続、scaled-bgr24 adoption HOLD / FAIL を維持する。次点で FFmpeg scale path split opt-in experiment、reader blocking phase diagnostics の順に比較する
