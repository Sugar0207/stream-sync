...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-170552` では、persistent decoder fail-fast / circuit breaker は狙い通り効いた。`persistent_decode_attempt_count=1`、`persistent_decode_process_spawn_count=1`、`persistent_decode_process_restart_count=0`、`persistent_decode_runtime_disabled=true`、`persistent_decode_runtime_disabled_reason=persistent_decode_stdout_read_timeout`、`persistent_decode_skipped_after_disabled_count=50`、`persistent_decode_timeout_count=1` で、timeout 1 回後に restart storm は止まった
- circuit breaker 後の rerun では FPS も `manual-logs/two-client-render-rerun-20260517-125737` の `effective_render_fps_after_first_render=4.411` から `8.107` へ回復した。`effective_render_fps=6.887`、`decode_elapsed_ms=7661`、`avg_decode_elapsed_ms=150.216` まで戻っており、timeout wait regression stop としては PASS だが、one-shot baseline 未満のままである
- ただし persistent decoder 自体は runtime FAIL 継続で、`persistent_decode_success_count=0`、`persistent_decode_failure_count=1`、`persistent_decode_fallback_count=51`、`one_shot_decode_fallback_count=51` だった。persistent path は性能改善に寄与しておらず、current request/response 仮定は successful optimization ではない
- two-real preview loop には `--disable-persistent-decoder` の最小 runtime toggle を追加した。config-disabled rerun では `persistent_decode_config_enabled=false`、`persistent_decode_runtime_disabled=false`、`persistent_decode_skipped_by_config_count`、`one_shot_decode_attempt_count`、`one_shot_decode_elapsed_ms`、`one_shot_decode_input_write_elapsed_ms`、`one_shot_decode_output_read_elapsed_ms`、`one_shot_decode_output_read_exact_elapsed_ms` で one-shot-only baseline と circuit-breaker run を区別できる
- latest rerun の dominant cost は guarded one-shot fallback 側に戻っている。`decode_input_write_elapsed_ms=2578`、`decode_output_read_elapsed_ms=2359`、`decode_output_read_exact_elapsed_ms=1957`、`decode_process_spawn_elapsed_ms=377` が残り、同 run では `quad_view_compose_elapsed_ms=4041`、`avg_quad_view_compose_elapsed_ms=55.356`、`gdi_paint_wait_elapsed_ms=1192` も無視できない variance として再観測された
- current request/response persistent decoder 実装は凍結候補として扱う。continuous-stream decoder rewrite は別設計候補として保留し、この step ではそこへ進まず、next candidate を one-shot fallback path の input write / output read 再調査、decode cache ownership / buffer reuse follow-up、quad view / GDI variance 再確認に narrow 化する
- Production Readiness は FAIL 継続。direct compose / zero-fill removal / circuit breaker は維持するが、persistent decoder は current shape のまま main optimization path にしない
...
## Next Items
1. `S:\stream-sync` 側の next same-PC `2`-client rerun では `--four-view-two-real-handoff-preview-loop ... --disable-persistent-decoder` を使い、pure one-shot-only baseline の `one_shot_decode_attempt_count` / `one_shot_decode_elapsed_ms` / `one_shot_decode_input_write_elapsed_ms` / `one_shot_decode_output_read_elapsed_ms` / `one_shot_decode_output_read_exact_elapsed_ms` を回収する
2. one-shot-only baseline と circuit-breaker run を比較し、`decode_input_write_elapsed_ms` / `decode_output_read_elapsed_ms` / `decode_output_read_exact_elapsed_ms` と `quad_view_compose_elapsed_ms` / `gdi_paint_wait_elapsed_ms` のどちらが next dominant かを narrow に見直す
3. request/response persistent decoder は current path として凍結候補のまま維持し、continuous-stream decoder rewrite と decode cache ownership / buffer reuse follow-up は比較対象としてだけ残す
