...
## 現在位置
- 2026-06-03 latest snapshot-style low-cost Preview validation used `--program-first-preview-refresh-interval 5 --program-first-preview-decode-refresh-interval 90 --operator-preview-snapshot-retention`. OBS setup is correct: OBS captured `StreamSync Program Output`, did not capture `StreamSync 4-view Output`, and Program did not mix 4-view / borders / debug UI / Preview labels. ProgramOutput is structurally promising but not closeout-ready: black / placeholder none, perceived stutter small, continuous latest used (`program_render_used_continuous_latest_count=2736`), one-shot fallback `0`, black / placeholder counters `0`, but `program_render_effective_fps=16.201`, startup was slow (`program_output_first_render_elapsed_ms=16045`), selected source was missing before first render (`program_output_missing_selected_source_count=264`, all before first render, reason `NoDecodedFrameForSelection`), and smooth-latest lag is still large / not acceptance-defined (`program_selected_source_frame_lag=299`, `program_continuous_selected_frame_lag=285`, `continuous_decode_latest_selected_to_output_frame_gap=299`). Snapshot retention fixed Preview black/flicker (`operator_preview_snapshot_retention_enabled=true`, `operator_preview_snapshot_reuse_count=2743`, `operator_preview_placeholder_avoided_by_snapshot_count=2743`, `operator_preview_slot_black_after_snapshot_count=0`; client1/client2 were not black and did not flicker), but Preview remained too slow for operator monitoring. Same-loop low-cost Preview refresh tuning is limited / paused. Do not claim ProgramOutput near-MVP closeout yet; next work is ProgramOutput non-FPS blocker audit, first-render / missing-selected-source investigation, selected-source verification, latency/lag acceptance criteria, and OBS capture safety checklist
- 2026-06-03 Program-first + `smooth-latest` + Preview one-shot suppression OBS validation is strong structural ProgramOutput evidence, but not a closeout claim: OBS captured `StreamSync Program Output`, did not accidentally capture `StreamSync 4-view Output`, Program had no Preview labels / borders / debug UI mixed in, black / placeholder did not occur, perceived stutter was small, `program_render_effective_fps=21.795`, `continuous_decode_output_throughput_fps=27.102`, `program_render_used_one_shot_fallback_count=0`, `one_shot_decode_attempt_count=0`, and `program_first_remaining_one_shot_decode_count=0`. In the same mode the 4-view Preview did not display (`frames_rendered=0`, `clean_output_render_result_kind=NoRenderableQuadView`, `output_width=none`, `output_height=none`), so `--program-first-validation-mode` is explicitly treated as ProgramOutput validation mode, not final operator mode
- 2026-06-01 opt-in live Program window path is manually validated and now has explicit source selection for `--four-view-two-real-handoff-preview-loop`. `--enable-program-output-window` still gates Program rendering, and `--program-selected-client-id <client_id>` requests the Program source by `selected_client_id`. With no explicit Program selection, fallback behavior remains the first renderable decoded real slot in slot-index order. `selected_slot_index` remains provenance only, `Focused(slot_index)` is not reused as Program state, and missing / non-renderable explicit selections report `MissingSelectedSource` plus a missing-source reason. Manual validation confirmed Preview and Program windows both appeared, Program showed one video only, Preview layout / selected border / debug UI / labels were not mixed into ProgramOutput, and OBS Window Capture listed `StreamSync Program Output`; OBS capture target is still not changed by code. Latest Program continuous decode validation showed the continuous decoder starts and outputs frames (`continuous_decode_output_frame_count=3318`, throughput `17.435fps`), but Program used no continuous frames because target-frame exact/bounded lookup rejected delayed output (`program_render_used_continuous_decoded_count=0`, `program_decode_mode=fallback`, selected/output lag `435`). Current code keeps default `target-frame` behavior unchanged and adds opt-in `--program-continuous-decode-mode smooth-latest`, where ProgramOutput prioritizes smooth continuous playout over low latency by accepting the latest available continuous decoded frame for the explicit Program source
- 2026-06-01 latest `smooth-latest` Program validation now shows Program continuous frames are being used (`program_decode_mode=continuous`, `program_render_used_continuous_latest_count=2887`, `program_render_used_one_shot_fallback_count=1`) and Program black/placeholder counters are no longer the main issue. Perceived stutter remains large with `program_decode_fps=19.724`, `effective_render_fps=12.021`, `quad_view_compose_elapsed_ms=15432`, `render_buffer_cpu_scale_copy_elapsed_ms=8798`, and `one_shot_decode_elapsed_ms=48683`, so the next target is the new opt-in `--program-first-validation-mode`. This mode keeps defaults unchanged, keeps Program selected-only, lets Preview reuse its previous composed output after the first Preview render during validation, and enables existing continuous-source one-shot suppression for the Program continuous source while the continuous process is running. Next long validation should include the Program-first flag and avoid `max_handoff_requests=16000` by using a much larger bounded handoff budget such as `64000` or the current no-budget / very-large-budget validation setting
...
7. [x] add ProgramOutput stability diagnostics and last-valid explicit Program frame reuse for temporary missing selected-source ticks
8. [x] add an opt-in selected Program continuous decode path via `--enable-program-continuous-decode`, mapping explicit Program client selection to the existing single-source continuous decoder without changing defaults
9. [x] add opt-in `--program-continuous-decode-mode smooth-latest` so ProgramOutput can accept delayed latest continuous decoded frames instead of requiring exact/bounded target-frame lookup
10. [x] rerun longer runtime validation with OBS capturing `StreamSync Program Output` using `--enable-program-output-window --program-selected-client-id player2 --enable-program-continuous-decode --program-continuous-decode-mode smooth-latest`; result: continuous frames are used, placeholder/black is improved, but perceived stutter remains
11. [x] add opt-in `--program-first-validation-mode` to reduce Preview workload during Program smoothness validation without changing default behavior
12. [x] suppress remaining non-Program Preview one-shot decode in Program-first validation mode after a previous Preview output exists; keep Program selected-only and add Program-first suppression diagnostics
13. [x] rerun longer Program-first validation with OBS capturing `StreamSync Program Output` using `--enable-program-output-window --program-selected-client-id player2 --enable-program-continuous-decode --program-continuous-decode-mode smooth-latest --program-first-validation-mode`; result: ProgramOutput structural separation improved with no black / placeholder, small perceived stutter, continuous latest used, and one-shot fallback eliminated, but this is not a closeout claim
14. [x] decide Program-first Preview semantics: keep `--program-first-validation-mode` as ProgramOutput validation mode where Preview may be absent, stale, or reduced; do not treat it as the final operator mode
15. [x] add a narrow opt-in low-cost Preview / multiview restore candidate for operator validation via `--program-first-preview-refresh-interval <ticks>`; keep Program-first defaults unchanged and keep non-Program Preview one-shot suppression active
16. [x] manually validate Program-first operator low-cost Preview with OBS still capturing `StreamSync Program Output`; result: ProgramOutput remained selected-only with no black/placeholder, but Preview monitoring failed because client1 was black / decode-deferred and refresh success was too low at interval `30`
17. [x] add opt-in low-frequency non-Program Preview decode allowance via `--program-first-preview-decode-refresh-interval <ticks>`; keep default behavior unchanged and allow at most one non-Program Preview one-shot decode on matching Preview refresh ticks
18. [x] manually validate Program-first operator Preview with both refresh knobs, e.g. `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30`; result: client1 / slot0 became visible, but Program-selected player2 / slot1 stayed black in Preview even while ProgramOutput rendered it through continuous latest
19. [x] add opt-in Program-selected Preview slot reuse for Program-first low-cost Preview mode, reusing Program continuous/latest or last-valid Program frame without requiring one-shot decode for the Program source Preview slot
20. [x] rerun Program-first operator Preview validation with `--program-first-preview-refresh-interval 10 --program-first-preview-decode-refresh-interval 30`; result: Program-selected reuse existed and both slots became visible sometimes, but both slots flickered and Program smoothness worsened, so low-FPS video Preview is not the next operator direction
21. [x] add opt-in snapshot-style low-cost Preview retention via `--operator-preview-snapshot-retention`; keep default behavior unchanged and retain last-visible per-slot snapshots instead of flickering back to placeholder/deferred output
22. [x] validate snapshot-style low-cost Preview with `--program-first-preview-refresh-interval 5 --program-first-preview-decode-refresh-interval 90 --operator-preview-snapshot-retention`; result: snapshot retention fixed black/flicker, but Preview was still too slow for operator monitoring and Program FPS dropped versus 10/90
23. [x] close same-loop low-cost Preview refresh tuning as limited / paused; do not continue lowering the Preview refresh interval in the current Program-first loop path
24. [x] audit ProgramOutput non-FPS blockers before closeout; status is structurally promising but not closeout-ready
25. [ ] investigate first Program render delay and missing selected source before first render (`program_output_first_render_elapsed_ms`, missing-selected-source counters, `NoDecodedFrameForSelection`)
26. [ ] define selected-source visual verification path, including stronger player1/player2 visual distinguishability evidence and selected client/run/slot identity checks
27. [ ] define smooth-latest latency/lag closeout criteria (`program_selected_source_frame_lag`, `program_continuous_selected_frame_lag`, `continuous_decode_latest_selected_to_output_frame_gap`) separately from FPS
28. [ ] write OBS capture safety checklist for ProgramOutput: exact window title, wrong-window prevention, manual misconfiguration evidence, and no Preview UI mixed into Program
29. [ ] later design Program source switching over hotkey or control pipe; keep current CLI/static selection as the only supported switch for now
30. [ ] later design separate Preview cadence/runtime or lighter renderer path for future operator monitoring; treat current 4-view Preview as stable snapshot-only, not the final monitoring Preview
31. [ ] later investigate Preview slot layout rendering / GPU renderer; do not block the first selected-only Program path on this
32. [ ] keep default BGRA as the safe path; optimized `scaled-bgr24` conversion is PASS, but adoption remains HOLD after the `20260528-103130` A/B
33. [ ] run the new opt-in `no-scale-bgra` scale path split A/B before any default promotion; treat it as diagnostics-only because source-size BGRA can be 4x 640x360 BGRA at 1280x720
34. [ ] keep continuous decoder diagnostics/performance work opt-in and non-blocking for ProgramOutput boundary work: no default threshold, feed max, slot1, 4-client, protocol, GPU decode, or one-shot fallback removal changes
