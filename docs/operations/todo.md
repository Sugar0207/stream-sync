...
- latest continuous output-lag diagnostics rerun is `S:\stream-sync\manual-logs\two-client-render-rerun-20260520-014041`. Continuous opt-in, slot0 runtime, low-latency args, bounded feed helper, bounded-lag lookup wiring, and output-lag diagnostics wiring are PASS: `continuous_decode_config_enabled=true`, `continuous_decode_runtime_enabled=true`, `continuous_decode_slot0_enabled=true`, `continuous_decode_ffmpeg_low_latency_args_enabled=true`, `continuous_feed_enabled=true`, and `continuous_decode_bounded_lookup_allowed_lag_frames=5`
- feeder is the primary continuous input source in the latest rerun: `continuous_feed_attempt_count=300`, `continuous_feed_handoff_request_count=930`, `continuous_feed_frame_received_count=418`, `continuous_feed_enqueued_count=412`, `continuous_feed_skipped_count=6`, `continuous_decode_input_from_feeder_count=412`, `continuous_decode_input_from_render_demand_count=5`, and `continuous_decode_feeder_lag_to_selected=7`
- continuous output itself is PASS in the latest rerun: `continuous_decode_input_frame_count=417`, `continuous_decode_output_frame_count=367`, `continuous_decode_queue_len=30`, `continuous_decode_dropped_stale_count=337`, and `continuous_decode_queue_drop_reason_counts=input_queue_full:0|decoded_cache_bound:337|unknown:0`
...
## Next Items
1. `S:\stream-sync\manual-logs\two-client-render-rerun-20260520-014041` を基準に、continuous output throughput が `23fps` 程度に留まる原因候補を docs-first で整理する
2. stdout full-frame read latency、`scale=640:360` + raw BGRA output path、reader buffering、raw frame copy/materialization、one-shot fallback double-load のどこを最小 diagnostics / opt-in experiment 候補にするか決める
3. output throughput を上げる最小候補を、pixel format / scale path見直し、output reader buffering改善、one-shot fallback抑制の危険性、feed max count現状維持の観点で比較する
