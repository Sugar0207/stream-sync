...
## 直近でやること
1. fallible handoff consumer result を targetTime source / 2-view scheduler へ接続する最小方針を計画する。handoff error は no-frame / waiting に潰さない
2. その方針に基づき、targetTime / scheduler 側で handoff error を明示的に保持する最小 validation slice を実装する。IPC/TCP/UDP/shared-memory transport はまだ実装しない
...
## Next Items
1. Plan the smallest targetTime/scheduler consumer path for `SwitcherQueuedFrameHandoffConsumerResult` without collapsing handoff errors into no-frame or waiting.
