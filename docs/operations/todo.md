...
## 現在位置
- same-PC `2`-client unchanged-frame reuse rerun では decode skip 自体は効いたが、switcher loop はまだ低 FPS で、handoff/decode/render 以外の未計測時間が残った。今回の code slice では `--four-view-two-real-handoff-preview-loop` final summary に `loop_total_elapsed_ms` / `attempt_body_elapsed_ms` / `loop_sleep_elapsed_ms` / `frame_interval_wait_elapsed_ms` / `event_pump_elapsed_ms` / `window_update_elapsed_ms` / `quad_view_compose_elapsed_ms` / `render_call_elapsed_ms` / `unaccounted_elapsed_ms` / `avg_attempt_elapsed_ms` / `max_attempt_elapsed_ms` / `slow_attempt_count` / `slow_attempt_threshold_ms` を追加し、sleep・attempt body・window update・validation/compose 由来の時間を切り分けられるようにした。unchanged-frame reuse、source-error placeholder、client encode behavior 非変更、server stopped summary 回収 out of scope は維持
