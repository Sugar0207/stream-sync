...
## 直近でやること
1. local IPC / named pipe 前提の server->switcher handoff request/response DTO と binary codec を定義する
2. existing `ServerVideoFrameQueueReadBoundary` を使う server 側 single-request handoff handler と、`SwitcherQueuedFrameHandoff` を実装する switcher 側 client adapter を追加する
...
## Next Items
1. Decide whether the fallible server-mediated validation path needs a manual/runtime entry point before real transport planning.
