...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-114532` では、zero-fill 除去が狙い通り効き `decode_output_vec_resize_elapsed_ms=0` になった。direct compose は引き続き有効で `render_buffer_half_scale_count=0`、`render_buffer_same_size_copy_count=38`、`render_buffer_scale_loop_elapsed_ms=0` を維持し、render materialization は主犯ではない。`avg_render_elapsed_ms=1.629`、`render_elapsed_ms=189`、`effective_render_fps_after_first_render=10.286`、`effective_render_fps=9.089` だった
- exact-read regression `manual-logs/two-client-render-rerun-20260517-090959` からは回復方向だった。`effective_render_fps_after_first_render=9.628 -> 10.286`、`decode_elapsed_ms=4536 -> 3687`、`decode_output_read_elapsed_ms=1946 -> 1514`、`decode_output_vec_resize_elapsed_ms=1081 -> 0` まで改善した一方、direct compose baseline `manual-logs/two-client-render-rerun-20260517-004552` の `effective_render_fps_after_first_render=12.920`、`decode_elapsed_ms=2180` までは戻っていない
- server/client/handoff は clean のままで、`server frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0`、`client1/client2 frames_sent=900`、`send_failures=0`、`encode_failures=0` だった。decoder-side の残り cost は `decode_process_spawn_elapsed_ms=564`、`decode_input_write_elapsed_ms=1418`、`decode_output_read_exact_elapsed_ms=990`、`decoded_buffer_clone_count=35`、`decode_output_buffer_reuse_count=0`、`decode cache ownership` に集約される
- current one-shot FFmpeg decode path の code-path review では、`decode_process_spawn_elapsed_ms` は `spawn_start` から `Command::spawn()` 成否まで、`decode_input_write_elapsed_ms` は `stdin.write_all(&input.encoded_payload)` のみ、`decode_output_read_exact_elapsed_ms` は `stdout.take(expected_len).read_to_end(...)` のみを計測していることを確認した。`stdin` close/drop、extra stdout probe、`child.wait()`, stderr read/join は別計測または未計測であり、`decode_output_read_exact_elapsed_ms` には FFmpeg 側 decode / stdout production / OS scheduling block が含まれうる
- `decoded_buffer_clone_count` は current summary では 1種類に見えるが、実際には `decoded_cache` hit の return clone と、decode success 後の cache store clone の両方を合算していた。current code には next rerun 用の最小 diagnostics として `decode_input_payload_bytes_total`、`decode_stdout_expected_bytes_total`、`avg_decode_input_write_elapsed_ms`、`decode_cache_hit_clone_count`、`decode_cache_store_clone_count`、`decoded_buffer_clone_elapsed_ms` を追加済みで、persistent decoder に進まずに one-shot path の残り内訳を切り分けられる状態にした
...
## Next Items
1. same-PC `2`-client rerun を次 step で回し、`avg_decode_input_write_elapsed_ms`、`decode_input_payload_bytes_total`、`decode_stdout_expected_bytes_total`、`decode_cache_hit_clone_count`、`decode_cache_store_clone_count`、`decoded_buffer_clone_elapsed_ms` を回収する
2. `decode_output_buffer_reuse_count=0` と `decode cache ownership` を前提に、clone/store 側が支配的か、spawn/write/read 側が支配的かを rerun で再判断する
3. direct compose は維持しつつ、decoder-side baseline が戻るまで `4`-client all-real へは広げない
