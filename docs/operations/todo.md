...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-174753` では、`--disable-persistent-decoder` による pure one-shot-only baseline が成立した。`persistent_decode_config_enabled=false`、`persistent_decode_enabled=false`、`persistent_decode_attempt_count=0`、`persistent_decode_timeout_count=0`、`persistent_decode_process_spawn_count=0`、`persistent_decode_process_restart_count=0`、`persistent_decode_skipped_by_config_count=60` で、persistent decoder config-disabled toggle は PASS だった
...
- one-shot-only baseline の switcher は `effective_render_fps_after_first_render=7.760`、`effective_render_fps=7.114`、`frames_rendered=231` で、本番目標にはまだ遠い。persistent decoder を完全に避けても FPS は回復しておらず、原因を persistent decoder 単独とは扱わない
- latest rerun の decoder-side では one-shot decode I/O が引き続き重い。`decode_attempt_count=60`、`decode_success_count=60`、`decode_elapsed_ms=8010`、`avg_decode_elapsed_ms=133.500`、`decode_process_spawn_elapsed_ms=635`、`decode_input_write_elapsed_ms=3300`、`decode_output_read_elapsed_ms=3671`、`decode_output_read_exact_elapsed_ms=2765`、`decoded_buffer_clone_elapsed_ms=63`、`decode_output_buffer_reuse_count=0` で、`one_shot_decode_attempt_count=60`、`one_shot_decode_elapsed_ms=8010`、`one_shot_decode_input_write_elapsed_ms=3300`、`one_shot_decode_output_read_elapsed_ms=3671`、`one_shot_decode_output_read_exact_elapsed_ms=2765` が pure baseline として取れた
- render-side では `quad_view_compose_elapsed_ms=3899`、`quad_view_compose_success_count=57`、`quad_view_full_compose_count=57`、`quad_view_incremental_update_count=0`、`avg_quad_view_compose_elapsed_ms=62.887` が残り、next dominant candidate は one-shot decode I/O と quad_view_compose full compose cost に絞る。今回 run の `gdi_paint_wait_elapsed_ms=51` は主犯扱いしない
