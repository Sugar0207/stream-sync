...
## Next Items
1. 人間側 `S:\stream-sync` の次回 rerun では既存 two-real command 末尾に `--disable-persistent-decoder --enable-continuous-stream-decoder --continuous-decoder-low-latency-args` を付け、`continuous_decode_ffmpeg_args_summary`、low-latency/probe/loglevel fields、stdout first-byte / partial-byte fields、output count を回収する
2. rerun 結果で `continuous_decode_stdout_first_byte_seen` と `continuous_decode_stdout_partial_bytes_read` を読み、FFmpeg が 1 byte も stdout に出していないのか、full raw frame `921600` bytes 未満で止まっているのかを切り分ける
