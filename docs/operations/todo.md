...
## 現在位置
- latest same-PC `2`-client rerun `manual-logs/two-client-render-rerun-20260517-223121` では、`--disable-persistent-decoder` による pure one-shot-only rerun 上で two-real preview loop 限定 scaled decode output が runtime PASS した。`persistent_decode_config_enabled=false`、`persistent_decode_attempt_count=0`、`persistent_decode_timeout_count=0`、`persistent_decode_skipped_by_config_count=26` で、persistent decoder config-disabled toggle は引き続き PASS だった
- server / client / transport はこの rerun でも成立している。server は `packets_received=36617`、`frames_queued=1800`、`per_client_queued_frames=player1/streamsync-dev-session:900|player2/streamsync-dev-session:900`、`io_error_count=0` で、client1/client2 も `frames_sent=900`、`encode_failures=0`、`send_failures=0` を維持した
- latest rerun の switcher は `effective_render_fps_after_first_render=16.579`、`effective_render_fps=12.942`、`render_elapsed_ms=94` まで改善したが、まだ 30fps 目標には遠い。baseline `manual-logs/two-client-render-rerun-20260517-194136` の `13.201` からは改善しているが、persistent decoder を完全に避けた状態でも FPS 未達なので、原因を persistent decoder 単独とは扱わない
- incremental quad compose は runtime PASS 継続と整理する。`quad_view_compose_elapsed_ms=548`、`quad_view_incremental_update_count=35`、`quad_view_full_compose_count=1`、`gdi_paint_wait_elapsed_ms=11` で、compose/render/GDI wait は current rerun でも first-order culprit ではない
- latest rerun の decoder-side では scaled decode output の runtime evidence を回収できた。`decode_attempt_count=26`、`decode_success_count=26`、`one_shot_decode_attempt_count=26`、`one_shot_decode_elapsed_ms=2029`、`one_shot_decode_elapsed_ms_max=101`、`one_shot_decode_input_write_elapsed_ms=937`、`one_shot_decode_input_write_elapsed_ms_max=57`、`one_shot_decode_output_read_elapsed_ms=816`、`one_shot_decode_output_read_elapsed_ms_max=39`、`one_shot_decode_output_read_exact_elapsed_ms=676`、`one_shot_decode_output_read_exact_elapsed_ms_max=33`、`one_shot_decode_extra_output_probe_elapsed_ms=128` を確認した
- current one-shot path の計測境界は維持されている。`decode_process_spawn_elapsed_ms` は `Command::spawn()` だけ、`decode_input_write_elapsed_ms` は `stdin.write_all(&input.encoded_payload)` だけ、`decode_output_read_exact_elapsed_ms` は `stdout.take(expected_len).read_to_end(...)` だけ、`decode_output_read_elapsed_ms` は bounded read に extra-output probe までを足した範囲で、`child.wait()` は別カウンタだと整理できる
- latest diagnostics では extra-output probe は今回も主犯ではない。`one_shot_decode_extra_output_probe_elapsed_ms=128` に留まり、`one_shot_decode_input_write_elapsed_ms=937` と `one_shot_decode_elapsed_ms=2029` のほうが依然として大きい。spawn/wait は latest dominant candidate には置かない
- current scaled decode slice は runtime PASS した。two-real preview loop は `one_shot_decode_output_width=640`、`one_shot_decode_output_height=360`、`one_shot_decode_output_pixel_format=Bgra8`、`one_shot_decode_scaled_output_enabled=true`、`one_shot_decode_scaled_output_reason=two_real_slot_size`、`one_shot_decode_expected_output_bytes_per_frame=921600` を返し、baseline `3686400` bytes/frame から expected stdout volume を `1/4` に落とせた
- `decode_stdout_expected_bytes_total` と `decode_output_bytes_total` は `23961600` まで下がり、baseline `manual-logs/two-client-render-rerun-20260517-194136` の `95846400` から大幅に低下した。`one_shot_decode_output_read_elapsed_ms=952 -> 816`、`one_shot_decode_output_read_exact_elapsed_ms=815 -> 676` も改善している
- latest compile-fix / build-validation step で fixed した one-shot-only routing は runtime でも成立している。`scaled_output_enabled=true` の decode input は request/response persistent decoder path を試行せず one-shot decode path に直行し、persistent helper / spawn path を scaled-output 対応へ広げ直していない
- final diagnostics には `handoff_error_count=15` と `scheduler_status=HandoffError` が混ざったため注意は必要だが、server/client transport 成立、persistent config-disabled toggle、scaled decode output evidence 自体は有効扱いにする
- keyframe/non-keyframe split は今回判断保留とする。latest rerun は `one_shot_decode_keyframe_attempt_count=0`、`one_shot_decode_non_keyframe_attempt_count=26`、`one_shot_decode_keyframe_elapsed_ms=0`、`one_shot_decode_non_keyframe_elapsed_ms=2287` で、IDR payload evidence がまだない
- current one-shot FFmpeg path の next dominant candidate は `stdin write` / FFmpeg stdin consumption wait / payload size impact に移す。stdout raw BGRA read volume は scaled decode output により大きく下がったため、次は payload bytes と `stdin.write_all(...)` / FFmpeg側 consumption wait の関係を code path と rerun evidence ベースで整理する
- current code では `decode_input_write_elapsed_ms` は `stdin.write_all(&input.encoded_payload)` だけを測っており、これは単純なメモリコピー時間ではなく、OS pipe backpressure と FFmpeg 側が stdin を消費して pipe 容量を空けるまでの待ちを含み得る。逆に `stdin` handle drop / EOF close は現状この timer に含まれていない
- latest rerun では `one_shot_decode_input_payload_bytes_avg=195142.192` と baseline `96744.115` より payload avg が大きく、scaled decode output により stdout 側が軽くなった後は payload size impact を切り分ける必要がある。挙動変更より先に、one-shot path を維持したまま入れられる safer diagnostics 追加を next slice 候補にする
- latest implementation step では one-shot decode の挙動を変えずに safer diagnostics を追加した。summary には `one_shot_decode_stdin_close_elapsed_ms`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdout_first_byte_elapsed_ms`、`one_shot_decode_write_throughput_bytes_per_ms`、`one_shot_decode_input_write_elapsed_per_payload_kb`、`one_shot_decode_read_throughput_bytes_per_ms` が出る。first byte は expected raw output length を壊さないよう 1 byte だけ先読みして残りを既存 bounded read に流す形で、scaled decode output / incremental compose / persistent config-disabled toggle は維持した
- current request/response persistent decoder 実装は凍結候補として扱う。continuous-stream decoder rewrite は別設計候補として保留し、今回の baseline 以後も main optimization path へ戻さない。GDI wait は latest rerun でも `gdi_paint_wait_elapsed_ms=15` に留まり、現時点の主犯扱いはしない。Production Readiness は FAIL 継続
...
## Next Items
1. next step では `S:\stream-sync\manual-logs\two-client-render-rerun-20260517-223121` を baseline にして、`one_shot_decode_input_write_elapsed_ms=937` と payload bytes `min=96690 / max=230687 / avg=195142.192` の関係を整理し、FFmpeg stdin consumption wait がどこまで支配的かを narrow に切り分ける
2. `S:\stream-sync` の next rerun では `one_shot_decode_stdin_close_elapsed_ms`、`one_shot_decode_stdin_write_to_stdout_first_byte_elapsed_ms`、`one_shot_decode_stdout_first_byte_elapsed_ms`、derived throughput / per-payload-kb summary を回収し、write中の backpressure と write後の first-byte wait のどちらが目立つかを確認する
3. request/response persistent decoder は凍結候補のまま維持し、continuous-stream decoder rewrite、decode cache ownership / buffer reuse follow-up、raw BGRA 以外の intermediate pixel format 比較は次候補としてだけ残す
