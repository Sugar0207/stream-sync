...
- 2026-05-13 の narrow switcher parity slice は完了した。`--four-view-four-real-handoff-preview-loop` は optional preview mode `[preview-oldest|preview-latest|preview-latest-decodable]` を受け付け、`preview-latest-decodable` で `PreviewLatestDecodableIfAtOrBefore` / `InspectLatestDecodable` を使える。retained-keyframe fallback は 2-client PASS path と同じ read-mode mapping に揃い、4-real targetTime も tick ごとに再計算される。summary には `preview_mode` / `read_mode` も出るため、次の main gate は human rerun の evidence 回収である
...
## Next Items
1. same-PC `4`-client all-real validation を `preview-latest-decodable` 付きの current 4-real command で rerun し、client/server/switcher stdout を `manual-logs/` に回収する
2. rerun の final switcher state を `AllSelected` / `Rendered` 基準で再分類し、`preview_mode` / `read_mode` / slot diagnostics / `decodable_source` を見て retained-keyframe fallback 実使用の有無を確認する
