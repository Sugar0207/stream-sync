...
/// One-shot client-side real encoded `VideoFrame` send PoC.
///
/// This launcher uses the existing client PoC config for destination and
/// client/session metadata, creates a primary-display Windows Graphics Capture
/// session when the platform runtime supports it, acquires one BGRA frame,
/// encodes it with the FFmpeg software hook, and sends exactly one
/// `RealCaptureH264` `VideoFrame`. It does not authenticate, loop, retry,
/// decode, render, or integrate OBS.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct ClientRealEncodedVideoFramePocLauncher {
...
impl ClientRealEncodedVideoFramePocLauncher {
    pub fn load_startup_config_from_path(
        &self,
        path: impl AsRef<Path>,
    ) -> Result<ClientRealEncodedVideoFramePocStartupConfig, ClientRealEncodedVideoFramePocError>
    {
        let path = path.as_ref();
        let content =
            fs::read_to_string(path).map_err(|error| ClientRealEncodedVideoFramePocError::Io {
                path: path.to_path_buf(),
                message: error.to_string(),
            })?;
        self.load_startup_config_from_str(&content)
    ...
    {
        let settings = parse_client_poc_settings(input)
            .map_err(ClientRealEncodedVideoFramePocError::from_auth_request_error)?;
        let destination = resolve_destination(&settings.server_host, settings.server_port)
            .map_err(ClientRealEncodedVideoFramePocError::from_auth_request_error)?;

        Ok(ClientRealEncodedVideoFramePocStartupConfig {
            destination,
            protocol_version: ProtocolVersion(settings.protocol_version),
            client_id: ClientId(settings.client_id),
            run_id: RunId(settings.run_id),
    ...
    pub fn send_with_socket_and_runtimes(
        ...
        let session_config = match self.session_config.prepare_from_target_config(
            ...
            ClientCaptureSessionConfigPrepareResult::NotPrepared { backend, reason } => {
                return Ok(
                    ClientRealEncodedVideoFramePocOutcome::SessionConfigNotPrepared {
                        destination: startup_config.destination,
                        backend,
                        reason,
                    },
        ...
        let mut capture_runtime = match self.session_runtime.create_session_with_runtime(
            ...
            ClientCaptureSessionRuntimeCreationResult::NotCreated {
                ...
                return Ok(ClientRealEncodedVideoFramePocOutcome::SessionNotCreated {
                    destination: startup_config.destination,
                    backend,
                    reason,
                    message,
        ...
        match self.one_shot.send_once_with_runtimes(
            ...
            ClientRealEncodedVideoFrameOneShotResult::Sent(sent) => {
                Ok(ClientRealEncodedVideoFramePocOutcome::Sent(sent))
            }
            result => Ok(ClientRealEncodedVideoFramePocOutcome::NotSent {
                destination: startup_config.destination,
                result,
            }),
        }
...
/// Convenience entry point for the manual real encoded video PoC.
pub fn run_real_encoded_video_frame_poc_once_from_path(
    path: impl AsRef<Path>,
) -> Result<ClientRealEncodedVideoFramePocOutcome, ClientRealEncodedVideoFramePocError> {
...
/// Startup config for one real encoded `VideoFrame` send.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientRealEncodedVideoFramePocStartupConfig {
    pub destination: SocketAddr,
    pub protocol_version: ProtocolVersion,
    pub client_id: ClientId,
    pub run_id: RunId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientRealEncodedVideoFramePocOutcome {
    Sent(ClientRealEncodedVideoFrameOneShotSent),
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum ClientHeartbeatLoopReconnectReason {
    SocketReestablishmentRequested,
}
...
/// Explicit wakeup state preserved for one future outer while-loop turn.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientHeartbeatLoopOuterWhileLoopWakeupState {
    NoWakeupSideEffect,
    WakeupSideEffectApplied {
        wakeup_apply: ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupApplyResult,
        wakeup_side_effect:
            ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupActualSideEffectApplyResult,
    },
}

/// Continue-path output surfaced after outer while-loop connection remains explicit.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientHeartbeatLoopOuterWhileLoopConnectionOutput {
    pub carry: ClientHeartbeatLoopRepeatedInvocationNextStepCarry,
...
/// Result of wiring one completed body turn into future outer while-loop state.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientHeartbeatLoopOuterWhileLoopConnectionResult {
    Continue {
        output: ClientHeartbeatLoopOuterWhileLoopConnectionOutput,
    },
    Stop {
        output: ClientHeartbeatLoopCompletedBodyTerminalOutput,
    },
...
/// Explicit input handed into reconnect policy from outer while-loop actual execution.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientHeartbeatLoopOuterWhileLoopReconnectPolicyInput {
    pub output: ClientHeartbeatLoopOuterWhileLoopActualExecutionOutput,
}

impl ClientHeartbeatLoopOuterWhileLoopReconnectPolicyInput {
    pub fn from_actual_execution(
        ...
        match execution {
            ...
            ClientHeartbeatLoopOuterWhileLoopActualExecutionResult::Stop { output } => Err(output),
        }
    }
}

/// Future socket re-establishment plan kept explicit before runtime behavior exists.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum ClientHeartbeatLoopFutureSocketReestablishmentPlan {
    ReestablishSocket {
        reason: ClientHeartbeatLoopReconnectReason,
    },
}

/// Continue-path reconnect policy handoff returned before socket re-establishment runs.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientHeartbeatLoopOuterWhileLoopReconnectPolicyHandoff {
    pub output: ClientHeartbeatLoopOuterWhileLoopActualExecutionOutput,
    pub reconnect_plan: ClientHeartbeatLoopFutureSocketReestablishmentPlan,
}

/// Result of applying reconnect policy after actual reconnect execution stays explicit.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult {
    ContinueWithoutReconnect {
        output: ClientHeartbeatLoopOuterWhileLoopActualExecutionOutput,
    },
    ContinueWithReconnectPlanned {
        handoff: ClientHeartbeatLoopOuterWhileLoopReconnectPolicyHandoff,
    },
    Stop {
        output: ClientHeartbeatLoopCompletedBodyTerminalOutput,
    },
...
/// Explicit input handed into socket re-establishment from reconnect policy handoff.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentInput {
    pub handoff: ClientHeartbeatLoopOuterWhileLoopReconnectPolicyHandoff,
}

impl ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentInput {
    pub fn from_reconnect_policy(
        policy: ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult,
    ) -> Result<Self, ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult> {
        match policy {
            ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult::ContinueWithReconnectPlanned {
                handoff,
            } => Ok(Self { handoff }),
            passthrough => Err(passthrough),
        }
...
/// Continue-path output surfaced after socket re-establishment stays explicit.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentOutput {
    pub output: ClientHeartbeatLoopOuterWhileLoopActualExecutionOutput,
    pub reconnect_plan: ClientHeartbeatLoopFutureSocketReestablishmentPlan,
    pub socket_reestablishment: ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentApplyResult,
}

/// Result of connecting reconnect policy into minimal socket re-establishment flow.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientHeartbeatLoopOuterWhileLoopReconnectResult {
    ContinueWithoutReconnect {
        output: ClientHeartbeatLoopOuterWhileLoopActualExecutionOutput,
    },
    ContinueWithReconnectDeferred {
...
/// Explicit reconnect state surfaced to outer while-loop continuation without collapsing concerns.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientHeartbeatLoopOuterWhileLoopReconnectState {
    NoReconnectNeeded,
    ReconnectDeferred {
...
pub struct ClientHeartbeatLoopOuterWhileLoopRepeatedBodyContinuationState {
    ...
    pub last_reconnect: ClientHeartbeatLoopOuterWhileLoopReconnectState,
}
...
/// Boundary that wires completed body output into one future outer while-loop turn.
///
/// This boundary keeps the outer while-loop dumb: it delegates completed body,
/// wakeup planning, wakeup execution, and wakeup actual side effect to the
/// existing boundaries, then returns explicit continue actions or explicit stop
/// terminal output. It does not execute timer wait, retry work, reconnects,
/// metrics cadence, or reinterpret cleanup semantics.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct ClientHeartbeatLoopOuterWhileLoopConnectionBoundary {
    completed_body: ClientHeartbeatLoopCompletedContinuousBodyBoundary,
    wakeup_planning: ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupBoundary,
    wakeup_execution: ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupExecutionBoundary,
    wakeup_actual_side_effect:
        ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupActualSideEffectBoundary,
}

impl ClientHeartbeatLoopOuterWhileLoopConnectionBoundary {
    pub fn plan_next(
        &self,
        repeated: ClientHeartbeatLoopRepeatedInvocationResult,
    ) -> ClientHeartbeatLoopOuterWhileLoopConnectionResult {
        let body = self.completed_body.plan_next(repeated);
        let wakeup = self.wakeup_planning.plan_next(body);
        let wakeup_execution = self.wakeup_execution.apply(wakeup);
        let wakeup_actual_side_effect = self.wakeup_actual_side_effect.apply(wakeup_execution);

        match wakeup_actual_side_effect {
            ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupActualSideEffectResult::ContinueWithoutWakeupSideEffect {
                output,
            } => ClientHeartbeatLoopOuterWhileLoopConnectionResult::Continue {
                output: ClientHeartbeatLoopOuterWhileLoopConnectionOutput {
                    carry: output.carry,
                    wakeup: ClientHeartbeatLoopOuterWhileLoopWakeupState::NoWakeupSideEffect,
                    timer_wait: output.timer_wait,
                    retry_execution: output.retry_execution,
                    reconnect_execution: output.reconnect_execution,
                },
            },
            ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupActualSideEffectResult::ContinueWithWakeupSideEffectApplied {
                output,
                ...
                output: ClientHeartbeatLoopOuterWhileLoopConnectionOutput {
                    ...
                    wakeup: ClientHeartbeatLoopOuterWhileLoopWakeupState::WakeupSideEffectApplied {
                        wakeup_apply: output.wakeup_apply,
                        wakeup_side_effect: output.wakeup_side_effect,
                    },
                    timer_wait: output.output.timer_wait,
                    retry_execution: output.output.retry_execution,
                    reconnect_execution: output.output.reconnect_execution,
                },
            },
            ClientHeartbeatLoopHeartbeatTimeoutNoticeWakeupActualSideEffectResult::Stop {
                output,
        ...
        match ClientHeartbeatLoopOuterWhileLoopActualReconnectExecutionInput::from_one_turn_execution(
            ...
            Ok(input) => {
                let reconnect_execution = match input.output.reconnect_execution {
                    ...
                    ClientHeartbeatLoopFutureActualReconnectExecutionAction::ReconnectExecution {
                        reason,
                    } => {
...
/// Boundary that keeps reconnect policy separate from timer wait and retry execution.
///
/// This boundary consumes actual execution result only. It reads reconnect
/// execution state, preserves wakeup / timer wait / retry execution unchanged,
/// and decides whether socket re-establishment should be planned next.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct ClientHeartbeatLoopOuterWhileLoopReconnectPolicyBoundary;

impl ClientHeartbeatLoopOuterWhileLoopReconnectPolicyBoundary {
    pub fn plan_next(
        &self,
        execution: ClientHeartbeatLoopOuterWhileLoopActualExecutionResult,
    ) -> ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult {
        match ClientHeartbeatLoopOuterWhileLoopReconnectPolicyInput::from_actual_execution(
            execution,
        ) {
            Ok(input) => match input.output.reconnect_execution {
                ClientHeartbeatLoopOuterWhileLoopActualReconnectExecutionApplyResult::NoReconnectExecutionApplied => {
                    ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult::ContinueWithoutReconnect {
                        output: input.output,
                    }
                }
                ClientHeartbeatLoopOuterWhileLoopActualReconnectExecutionApplyResult::ReconnectExecutionApplied {
            ...
            Err(output) => ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult::Stop { output },
        }
    }
}
...
impl ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentBoundary {
    pub fn apply(
        &self,
        policy: ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult,
    ...
    pub fn apply_with_hook<H: ClientHeartbeatLoopSocketReestablishmentHook>(
        ...
        match ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentInput::from_reconnect_policy(
            ...
            Ok(input) => {
                let reason = match input.handoff.reconnect_plan {
                    ClientHeartbeatLoopFutureSocketReestablishmentPlan::ReestablishSocket {
                        reason,
                    } => reason,
                };
            ...
            ) => ClientHeartbeatLoopOuterWhileLoopReconnectResult::ContinueWithoutReconnect {
                output,
            },
            Err(ClientHeartbeatLoopOuterWhileLoopReconnectPolicyResult::Stop { output }) => {
            ...
            Err(
                ...
                "continue-with-reconnect policy should already be converted into socket re-establishment input"
            ),
        }
    }
}
...
pub struct ClientHeartbeatLoopOuterWhileLoopReconnectBoundary {
    ...
    socket_reestablishment: ClientHeartbeatLoopOuterWhileLoopSocketReestablishmentBoundary,
}

impl ClientHeartbeatLoopOuterWhileLoopReconnectBoundary {
    pub fn apply(
        &self,
        execution: ClientHeartbeatLoopOuterWhileLoopActualExecutionResult,
    ) -> ClientHeartbeatLoopOuterWhileLoopReconnectResult {
...
/// connection, one-turn execution, actual timer/retry/reconnect execution, and
/// reconnect policy/socket re-establishment connection. It does not reinterpret
/// stop output, add metrics cadence, or implement a broad reconnect policy. A
/// caller-owned max-turn guard keeps tests deterministic while preserving
/// explicit continue state.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct ClientHeartbeatLoopOuterWhileLoopRepeatedBodyBoundary {
    ...
    reconnect: ClientHeartbeatLoopOuterWhileLoopReconnectBoundary,
}
            ...
            match reconnect {
                ...
                ClientHeartbeatLoopOuterWhileLoopReconnectResult::ContinueWithoutReconnect {
                    ...
                    let state = ClientHeartbeatLoopOuterWhileLoopRepeatedBodyContinuationState {
                        ...
                        last_reconnect: ClientHeartbeatLoopOuterWhileLoopReconnectState::NoReconnectNeeded,
                    };
                ...
                ClientHeartbeatLoopOuterWhileLoopReconnectResult::ContinueWithReconnectDeferred {
                    ...
                    let state = ClientHeartbeatLoopOuterWhileLoopRepeatedBodyContinuationState {
                        ...
                        last_reconnect: ClientHeartbeatLoopOuterWhileLoopReconnectState::ReconnectDeferred {
...
/// Error from the manual real encoded `VideoFrame` PoC launcher.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ClientRealEncodedVideoFramePocError {
    Io { path: PathBuf, message: String },
    Config(ClientAuthRequestPocConfigError),
    Destination(ClientAuthRequestPocConfigError),
    Bind(io::ErrorKind),
...
impl ClientRealEncodedVideoFramePocError {
    fn from_auth_request_error(error: ClientAuthRequestPocError) -> Self {
        match error {
            ClientAuthRequestPocError::Io { path, message } => Self::Io { path, message },
            ClientAuthRequestPocError::Config(error) => Self::Config(error),
            ClientAuthRequestPocError::Destination(error) => Self::Destination(error),
            ClientAuthRequestPocError::Bind(error)
            | ClientAuthRequestPocError::SetReadTimeout(error)
            | ClientAuthRequestPocError::Send(error)
            | ClientAuthRequestPocError::Receive(error) => Self::Bind(error),
            ClientAuthRequestPocError::Encode(error) | ClientAuthRequestPocError::Decode(error) => {
                Self::Config(ClientAuthRequestPocConfigError::InvalidDestination {
                    value: "protocol encode/decode error while loading real video PoC config"
                        .to_string(),
            ...
            ClientAuthRequestPocError::UnexpectedResponseMessage { actual } => {
                Self::Config(ClientAuthRequestPocConfigError::InvalidDestination {
                    value: "unexpected response while loading real video PoC config".to_string(),
...
mod tests {
    ...
    #[test]
    fn client_video_frame_real_encoded_poc_launcher_loads_config_with_defaults() {
        ...
        assert!(config.start_capture_if_needed);
        assert_eq!(
    ...
    #[test]
    fn client_video_frame_real_encoded_poc_launcher_surfaces_capture_unavailable() {
        ...
        let startup_config = ClientRealEncodedVideoFramePocLauncher::default()
            .load_startup_config_from_str(include_str!(
                "../../../configs/examples/client.accepted.example.toml"
            ))
            .expect("accepted example config should load");
        let outcome = ClientRealEncodedVideoFramePocLauncher::default()
    ...
    #[test]
    fn client_video_frame_real_encoded_poc_launcher_surfaces_encode_failure() {
        struct FixtureSession;
        impl ClientCaptureSessionRuntimeHook for FixtureSession {
            fn create_windows_graphics_capture_session(
                &self,
    ...
    #[test]
    fn client_video_frame_real_encoded_poc_launcher_surfaces_send_failure() {
        struct FixtureSession;
        impl ClientCaptureSessionRuntimeHook for FixtureSession {
            fn create_windows_graphics_capture_session(
                &self,
    ...
    fn cleanup_test_timer_sleep() -> ClientHeartbeatLoopSleepDecision {
        ClientHeartbeatLoopSleepDecision::Sleep {
            reason: ClientHeartbeatLoopSleepReason::CadenceWait,
            sleep_micros: 1_000,
            wake_at: TimestampMicros(12_000),
    ...
    fn cleanup_test_iteration_carry(
        ordering: ClientHeartbeatLoopStepOrdering,
        retry_attempts_used: u32,
        ...
        ClientHeartbeatLoopIterationCarryState {
            ...
            next_runtime_input: ClientHeartbeatLoopCompletedStepRuntimeInput {
                continue_requested: true,
                body: ClientHeartbeatLoopRepeatedRuntimeBodyInput {
                    handoff: cleanup_test_runtime_handoff(),
                    now: TimestampMicros(11_000),
                    stop_requested: false,
                    retry_attempts_used,
                },
    ...
    fn cleanup_test_retry_apply() -> ClientHeartbeatLoopRetryApplyResult {
        ClientHeartbeatLoopRetryApplyResult {
            failure_result: ClientHeartbeatLoopIterationRuntimeResult::Failed {
                kind: ClientHeartbeatLoopIterationFailureKind::AckReceive,
                failed_at: TimestampMicros(11_000),
    ...
    fn cleanup_test_repeated_apply_timer_continue() -> ClientHeartbeatLoopRepeatedInvocationResult {
        let sleep = cleanup_test_timer_sleep();
    ...
    fn cleanup_test_repeated_apply_retry_continue() -> ClientHeartbeatLoopRepeatedInvocationResult {
    ...
    fn cleanup_test_repeated_stop() -> ClientHeartbeatLoopRepeatedInvocationResult {
        ClientHeartbeatLoopRepeatedInvocationResult::Stop {
            reason: ClientHeartbeatLoopRepeatedInvocationStopReason::CleanupRequested {
                stop_reason: ClientHeartbeatLoopLifecycleStopReason::CallerRequestedStop,
            },
    ...
    fn cleanup_test_outer_one_turn_continue_with_reconnect_needed(
    ) -> ClientHeartbeatLoopOuterWhileLoopOneTurnExecutionResult {
    ...
    fn cleanup_test_outer_actual_execution_continue_no_reconnect(
    ) -> ClientHeartbeatLoopOuterWhileLoopActualExecutionResult {
        ClientHeartbeatLoopOuterWhileLoopActualExecutionBoundary::default()
            .apply(cleanup_test_outer_one_turn_apply_timer_continue())
    }

    fn cleanup_test_outer_actual_execution_continue_with_reconnect_needed(
    ) -> ClientHeartbeatLoopOuterWhileLoopActualExecutionResult {
        ClientHeartbeatLoopOuterWhileLoopActualExecutionBoundary::default()
    ...
    fn client_heartbeat_loop_cleanup_outer_while_loop_connection_preserves_continue_path_and_wakeup_timer_retry_reconnect_separation(
        ...
        let result = ClientHeartbeatLoopOuterWhileLoopConnectionBoundary::default()
            .plan_next(cleanup_test_repeated_apply_timer_continue());

        let ClientHeartbeatLoopOuterWhileLoopConnectionResult::Continue { output } = result else {
            panic!("timer-wait continue path should remain a continue output");
        };
    ...
    #[test]
    fn client_heartbeat_loop_cleanup_outer_while_loop_connection_preserves_retry_without_collapsing_into_wakeup(
    ) {
        let retry = cleanup_test_retry_apply();
        let result = ClientHeartbeatLoopOuterWhileLoopConnectionBoundary::default()
            .plan_next(cleanup_test_repeated_apply_retry_continue());
        ...
        let ClientHeartbeatLoopOuterWhileLoopConnectionResult::Continue { output } = result else {
            panic!("retry continue path should remain a continue output");
        };
    ...
    #[test]
    fn client_heartbeat_loop_cleanup_outer_while_loop_connection_preserves_stop_passthrough() {
        let result = ClientHeartbeatLoopOuterWhileLoopConnectionBoundary::default()
            .plan_next(cleanup_test_repeated_stop());

        assert_eq!(
            result,
            ClientHeartbeatLoopOuterWhileLoopConnectionResult::Stop {
                output: ClientHeartbeatLoopCompletedBodyTerminalOutput {
                    stop_reason:
                        ClientHeartbeatLoopRepeatedInvocationStopReason::CleanupRequested {
                            stop_reason:
    ...
    #[test]
    fn client_heartbeat_loop_cleanup_outer_while_loop_connection_does_not_reinterpret_cleanup_or_stop_semantics(
    ) {
        let repeated = cleanup_test_repeated_stop();
        let completed_body = ClientHeartbeatLoopCompletedContinuousBodyBoundary::default()
            .plan_next(repeated.clone());
        let connected =
            ClientHeartbeatLoopOuterWhileLoopConnectionBoundary::default().plan_next(repeated);

        let ClientHeartbeatLoopCompletedContinuousBodyResult::Stop {
            output: completed_stop,
        } = completed_body
        else {
            panic!("stop repeated result should remain stop through completed body");
        };
        let ClientHeartbeatLoopOuterWhileLoopConnectionResult::Stop { output: outer_stop } =
            connected
        else {
            panic!("stop repeated result should remain stop through outer while-loop connection");
        };
    ...
    fn client_heartbeat_loop_cleanup_outer_while_loop_reconnect_policy_no_reconnect_path_remains_explicit(
        ...
        let ClientHeartbeatLoopOuterWhileLoopReconnectResult::ContinueWithoutReconnect { output } =
            ...
            panic!("no-reconnect path should remain explicit");
        };
    ...
    fn client_heartbeat_loop_cleanup_outer_while_loop_actual_socket_reestablishment_preserves_stop_passthrough(
        ...
        assert_eq!(
            result,
            ClientHeartbeatLoopOuterWhileLoopReconnectResult::Stop {
                output: ClientHeartbeatLoopCompletedBodyTerminalOutput {
                    stop_reason:
                        ClientHeartbeatLoopRepeatedInvocationStopReason::CleanupRequested {
                            stop_reason:
    ...
    fn client_heartbeat_loop_cleanup_outer_while_loop_actual_socket_reestablishment_result_can_be_carried_without_losing_separation(
        ...
        let ClientHeartbeatLoopOuterWhileLoopReconnectResult::ContinueWithReconnectDeferred {
            ...
            panic!(
                "reconnect-needed execution should remain explicit for the next integration point"
            );
        };
        ...
        assert_eq!(
            ...
            ClientHeartbeatLoopOuterWhileLoopReconnectState::ReconnectDeferred {
                reconnect_plan:
                    ClientHeartbeatLoopFutureSocketReestablishmentPlan::ReestablishSocket {
                        reason: ClientHeartbeatLoopReconnectReason::SocketReestablishmentRequested,
                    },
