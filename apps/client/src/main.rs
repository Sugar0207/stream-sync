            ...
            while let Some(flag) = args.next() {
                match flag.as_str() {
                    ...
                    "--cadence-mode" => {
                        let Some(value) = args.next() else {
                            eprintln!(
                                "missing cadence mode value for bounded auth real encoded video PoC"
                            );
                            std::process::exit(1);
                        };
                        cadence_mode = stream_sync_client::ClientContinuousRealEncodedVideoFrameCadenceMode::parse_config_str(
                            &value,
                        )
                        .unwrap_or_else(|| {
                            eprintln!(
                                "invalid cadence mode for bounded auth real encoded video PoC: {value}"
                            );
                            std::process::exit(1);
                        });
                    }
                    ...
                    stream_sync_client::ClientContinuousRealEncodedVideoFramePocOutcome::Completed(runtime) => {
                        ...
                        let avg_encode_elapsed_ms = format_average_duration_ms(
                            ...
                            summary
                                ...
                                .saturating_add(summary.persistent_no_complete_access_unit_count)
                                .saturating_add(summary.h264_parameter_sets_missing_count),
                        );
                        ...
                        let deadline_sleep_ms =
                            format_duration_ms(summary.deadline_sleep_micros);
                        let deadline_overrun_ms =
                            format_duration_ms(summary.deadline_overrun_micros);
                        let max_deadline_overrun_ms =
                            format_duration_ms(summary.max_deadline_overrun_micros);
                        ...
                        println!(
                            ...
                            summary.h264_parameter_sets_cached,
                            summary.h264_sps_count,
                            summary.h264_pps_count,
                            summary.h264_idr_count,
                            summary.h264_non_idr_vcl_count,
                            summary.h264_parameter_sets_prepended_count,
                            summary.last_payload_had_parameter_sets,
                            summary.h264_parameter_sets_missing_count,
                            summary.keyframes_encoded,
                            summary.keyframes_sent,
                            summary
                            ...
                            summary
                                ...
                                .map(|value| value.to_string())
                                .unwrap_or_else(|| "none".to_string()),
                            summary.last_payload_has_sps,
                            summary.last_payload_has_pps,
                            summary.last_payload_has_idr,
                            summary.last_payload_has_non_idr_vcl,
                            last_encoder_exit_status,
