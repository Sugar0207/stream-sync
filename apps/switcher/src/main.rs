...
use stream_sync_switcher::{
    ...
    SwitcherWindowRenderBoundary, SwitcherWindowRenderRequest, SwitcherWindowRenderResult,
    SwitcherWindowRenderRuntimeHook, SWITCHER_FOUR_VIEW_CLEAN_OUTPUT_WINDOW_TITLE,
...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some("--four-view-two-real-handoff-preview-loop") => {
            ...
            let frames = parse_positive_u32_arg_or_exit(args.next(), "frames");
            let two_real_options =
                parse_two_real_handoff_preview_options_or_exit(args.collect::<Vec<_>>());
            match run_four_view_two_real_handoff_preview_loop(
                ...
                two_real_options.continuous_decoder_output_pipeline_experiment_mode,
                ...
                two_real_options.operator_preview_snapshot_retention_enabled,
            ) {
                Ok(summary) => println!(
                    "{}",
...
impl TwoRealContinuousOutputPipelineExperimentMode {
    fn parse_or_exit(value: &str) -> Self {
        match value {
    ...
    fn ffmpeg_output_pixel_format(self) -> &'static str {
        match self {
            Self::Default => "bgra",
    ...
    fn output_frame_bytes(self, width: u32, height: u32) -> usize {
        width as usize * height as usize * self.output_bytes_per_pixel()
    }

    fn pipe_bytes_saved_per_frame(self, width: u32, height: u32) -> usize {
        let default_bytes = Self::Default.output_frame_bytes(width, height);
        default_bytes.saturating_sub(self.output_frame_bytes(width, height))
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealHandoffPreviewLoopOptions {
    ...
    continuous_decoder_output_pipeline_experiment_mode:
        TwoRealContinuousOutputPipelineExperimentMode,
    ...
    operator_preview_snapshot_retention_enabled: bool,
}
...
fn parse_two_real_handoff_preview_options_or_exit(
    values: Vec<String>,
) -> TwoRealHandoffPreviewLoopOptions {
    let mut read_mode = SwitcherSingleClientQueueSourceMode::PreviewLatest;
    ...
    let mut continuous_decoder_output_pipeline_experiment_mode =
    ...
    let mut operator_preview_snapshot_retention_enabled = false;
    ...
    while let Some(value) = values.next() {
        ...
        if value == "--enable-continuous-stream-decoder" {
        ...
        if value == "--continuous-decoder-output-pipeline-experiment" {
            let mode = values.next().unwrap_or_else(|| {
                eprintln!("missing continuous-decoder-output-pipeline-experiment");
                std::process::exit(1);
            });
            continuous_decoder_output_pipeline_experiment_mode =
                TwoRealContinuousOutputPipelineExperimentMode::parse_or_exit(&mode);
            continue;
        }
        if value == "--enable-program-output-window" {
            program_output_window_enabled = true;
            continue;
        }
        if value == "--program-selected-client-id" {
            let client_id = values.next().unwrap_or_else(|| {
                eprintln!("missing program-selected-client-id");
                std::process::exit(1);
            });
            program_selected_client_id = Some(ClientId(client_id));
            continue;
        }
        if value == "--enable-program-continuous-decode" {
        ...
        if value == "--program-first-validation-mode" {
            program_first_validation_mode = true;
            continue;
        }
        if value == "--program-first-preview-refresh-interval" {
            let interval_ticks = values.next().unwrap_or_else(|| {
                eprintln!("missing program-first-preview-refresh-interval");
                std::process::exit(1);
            });
            let interval_ticks = interval_ticks.parse::<u32>().unwrap_or_else(|error| {
                eprintln!("invalid program-first-preview-refresh-interval: {error}");
                std::process::exit(1);
            });
            program_first_preview_refresh_interval =
                Some(NonZeroU32::new(interval_ticks).unwrap_or_else(|| {
                    eprintln!("invalid program-first-preview-refresh-interval: expected > 0");
                    std::process::exit(1);
                }));
            continue;
        }
        ...
        if value == "--operator-preview-snapshot-retention" {
...
struct SwitcherFourViewTwoRealHandoffPreviewLoopSummary {
    ...
    persistent_decode_config_enabled: bool,
    ...
    persistent_decode_skipped_by_config_count: u32,
    ...
    continuous_decode_config_enabled: bool,
    continuous_decode_runtime_enabled: bool,
    continuous_decode_slot0_enabled: bool,
    continuous_decode_input_frame_count: u32,
    continuous_decode_output_frame_count: u32,
    continuous_decode_queue_len: usize,
    continuous_decode_dropped_stale_count: u32,
    continuous_decode_frame_id_lag: u64,
    continuous_decode_stdout_read_elapsed_ms: u128,
    continuous_decode_stall_count: u32,
    continuous_decode_restart_count: u32,
    continuous_decode_runtime_disabled: bool,
    continuous_decode_runtime_disabled_reason: String,
    continuous_decode_fallback_to_one_shot_count: u32,
    continuous_decode_lookup_hit_count: u32,
    continuous_decode_lookup_miss_count: u32,
    continuous_decode_lookup_miss_reason_counts: String,
    continuous_decode_requested_frame_id: Option<u64>,
    continuous_decode_latest_decoded_frame_id: Option<u64>,
    continuous_decode_requested_minus_latest_lag: Option<u64>,
    continuous_decode_queue_oldest_frame_id: Option<u64>,
    continuous_decode_queue_newest_frame_id: Option<u64>,
    continuous_decode_input_frame_id_min: Option<u64>,
    continuous_decode_input_frame_id_max: Option<u64>,
    continuous_decode_output_frame_id_min: Option<u64>,
    continuous_decode_output_frame_id_max: Option<u64>,
    continuous_decode_output_pending_correspondence_count: usize,
    continuous_decode_pending_correspondence_count: usize,
    ...
    continuous_decode_input_frame_id_gap_max: u64,
    continuous_decode_input_frame_id_gap_total: u64,
    continuous_decode_input_non_consecutive_count: u32,
    continuous_decode_input_keyframe_count: u32,
    continuous_decode_input_non_keyframe_count: u32,
    continuous_decode_input_has_sps_count: u32,
    continuous_decode_input_has_pps_count: u32,
    continuous_decode_input_has_idr_count: u32,
    continuous_decode_input_has_non_idr_vcl_count: u32,
    continuous_decode_last_input_payload_nal_kinds: String,
    continuous_decode_ffmpeg_stderr_summary: String,
    continuous_decode_stdout_reader_blocked_count: u32,
    continuous_decode_no_output_after_input_count: u32,
    continuous_decode_no_output_after_keyframe_count: u32,
    continuous_decode_bootstrap_input_count: u32,
    continuous_decode_bootstrap_output_count: u32,
    ...
    continuous_decode_last_stdin_write_error: String,
    continuous_decode_process_running: bool,
    ...
    continuous_decode_last_input_write_elapsed_ms: u128,
    ...
    continuous_decode_latest_input_to_output_frame_gap: Option<u64>,
    ...
    continuous_decode_latest_selected_to_output_frame_gap: Option<u64>,
    continuous_decode_output_throughput_fps: String,
    continuous_decode_reader_full_frame_elapsed_ms_max: u128,
    continuous_decode_reader_full_frame_elapsed_ms_avg: String,
    continuous_decode_reader_full_frame_slow_count: u32,
    continuous_decode_reader_full_frame_slow_threshold_ms: u128,
    continuous_decode_output_bytes_total: usize,
    continuous_decode_output_bytes_per_sec: String,
    continuous_decode_output_frame_interval_ms_avg: String,
    continuous_decode_output_frame_interval_ms_max: u128,
    continuous_decode_stdout_read_throughput_bytes_per_ms: String,
    ...
    continuous_decode_output_pixel_convert_elapsed_ms_max: u128,
    continuous_decode_output_pixel_convert_count: u32,
    continuous_decode_output_pixel_convert_buffer_reuse_count: u32,
    continuous_decode_output_pixel_convert_buffer_allocation_count: u32,
    continuous_decode_output_pixel_convert_bytes_written_total: usize,
    continuous_decode_output_pixel_convert_bytes_written_per_frame: usize,
    continuous_decode_output_pixel_convert_mode: String,
    continuous_decode_competing_one_shot_decode_elapsed_ms: u128,
    continuous_decode_competing_one_shot_attempt_count: u32,
    continuous_decode_slot0_one_shot_suppression_enabled: bool,
    continuous_decode_slot0_one_shot_suppressed_count: u32,
    continuous_decode_slot0_one_shot_suppressed_reason_counts: String,
    continuous_decode_slot0_one_shot_suppressed_render_safety_counts: String,
    continuous_decode_slot0_one_shot_suppressed_continuous_not_ready_count: u32,
    continuous_decode_slot0_one_shot_suppressed_stale_count: u32,
    ...
    continuous_feed_frame_received_count: u32,
    continuous_feed_no_frame_count: u32,
    continuous_feed_handoff_error_count: u32,
    continuous_feed_enqueued_count: u32,
    continuous_feed_skipped_count: u32,
    continuous_feed_skip_reason_counts: String,
    continuous_feed_dropped_stale_input_count: u32,
    continuous_feed_latest_received_frame_id: Option<u64>,
    continuous_feed_latest_enqueued_frame_id: Option<u64>,
    continuous_decode_input_from_feeder_count: u32,
    continuous_decode_input_from_render_demand_count: u32,
    continuous_decode_feeder_lag_to_selected: Option<u64>,
    ...
    continuous_decode_output_availability_not_ready_count: u32,
    continuous_decode_output_availability_stale_count: u32,
    continuous_decode_output_availability_future_count: u32,
    ...
    one_shot_decode_attempt_count: u32,
    one_shot_decode_elapsed_ms: u128,
    one_shot_decode_elapsed_ms_max: u128,
    one_shot_decode_input_write_elapsed_ms: u128,
    ...
    one_shot_decode_slow_first_byte_slot_counts: String,
    one_shot_decode_slow_first_byte_source_counts: String,
    one_shot_decode_slow_first_byte_reason_counts: String,
    one_shot_decode_slow_output_read_slot_counts: String,
    one_shot_decode_slow_output_read_source_counts: String,
    one_shot_decode_slow_output_read_reason_counts: String,
    one_shot_decode_input_write_outlier_count: u32,
    one_shot_decode_input_write_outlier_threshold_ms: u128,
    one_shot_decode_input_write_outlier_slot_counts: String,
    one_shot_decode_input_write_outlier_source_counts: String,
    one_shot_decode_input_write_outlier_reason_counts: String,
    one_shot_decode_input_write_outlier_payload_bytes_total: usize,
    one_shot_decode_input_write_outlier_payload_bytes_avg: String,
    ...
    one_shot_decode_keyframe_attempt_count: u32,
    one_shot_decode_non_keyframe_attempt_count: u32,
    one_shot_decode_keyframe_elapsed_ms: u128,
    one_shot_decode_non_keyframe_elapsed_ms: u128,
    one_shot_decode_keyframe_input_payload_bytes_total: usize,
    one_shot_decode_non_keyframe_input_payload_bytes_total: usize,
    one_shot_decode_expected_output_bytes_per_frame: String,
    one_shot_decode_output_width: Option<u32>,
    one_shot_decode_output_height: Option<u32>,
    one_shot_decode_output_pixel_format: String,
    one_shot_decode_scaled_output_enabled: bool,
    one_shot_decode_scaled_output_reason: String,
    one_shot_decode_write_throughput_bytes_per_ms: String,
    one_shot_decode_input_write_elapsed_per_payload_kb: String,
    one_shot_decode_read_throughput_bytes_per_ms: String,
    ...
    output_width: Option<u32>,
    output_height: Option<u32>,
    program_output_enabled: bool,
    program_output_selection_mode: &'static str,
    program_output_requested_client_id: Option<ClientId>,
    program_output_render_count: u32,
    program_output_missing_selected_source_count: u32,
    program_output_missing_before_first_render_count: u32,
    program_output_missing_after_first_render_count: u32,
    program_output_reused_previous_frame_count: u32,
    program_output_placeholder_render_count: u32,
    program_output_black_frame_render_count: u32,
    program_output_first_render_attempt_index: Option<u32>,
    program_output_first_render_elapsed_ms: Option<u128>,
    program_output_missing_selected_source_reason: Option<&'static str>,
    program_output_last_result_kind: &'static str,
    program_output_selected_client_id: Option<ClientId>,
    program_output_selected_slot_index: Option<usize>,
    ...
    program_first_validation_enabled: bool,
    program_first_suppressed_preview_one_shot_decode_count: u32,
    program_first_suppressed_preview_one_shot_decode_slot_counts: String,
    program_first_program_only_decode_path_enabled: bool,
    program_first_remaining_one_shot_decode_count: u32,
    program_first_preview_visible: bool,
    program_first_preview_refresh_interval: Option<u32>,
    program_first_preview_refresh_count: u32,
    program_first_preview_suppressed_count: u32,
    operator_low_cost_preview_enabled: bool,
    operator_preview_refresh_interval_ticks: Option<u32>,
    operator_preview_refresh_attempt_count: u32,
    operator_preview_refresh_success_count: u32,
    operator_preview_refresh_skipped_count: u32,
    ...
    operator_preview_reused_program_frame_count: u32,
    operator_preview_program_slot_visible_count: u32,
    operator_preview_program_slot_reuse_source: &'static str,
    operator_preview_program_slot_black_count: u32,
    operator_preview_snapshot_retention_enabled: bool,
    operator_preview_snapshot_reuse_count: u32,
    operator_preview_snapshot_reuse_slot_counts: String,
    operator_preview_snapshot_created_count: u32,
    operator_preview_snapshot_created_slot_counts: String,
    operator_preview_placeholder_avoided_by_snapshot_count: u32,
    operator_preview_slot_black_after_snapshot_count: u32,
    ...
    program_window_render_success_count: u32,
    program_window_render_failure_count: u32,
    program_render_effective_fps: String,
    effective_program_render_fps: String,
}
...
struct TwoRealPreviewLoopRuntimeTiming {
    ...
    continuous_decode_restart_count: u32,
    continuous_decode_runtime_disabled: bool,
    continuous_decode_runtime_disabled_reason: Option<String>,
    continuous_decode_fallback_to_one_shot_count: u32,
    continuous_decode_lookup_hit_count: u32,
    continuous_decode_lookup_miss_count: u32,
    continuous_decode_lookup_miss_reason_exact_key_missing_count: u32,
    continuous_decode_lookup_miss_reason_queue_empty_count: u32,
    continuous_decode_lookup_miss_reason_runtime_disabled_count: u32,
    continuous_decode_lookup_miss_reason_output_pending_count: u32,
    continuous_decode_lookup_miss_reason_frame_id_lagging_count: u32,
    continuous_decode_lookup_miss_reason_unknown_count: u32,
    continuous_decode_requested_frame_id: Option<u64>,
    continuous_decode_latest_decoded_frame_id: Option<u64>,
    continuous_decode_requested_minus_latest_lag: Option<u64>,
    continuous_decode_queue_oldest_frame_id: Option<u64>,
    ...
    continuous_decode_output_frame_id_max: Option<u64>,
    continuous_decode_output_pending_correspondence_count: usize,
    continuous_decode_pending_correspondence_age_ms_total: u128,
    continuous_decode_pending_correspondence_age_sample_count: u32,
    continuous_decode_pending_correspondence_age_ms_max: Option<u128>,
    continuous_decode_pending_correspondence_oldest_frame_id: Option<u64>,
    continuous_decode_pending_correspondence_newest_frame_id: Option<u64>,
    continuous_decode_completed_correspondence_count: u32,
    ...
    continuous_decode_input_has_idr_count: u32,
    continuous_decode_input_has_non_idr_vcl_count: u32,
    continuous_decode_last_input_payload_nal_kinds: Option<String>,
    continuous_decode_ffmpeg_stderr_summary: Option<String>,
    continuous_decode_stdout_reader_blocked_count: u32,
    continuous_decode_no_output_after_input_count: u32,
    continuous_decode_no_output_after_keyframe_count: u32,
    continuous_decode_bootstrap_input_count: u32,
    ...
    continuous_decode_stdin_write_bytes_total: usize,
    continuous_decode_stdin_write_error_count: usize,
    continuous_decode_last_stdin_write_error: Option<String>,
    continuous_decode_process_running: bool,
    ...
    continuous_decode_reader_full_frame_slow_count: u32,
    continuous_decode_output_bytes_total: usize,
    continuous_decode_output_frame_interval_elapsed_ms: u128,
    continuous_decode_output_frame_interval_count: u32,
    continuous_decode_output_frame_interval_ms_max: u128,
    continuous_decode_ffmpeg_scale_enabled: bool,
    ...
    continuous_decode_output_pixel_convert_bytes_written_total: usize,
    continuous_decode_output_pixel_convert_bytes_written_per_frame: usize,
    continuous_decode_output_pixel_convert_mode: Option<String>,
    continuous_decode_competing_one_shot_decode_elapsed_ms: u128,
    continuous_decode_competing_one_shot_attempt_count: u32,
    continuous_decode_slot0_one_shot_suppression_enabled: bool,
    continuous_decode_slot0_one_shot_suppressed_count: u32,
    continuous_decode_slot0_one_shot_suppressed_reason_not_ready_count: u32,
    continuous_decode_slot0_one_shot_suppressed_reason_stale_count: u32,
    continuous_decode_slot0_one_shot_suppressed_reason_future_count: u32,
    continuous_decode_slot0_one_shot_suppressed_reason_unknown_count: u32,
    continuous_decode_slot0_one_shot_suppressed_render_safety_decode_deferred_placeholder_count:
        u32,
    continuous_decode_slot0_one_shot_suppressed_render_safety_unknown_count: u32,
    ...
    continuous_feed_enqueued_count: u32,
    continuous_feed_skipped_count: u32,
    continuous_feed_skip_reason_duplicate_count: u32,
    continuous_feed_skip_reason_future_frame_count: u32,
    continuous_feed_skip_reason_runtime_disabled_count: u32,
    continuous_feed_skip_reason_startup_not_ready_count: u32,
    continuous_feed_skip_reason_input_queue_full_count: u32,
    continuous_feed_skip_reason_source_mismatch_count: u32,
    continuous_feed_skip_reason_consume_mismatch_count: u32,
    continuous_feed_skip_reason_unknown_count: u32,
    continuous_feed_dropped_stale_input_count: u32,
    continuous_feed_latest_received_frame_id: Option<u64>,
    continuous_feed_latest_enqueued_frame_id: Option<u64>,
    continuous_decode_input_from_feeder_count: u32,
    ...
    one_shot_decode_slow_first_byte_slot0_count: u32,
    one_shot_decode_slow_first_byte_slot1_count: u32,
    one_shot_decode_slow_first_byte_source_queue_count: u32,
    one_shot_decode_slow_first_byte_source_retained_keyframe_count: u32,
    one_shot_decode_slow_first_byte_source_unknown_count: u32,
    one_shot_decode_slow_first_byte_reason_frame_id_changed_count: u32,
    one_shot_decode_slow_first_byte_reason_cache_miss_count: u32,
    one_shot_decode_slow_first_byte_reason_previous_unavailable_count: u32,
    one_shot_decode_slow_first_byte_reason_source_recovered_count: u32,
    one_shot_decode_slow_first_byte_reason_unknown_count: u32,
    one_shot_decode_slow_output_read_slot0_count: u32,
    one_shot_decode_slow_output_read_slot1_count: u32,
    one_shot_decode_slow_output_read_source_queue_count: u32,
    one_shot_decode_slow_output_read_source_retained_keyframe_count: u32,
    one_shot_decode_slow_output_read_source_unknown_count: u32,
    one_shot_decode_slow_output_read_reason_frame_id_changed_count: u32,
    one_shot_decode_slow_output_read_reason_cache_miss_count: u32,
    one_shot_decode_slow_output_read_reason_previous_unavailable_count: u32,
    one_shot_decode_slow_output_read_reason_source_recovered_count: u32,
    one_shot_decode_slow_output_read_reason_unknown_count: u32,
    one_shot_decode_input_write_outlier_count: u32,
    one_shot_decode_input_write_outlier_slot0_count: u32,
    one_shot_decode_input_write_outlier_slot1_count: u32,
    one_shot_decode_input_write_outlier_source_queue_count: u32,
    one_shot_decode_input_write_outlier_source_retained_keyframe_count: u32,
    one_shot_decode_input_write_outlier_source_unknown_count: u32,
    one_shot_decode_input_write_outlier_reason_frame_id_changed_count: u32,
    one_shot_decode_input_write_outlier_reason_cache_miss_count: u32,
    one_shot_decode_input_write_outlier_reason_previous_unavailable_count: u32,
    one_shot_decode_input_write_outlier_reason_source_recovered_count: u32,
    one_shot_decode_input_write_outlier_reason_unknown_count: u32,
    one_shot_decode_input_write_outlier_payload_bytes_total: usize,
    decode_cache_miss_slot0_count: u32,
    decode_cache_miss_slot1_count: u32,
    skipped_decode_unchanged_slot0_count: u32,
    ...
    one_shot_decode_keyframe_input_payload_bytes_total: usize,
    one_shot_decode_non_keyframe_input_payload_bytes_total: usize,
    one_shot_decode_expected_output_bytes_total: usize,
    one_shot_decode_output_width: Option<u32>,
    one_shot_decode_output_height: Option<u32>,
    one_shot_decode_output_pixel_format: Option<SwitcherDecodedFramePixelFormat>,
    one_shot_decode_scaled_output_enabled: bool,
    one_shot_decode_scaled_output_reason: Option<String>,
    program_first_suppressed_preview_one_shot_decode_count: u32,
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealPreviewLoopDecodeAttemptObservation {
    key: Option<TwoRealPreviewLoopSeenDecodeCacheKey>,
    first_byte_slow: bool,
    output_read_slow: bool,
    input_write_outlier: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
struct TwoRealPreviewLoopDecodeOutputOverride {
    width: u32,
    height: u32,
    reason: &'static str,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealContinuousDecodeSource {
    client_id: ClientId,
    run_id: RunId,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealProgramFirstDecodeSuppressionConfig {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealOperatorPreviewProgramReuseSource {
    ContinuousLatest,
    LastValidProgram,
    None,
}

impl TwoRealOperatorPreviewProgramReuseSource {
    fn as_summary_value(self) -> &'static str {
        match self {
            Self::ContinuousLatest => "continuous_latest",
            Self::LastValidProgram => "last_valid_program",
            Self::None => "none",
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealContinuousDecodeInputOrigin {
    Feeder,
    RenderDemand,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealContinuousDecodeEnqueueResult {
    Enqueued,
    SkippedDuplicate,
    SkippedRuntimeDisabled,
    SkippedStartupNotReady,
    SkippedSourceMismatch,
    DroppedInputQueueFull,
    Disconnected,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealContinuousFeedSkipReason {
    Duplicate,
    FutureFrame,
    RuntimeDisabled,
    StartupNotReady,
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealContinuousDecodeMetadata {
    key: TwoRealPreviewLoopDecodeCacheKey,
    frame_id: u64,
    correspondence_enqueued_at: Option<Instant>,
}

#[derive(Debug, Default, Clone, PartialEq, Eq)]
struct TwoRealContinuousPendingCorrespondenceSnapshot {
    count: usize,
    frame_id_min: Option<u64>,
    frame_id_max: Option<u64>,
    oldest_frame_id: Option<u64>,
    newest_frame_id: Option<u64>,
    age_ms_total: u128,
    age_sample_count: u32,
    age_ms_max: Option<u128>,
}

struct TwoRealContinuousDecodeInput {
    metadata: TwoRealContinuousDecodeMetadata,
    encoded_payload: Vec<u8>,
}

struct TwoRealContinuousDecodeOutput {
    metadata: TwoRealContinuousDecodeMetadata,
    frame: SwitcherDecodedFrame,
    stdout_read_elapsed_ms: u128,
    output_pipe_bytes_len: usize,
    pixel_convert_elapsed_ms: u128,
    pixel_convert_count: u32,
    pixel_convert_buffer_reuse_count: u32,
...
enum TwoRealContinuousDecodeEvent {
    Decoded(TwoRealContinuousDecodeOutput),
    RuntimeError(String),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealContinuousDecodeLookupMissReason {
    ExactKeyMissing,
    QueueEmpty,
    RuntimeDisabled,
    OutputPending,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum TwoRealContinuousSlot0OneShotSuppressionReason {
    ContinuousNotReady,
    Stale,
    Future,
}
...
struct TwoRealContinuousDecodeSharedDiagnostics {
    stdin_write_count: AtomicUsize,
    stdin_write_bytes_total: AtomicUsize,
    stdin_write_error_count: AtomicUsize,
    last_stdin_write_elapsed_ms: AtomicUsize,
    ...
    last_stdin_write_error: Mutex<Option<String>>,
    process_exit_status: Mutex<Option<String>>,
}

impl TwoRealContinuousDecodeSharedDiagnostics {
    fn new() -> Self {
        Self {
            stdin_write_count: AtomicUsize::new(0),
            stdin_write_bytes_total: AtomicUsize::new(0),
            stdin_write_error_count: AtomicUsize::new(0),
            last_stdin_write_elapsed_ms: AtomicUsize::new(0),
            ...
            last_stdin_write_error: Mutex::new(None),
            process_exit_status: Mutex::new(None),
        }
    }
}

struct TwoRealContinuousDecodeSession {
    child: Mutex<Child>,
    ffmpeg_args_summary: String,
    input_tx: mpsc::SyncSender<TwoRealContinuousDecodeInput>,
    output_rx: mpsc::Receiver<TwoRealContinuousDecodeEvent>,
    correspondence: Arc<Mutex<VecDeque<TwoRealContinuousDecodeMetadata>>>,
    writer_input_queue_len: Arc<AtomicUsize>,
    stdout_reader_waiting: Arc<AtomicUsize>,
    stderr_summary_bytes: Arc<Mutex<Vec<u8>>>,
    diagnostics: Arc<TwoRealContinuousDecodeSharedDiagnostics>,
    writer_thread: Option<thread::JoinHandle<()>>,
    reader_thread: Option<thread::JoinHandle<()>>,
    stderr_thread: Option<thread::JoinHandle<()>>,
}

impl TwoRealContinuousDecodeSession {
    fn process_exit_status(&self) -> Option<String> {
        ...
        if let Some(status) = status.as_ref() {
            if let Ok(mut stored_status) = self.diagnostics.process_exit_status.lock() {
                *stored_status = Some(status.clone());
            }
        }
        status
    }
...
struct TwoRealContinuousDecodeConfig {
    ...
    output_pipeline_experiment_mode: TwoRealContinuousOutputPipelineExperimentMode,
}
...
impl TwoRealContinuousDecodeConfig {
    ...
    fn output_pipe_bytes_saved_per_frame(self, width: u32, height: u32) -> usize {
        self.output_pipeline_experiment_mode
            .pipe_bytes_saved_per_frame(width, height)
    }
}
...
struct TwoRealContinuousDecodeRuntime {
    ...
    continuous_decoded_keys: HashSet<TwoRealPreviewLoopDecodeCacheKey>,
    continuous_key_order: VecDeque<TwoRealPreviewLoopDecodeCacheKey>,
    latest_decoded_frame_id: Option<u64>,
    last_output_at: Option<Instant>,
    first_input_at: Option<Instant>,
    first_output_at: Option<Instant>,
    restart_count: u32,
    last_ffmpeg_stderr_summary: Option<String>,
}
...
impl TwoRealContinuousDecodeRuntime {
    fn new(
        ...
        Self {
            ...
            continuous_decoded_keys: HashSet::new(),
            continuous_key_order: VecDeque::new(),
            latest_decoded_frame_id: None,
            last_output_at: None,
            first_input_at: None,
            first_output_at: None,
            restart_count: 0,
            last_ffmpeg_stderr_summary: None,
        }
    }

    fn matches_source(&self, input: &SwitcherH264DecodeInput) -> bool {
        input
            .source_identity
            .as_ref()
            .map(|source| {
                source.client_id == self.source.client_id && source.run_id == self.source.run_id
            })
            .unwrap_or(false)
    }

    fn decoded_key_frame_id(key: &TwoRealPreviewLoopDecodeCacheKey) -> Option<u64> {
        key.source_identity.as_ref().map(|source| source.frame_id)
    }

    fn queue_oldest_frame_id(&self) -> Option<u64> {
        self.continuous_key_order
            .front()
            .and_then(Self::decoded_key_frame_id)
    }

    fn queue_newest_frame_id(&self) -> Option<u64> {
        self.continuous_key_order
            .back()
            .and_then(Self::decoded_key_frame_id)
    }
    ...
    fn pending_correspondence_count(&self) -> usize {
        self.session
            .as_ref()
            .and_then(|session| session.correspondence.lock().ok().map(|queue| queue.len()))
            .unwrap_or(0)
    }

    fn pending_correspondence_snapshot(&self) -> TwoRealContinuousPendingCorrespondenceSnapshot {
        self.session
            .as_ref()
            .and_then(|session| {
                session.correspondence.lock().ok().map(|queue| {
                    let now = Instant::now();
                    let mut snapshot = TwoRealContinuousPendingCorrespondenceSnapshot {
                        count: queue.len(),
                        oldest_frame_id: queue.front().map(|metadata| metadata.frame_id),
                        newest_frame_id: queue.back().map(|metadata| metadata.frame_id),
                        ..TwoRealContinuousPendingCorrespondenceSnapshot::default()
                    };
                    for metadata in queue.iter() {
                        let frame_id = metadata.frame_id;
                        snapshot.frame_id_min = Some(
                            snapshot
                                .frame_id_min
                                .map_or(frame_id, |current| current.min(frame_id)),
                        );
                        snapshot.frame_id_max = Some(
                            snapshot
                                .frame_id_max
                                .map_or(frame_id, |current| current.max(frame_id)),
                        );
                        if let Some(enqueued_at) = metadata.correspondence_enqueued_at {
                            let age_ms = now.saturating_duration_since(enqueued_at).as_millis();
                            snapshot.age_ms_total = snapshot.age_ms_total.saturating_add(age_ms);
                            snapshot.age_sample_count = snapshot.age_sample_count.saturating_add(1);
                            snapshot.age_ms_max = Some(
                                snapshot
                                    .age_ms_max
    ...
    fn writer_input_queue_len(&self) -> usize {
        self.session
            .as_ref()
            .map(|session| session.writer_input_queue_len.load(Ordering::Relaxed))
            .unwrap_or(0)
    }

    fn stdout_reader_waiting(&self) -> bool {
        self.session
            .as_ref()
            .map(|session| session.stdout_reader_waiting.load(Ordering::Relaxed) > 0)
            .unwrap_or(false)
    }

    fn current_ffmpeg_stderr_summary(&self) -> Option<String> {
        self.session
            .as_ref()
            .and_then(|session| {
                format_continuous_ffmpeg_stderr_summary(&session.stderr_summary_bytes)
            })
            .or_else(|| self.last_ffmpeg_stderr_summary.clone())
    }

    fn update_timing_flags(&self, timing: &mut TwoRealPreviewLoopRuntimeTiming) {
        timing.continuous_decode_config_enabled = true;
        timing.continuous_decode_slot0_enabled = true;
        ...
        timing.continuous_decode_output_pipeline_experiment_mode = Some(
            self.config
        ...
        timing.continuous_decode_output_pipe_bytes_saved_per_frame = self
            .config
            .output_pipe_bytes_saved_per_frame(self.output_width, self.output_height);
        timing.continuous_decode_slot0_one_shot_suppression_enabled =
            self.config.slot0_one_shot_suppression_enabled;
        ...
        let pending_snapshot = self.pending_correspondence_snapshot();
        timing.continuous_decode_output_pending_correspondence_count = pending_snapshot.count;
        timing.continuous_decode_pending_correspondence_frame_id_min =
            pending_snapshot.frame_id_min;
        timing.continuous_decode_pending_correspondence_frame_id_max =
            pending_snapshot.frame_id_max;
        timing.continuous_decode_pending_correspondence_oldest_frame_id =
            pending_snapshot.oldest_frame_id;
        timing.continuous_decode_pending_correspondence_newest_frame_id =
            pending_snapshot.newest_frame_id;
        timing.continuous_decode_pending_correspondence_age_ms_total =
            pending_snapshot.age_ms_total;
        timing.continuous_decode_pending_correspondence_age_sample_count =
            pending_snapshot.age_sample_count;
        timing.continuous_decode_pending_correspondence_age_ms_max = pending_snapshot.age_ms_max;
        ...
        if let Some(stderr_summary) = self.current_ffmpeg_stderr_summary() {
            timing.continuous_decode_ffmpeg_stderr_summary = Some(stderr_summary);
        }
        if let Some(first_input_at) = self.first_input_at {
            timing.continuous_decode_first_input_to_now_elapsed_ms =
                Some(first_input_at.elapsed().as_millis());
            if let Some(first_output_at) = self.first_output_at {
                timing.continuous_decode_first_input_to_first_output_elapsed_ms =
                    Some(first_output_at.duration_since(first_input_at).as_millis());
            }
        }
        if let Some(session) = self.session.as_ref() {
            let diagnostics = &session.diagnostics;
            timing.continuous_decode_ffmpeg_args_summary =
                Some(session.ffmpeg_args_summary.clone());
            timing.continuous_decode_stdin_write_count =
                diagnostics.stdin_write_count.load(Ordering::Relaxed);
            timing.continuous_decode_stdin_write_bytes_total =
                diagnostics.stdin_write_bytes_total.load(Ordering::Relaxed);
            timing.continuous_decode_stdin_write_error_count =
                diagnostics.stdin_write_error_count.load(Ordering::Relaxed);
            timing.continuous_decode_last_stdin_write_error = diagnostics
                .last_stdin_write_error
                .lock()
                .ok()
                .and_then(|error| error.clone());
            let process_exit_status = session.process_exit_status();
            timing.continuous_decode_process_running = process_exit_status.is_none();
            timing.continuous_decode_process_exit_status = process_exit_status;
            timing.continuous_decode_stdout_read_attempt_count = diagnostics
            ...
            timing.continuous_decode_stderr_bytes_total =
                diagnostics.stderr_bytes_total.load(Ordering::Relaxed);
            timing.continuous_decode_last_stderr_at_ms =
                match diagnostics.last_stderr_at_ms.load(Ordering::Relaxed) {
                    0 => None,
                    value => Some(value.saturating_sub(1) as u128),
                };
            timing.continuous_decode_last_input_write_elapsed_ms = diagnostics
                .last_stdin_write_elapsed_ms
                .load(Ordering::Relaxed)
                as u128;
            timing.continuous_decode_last_input_payload_bytes =
                diagnostics.last_input_payload_bytes.load(Ordering::Relaxed);
        } else if self.runtime_disabled_reason.is_some() {
            timing.continuous_decode_process_running = false;
        }
    }

    fn observe_requested_frame(
        &self,
        input: &SwitcherH264DecodeInput,
        timing: &mut TwoRealPreviewLoopRuntimeTiming,
    ) {
        let Some(source) = input.source_identity.as_ref() else {
            return;
        };
        timing.continuous_decode_requested_frame_id = Some(source.frame_id);
        if let Some(latest_enqueued) = timing.continuous_feed_latest_enqueued_frame_id {
            timing.continuous_decode_feeder_lag_to_selected =
                Some(source.frame_id.saturating_sub(latest_enqueued));
        }
        timing.continuous_decode_exact_match_required_count = timing
        ...
        if let Some(latest_decoded) = self.latest_decoded_frame_id {
            ...
            if current_lag > TWO_REAL_CONTINUOUS_DECODE_QUEUE_BOUND as u64 {
                timing.continuous_decode_stall_count =
                    timing.continuous_decode_stall_count.saturating_add(1);
            }
        }
    }

    fn observe_lookup_hit(&self, timing: &mut TwoRealPreviewLoopRuntimeTiming) {
        timing.continuous_decode_lookup_hit_count =
            timing.continuous_decode_lookup_hit_count.saturating_add(1);
        ...
        match candidate {
            TwoRealContinuousBoundedLookupCandidate::RejectedStale => {
                timing.continuous_decode_bounded_lookup_rejected_stale_count = timing
                    ...
                    .saturating_add(1);
                timing.continuous_decode_output_availability_stale_count = timing
            ...
            TwoRealContinuousBoundedLookupCandidate::RejectedFuture => {
                timing.continuous_decode_bounded_lookup_rejected_future_count = timing
                    ...
                    .saturating_add(1);
                timing.continuous_decode_output_availability_future_count = timing
            ...
            TwoRealContinuousBoundedLookupCandidate::RejectedNotReady => {
                timing.continuous_decode_bounded_lookup_rejected_not_ready_count = timing
                    ...
                    .saturating_add(1);
                timing.continuous_decode_output_availability_not_ready_count = timing
    ...
    fn classify_lookup_miss(
        &self,
        input: &SwitcherH264DecodeInput,
    ) -> TwoRealContinuousDecodeLookupMissReason {
        if self.runtime_disabled {
            return TwoRealContinuousDecodeLookupMissReason::RuntimeDisabled;
        }
        let pending_correspondence_count = self.pending_correspondence_count();
        let writer_input_queue_len = self.writer_input_queue_len();
        if pending_correspondence_count > 0 || writer_input_queue_len > 0 {
            return TwoRealContinuousDecodeLookupMissReason::OutputPending;
        }
        if self.continuous_key_order.is_empty() {
            return TwoRealContinuousDecodeLookupMissReason::QueueEmpty;
        }
        if let (Some(source), Some(latest_decoded)) =
            (input.source_identity.as_ref(), self.latest_decoded_frame_id)
        {
            if source.frame_id > latest_decoded {
                return TwoRealContinuousDecodeLookupMissReason::FrameIdLagging;
            }
        }
        TwoRealContinuousDecodeLookupMissReason::ExactKeyMissing
    }

    fn observe_lookup_miss(
        &self,
        input: &SwitcherH264DecodeInput,
        timing: &mut TwoRealPreviewLoopRuntimeTiming,
    ) {
        timing.continuous_decode_lookup_miss_count =
            timing.continuous_decode_lookup_miss_count.saturating_add(1);
        let miss_reason = self.classify_lookup_miss(input);
        match miss_reason {
            TwoRealContinuousDecodeLookupMissReason::ExactKeyMissing => {
                timing.continuous_decode_lookup_miss_reason_exact_key_missing_count = timing
                    .continuous_decode_lookup_miss_reason_exact_key_missing_count
                    .saturating_add(1);
            }
            TwoRealContinuousDecodeLookupMissReason::QueueEmpty => {
                timing.continuous_decode_lookup_miss_reason_queue_empty_count = timing
                    .continuous_decode_lookup_miss_reason_queue_empty_count
                    .saturating_add(1);
            }
            TwoRealContinuousDecodeLookupMissReason::RuntimeDisabled => {
                timing.continuous_decode_lookup_miss_reason_runtime_disabled_count = timing
                    .continuous_decode_lookup_miss_reason_runtime_disabled_count
                    .saturating_add(1);
            }
            TwoRealContinuousDecodeLookupMissReason::OutputPending => {
                timing.continuous_decode_lookup_miss_reason_output_pending_count = timing
                    .continuous_decode_lookup_miss_reason_output_pending_count
                    .saturating_add(1);
                let pending_correspondence_count = self.pending_correspondence_count();
                if pending_correspondence_count > 0 {
                    timing.continuous_decode_no_output_after_input_count = timing
                        .continuous_decode_no_output_after_input_count
                        .saturating_add(1);
                    if timing.continuous_decode_input_has_idr_count
                        > timing.continuous_decode_bootstrap_output_count
                    {
                        timing.continuous_decode_no_output_after_keyframe_count = timing
                            .continuous_decode_no_output_after_keyframe_count
                            .saturating_add(1);
                    }
                    if self.stdout_reader_waiting() && self.writer_input_queue_len() == 0 {
                        timing.continuous_decode_stdout_reader_blocked_count = timing
                            .continuous_decode_stdout_reader_blocked_count
                            .saturating_add(1);
                    }
                }
            }
            TwoRealContinuousDecodeLookupMissReason::FrameIdLagging => {
                timing.continuous_decode_lookup_miss_reason_frame_id_lagging_count = timing
                    .continuous_decode_lookup_miss_reason_frame_id_lagging_count
                    .saturating_add(1);
            }
        }
        let stale_frame_available = input
            .source_identity
            .as_ref()
            .zip(self.queue_newest_frame_id())
            .map(|(source, newest)| newest < source.frame_id)
            .unwrap_or(matches!(
                miss_reason,
                TwoRealContinuousDecodeLookupMissReason::FrameIdLagging
            ));
        if stale_frame_available {
            timing.continuous_decode_render_miss_stale_count = timing
                .continuous_decode_render_miss_stale_count
                .saturating_add(1);
        } else {
            timing.continuous_decode_render_miss_not_ready_count = timing
                .continuous_decode_render_miss_not_ready_count
                .saturating_add(1);
        }
        if let (Some(source), Some(newest)) =
            (input.source_identity.as_ref(), self.queue_newest_frame_id())
        {
            if newest < source.frame_id {
                timing.continuous_decode_stale_frame_available_count = timing
    ...
    fn disable(&mut self, timing: &mut TwoRealPreviewLoopRuntimeTiming, reason: impl Into<String>) {
        let reason = reason.into();
        self.runtime_disabled = true;
        self.runtime_disabled_reason = Some(reason.clone());
        if let Some(session) = self.session.take() {
            let stderr_summary_bytes = Arc::clone(&session.stderr_summary_bytes);
            session.stop();
            if let Some(stderr_summary) =
                format_continuous_ffmpeg_stderr_summary(&stderr_summary_bytes)
            {
                self.last_ffmpeg_stderr_summary = Some(stderr_summary);
            }
        }
        timing.continuous_decode_runtime_enabled = false;
        timing.continuous_decode_runtime_disabled = true;
        ...
        match spawn_two_real_continuous_ffmpeg_decode_session(
            ...
            Err(error) => {
                let reason = if error.kind() == io::ErrorKind::NotFound {
                    "continuous_ffmpeg_unavailable".to_string()
                } else {
                    format!("continuous_ffmpeg_spawn_failed:{}", error)
                };
                self.disable(timing, reason.clone());
                Err(reason)
            }
        }
    }
    ...
    fn enqueue(
        ...
        key: &TwoRealPreviewLoopDecodeCacheKey,
        timing: &mut TwoRealPreviewLoopRuntimeTiming,
        origin: TwoRealContinuousDecodeInputOrigin,
    ) -> TwoRealContinuousDecodeEnqueueResult {
        self.update_timing_flags(timing);
        if self.runtime_disabled {
            return TwoRealContinuousDecodeEnqueueResult::SkippedRuntimeDisabled;
        }
        if self.enqueued_keys.contains(key) {
            return TwoRealContinuousDecodeEnqueueResult::SkippedDuplicate;
        }
        let Some(source_identity) = input.source_identity.as_ref() else {
            return TwoRealContinuousDecodeEnqueueResult::SkippedSourceMismatch;
        };
        if self.ensure_started(input, timing).is_err() {
            return TwoRealContinuousDecodeEnqueueResult::SkippedStartupNotReady;
        }
        let metadata = TwoRealContinuousDecodeMetadata {
            key: key.clone(),
            frame_id: source_identity.frame_id,
            correspondence_enqueued_at: None,
        };
        let Some(session) = self.session.as_ref() else {
            return TwoRealContinuousDecodeEnqueueResult::SkippedRuntimeDisabled;
        };
        session
            .writer_input_queue_len
            .fetch_add(1, Ordering::Relaxed);
        match session.input_tx.try_send(TwoRealContinuousDecodeInput {
            metadata,
            encoded_payload: input.encoded_payload.clone(),
        }) {
            Ok(()) => {
                if self.first_input_at.is_none() {
                    self.first_input_at = Some(Instant::now());
                }
                let payload_summary = SwitcherH264AnnexBPayloadInspectionBoundary
                    .inspect_payload(&input.encoded_payload);
                if let Some(previous_frame_id) = timing.continuous_decode_last_input_frame_id {
                    let frame_gap = source_identity.frame_id.abs_diff(previous_frame_id);
                    timing.continuous_decode_input_frame_id_gap_max = timing
                        .continuous_decode_input_frame_id_gap_max
                        .max(frame_gap);
                    timing.continuous_decode_input_frame_id_gap_total = timing
                        .continuous_decode_input_frame_id_gap_total
                        .saturating_add(frame_gap);
                    if frame_gap != 1 {
                        timing.continuous_decode_input_non_consecutive_count = timing
                ...
                if payload_summary.has_idr {
                    ...
                    timing.continuous_decode_input_non_keyframe_count = timing
                        .continuous_decode_input_non_keyframe_count
                        .saturating_add(1);
                }
                if payload_summary.has_sps {
                    timing.continuous_decode_input_has_sps_count = timing
                        .continuous_decode_input_has_sps_count
                        .saturating_add(1);
                }
                if payload_summary.has_pps {
                    timing.continuous_decode_input_has_pps_count = timing
                        .continuous_decode_input_has_pps_count
                        .saturating_add(1);
                }
                if payload_summary.has_idr {
                    timing.continuous_decode_input_has_idr_count = timing
                        .continuous_decode_input_has_idr_count
                        .saturating_add(1);
                }
                if payload_summary.has_non_idr_vcl {
                    timing.continuous_decode_input_has_non_idr_vcl_count = timing
                        .continuous_decode_input_has_non_idr_vcl_count
                        .saturating_add(1);
                }
                timing.continuous_decode_last_input_payload_nal_kinds =
                    Some(format_switcher_payload_nal_kinds(&payload_summary));
                if self.latest_decoded_frame_id.is_none() {
                    timing.continuous_decode_bootstrap_input_count = timing
                        .continuous_decode_bootstrap_input_count
                        .saturating_add(1);
                }
                self.enqueued_keys.insert(key.clone());
                timing.continuous_decode_input_frame_count =
                    timing.continuous_decode_input_frame_count.saturating_add(1);
                match origin {
                    TwoRealContinuousDecodeInputOrigin::Feeder => {
                        timing.continuous_decode_input_from_feeder_count = timing
                            .continuous_decode_input_from_feeder_count
                            .saturating_add(1);
                    }
                    TwoRealContinuousDecodeInputOrigin::RenderDemand => {
                        timing.continuous_decode_input_from_render_demand_count = timing
                            .continuous_decode_input_from_render_demand_count
                            .saturating_add(1);
                    }
                }
                timing.continuous_decode_input_frame_id_min = Some(
                    timing
                        .continuous_decode_input_frame_id_min
                        .map(|current| current.min(source_identity.frame_id))
                        .unwrap_or(source_identity.frame_id),
                );
                timing.continuous_decode_input_frame_id_max = Some(
                    timing
                        .continuous_decode_input_frame_id_max
                        .map(|current| current.max(source_identity.frame_id))
                        .unwrap_or(source_identity.frame_id),
                );
            }
            ...
            Err(mpsc::TrySendError::Disconnected(_)) => {
                session
                    .writer_input_queue_len
                    .fetch_sub(1, Ordering::Relaxed);
                self.disable(timing, "continuous_input_writer_disconnected");
                return TwoRealContinuousDecodeEnqueueResult::Disconnected;
            }
        }
        TwoRealContinuousDecodeEnqueueResult::Enqueued
    }

    fn drain_outputs(
        &mut self,
        decoded_cache: &mut HashMap<TwoRealPreviewLoopDecodeCacheKey, SwitcherDecodedFrame>,
        timing: &mut TwoRealPreviewLoopRuntimeTiming,
    ) {
        self.update_timing_flags(timing);
        ...
        for event in events {
            match event {
                TwoRealContinuousDecodeEvent::Decoded(output) => {
                    if self.latest_decoded_frame_id.is_none() {
                        timing.continuous_decode_bootstrap_output_count = timing
                            .continuous_decode_bootstrap_output_count
                            .saturating_add(1);
                    }
                    timing.continuous_decode_output_frame_count = timing
                        .continuous_decode_output_frame_count
                        .saturating_add(1);
                    timing.continuous_decode_output_frame_id_min = Some(
                        timing
                            .continuous_decode_output_frame_id_min
                            .map(|current| current.min(output.metadata.frame_id))
                            .unwrap_or(output.metadata.frame_id),
                    );
                    timing.continuous_decode_output_frame_id_max = Some(
                        timing
                            .continuous_decode_output_frame_id_max
                            .map(|current| current.max(output.metadata.frame_id))
                            .unwrap_or(output.metadata.frame_id),
                    );
                    ...
                    if output.stdout_read_elapsed_ms
                        > TWO_REAL_CONTINUOUS_READER_FULL_FRAME_SLOW_THRESHOLD_MS
                    {
                        timing.continuous_decode_reader_full_frame_slow_count = timing
                    ...
                    timing.continuous_decode_output_bytes_total = timing
                        ...
                        .saturating_add(output.output_pipe_bytes_len);
                    timing.continuous_decode_output_pixel_convert_elapsed_ms = timing
                        .continuous_decode_output_pixel_convert_elapsed_ms
                        .saturating_add(output.pixel_convert_elapsed_ms);
                    timing.continuous_decode_output_pixel_convert_elapsed_ms_max = timing
                        .continuous_decode_output_pixel_convert_elapsed_ms_max
                        .max(output.pixel_convert_elapsed_ms);
                    timing.continuous_decode_output_pixel_convert_count = timing
                        .continuous_decode_output_pixel_convert_count
                        .saturating_add(output.pixel_convert_count);
                    timing.continuous_decode_output_pixel_convert_buffer_reuse_count = timing
                        .continuous_decode_output_pixel_convert_buffer_reuse_count
                        .saturating_add(output.pixel_convert_buffer_reuse_count);
                    timing.continuous_decode_output_pixel_convert_buffer_allocation_count = timing
                        .continuous_decode_output_pixel_convert_buffer_allocation_count
                        .saturating_add(output.pixel_convert_buffer_allocation_count);
                    ...
                    if let Some(last_output_at) = self.last_output_at {
                        let output_interval_ms = output
                            .reader_emitted_at
                            .duration_since(last_output_at)
                            .as_millis();
                        timing.continuous_decode_output_frame_interval_elapsed_ms = timing
                            .continuous_decode_output_frame_interval_elapsed_ms
                            .saturating_add(output_interval_ms);
                        timing.continuous_decode_output_frame_interval_count = timing
                            .continuous_decode_output_frame_interval_count
                            .saturating_add(1);
                        timing.continuous_decode_output_frame_interval_ms_max = timing
                            .continuous_decode_output_frame_interval_ms_max
                            .max(output_interval_ms);
                    }
                    ...
                    if self.first_output_at.is_none() {
                        self.first_output_at = Some(output.reader_emitted_at);
                    }
                    self.last_output_at = Some(output.reader_emitted_at);
                    decoded_cache.insert(output.metadata.key.clone(), output.frame);
                    self.continuous_decoded_keys
                        .insert(output.metadata.key.clone());
                    self.continuous_key_order
                        .push_back(output.metadata.key.clone());
                    while self.continuous_key_order.len() > TWO_REAL_CONTINUOUS_DECODE_QUEUE_BOUND {
                        if let Some(stale_key) = self.continuous_key_order.pop_front() {
                            self.continuous_decoded_keys.remove(&stale_key);
                            decoded_cache.remove(&stale_key);
                            timing.continuous_decode_dropped_stale_count = timing
                                .continuous_decode_dropped_stale_count
                                .saturating_add(1);
                ...
                TwoRealContinuousDecodeEvent::RuntimeError(reason) => {
                    self.disable(timing, reason);
                }
            }
        }
        self.update_timing_flags(timing);
    ...
    fn is_continuous_decoded_key(&self, key: &TwoRealPreviewLoopDecodeCacheKey) -> bool {
        self.continuous_decoded_keys.contains(key)
    }
...
impl Drop for TwoRealContinuousDecodeRuntime {
    fn drop(&mut self) {
        self.stop();
    }
}
...
fn build_two_real_continuous_ffmpeg_args(
    ...
    ffmpeg_args.extend([
        ...
        "rawvideo".to_string(),
        "-pix_fmt".to_string(),
        config.ffmpeg_output_pixel_format().to_string(),
    ]);
    if config.low_latency_args_enabled {
...
fn spawn_two_real_continuous_ffmpeg_decode_session(
    ...
    let stdin = child.stdin.take().ok_or_else(|| {
        io::Error::new(
            ...
            "continuous ffmpeg stdin pipe unavailable",
        )
    })?;
    let stdout = child.stdout.take().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::BrokenPipe,
            "continuous ffmpeg stdout pipe unavailable",
        )
    })?;
    let stderr = child.stderr.take().ok_or_else(|| {
        io::Error::new(
            io::ErrorKind::BrokenPipe,
            "continuous ffmpeg stderr pipe unavailable",
        )
    })?;
    let (input_tx, input_rx) = mpsc::sync_channel(TWO_REAL_CONTINUOUS_DECODE_QUEUE_BOUND);
    let (output_tx, output_rx) = mpsc::channel();
    let correspondence = Arc::new(Mutex::new(VecDeque::new()));
    let writer_input_queue_len = Arc::new(AtomicUsize::new(0));
    let stdout_reader_waiting = Arc::new(AtomicUsize::new(0));
    let stderr_summary_bytes = Arc::new(Mutex::new(Vec::new()));
    let diagnostics = Arc::new(TwoRealContinuousDecodeSharedDiagnostics::new());
    let runtime_started_at = Instant::now();
    let writer_correspondence = Arc::clone(&correspondence);
    let reader_correspondence = Arc::clone(&correspondence);
    let writer_input_queue_len_for_thread = Arc::clone(&writer_input_queue_len);
    let stdout_reader_waiting_for_thread = Arc::clone(&stdout_reader_waiting);
    let stderr_summary_bytes_for_thread = Arc::clone(&stderr_summary_bytes);
    ...
    let writer_thread = thread::spawn(move || {
        two_real_continuous_ffmpeg_writer_loop(
            stdin,
            input_rx,
            writer_correspondence,
            writer_input_queue_len_for_thread,
            writer_diagnostics,
            writer_output_tx,
        );
    });
    let reader_thread = thread::spawn(move || {
        two_real_continuous_ffmpeg_reader_loop(
            stdout,
            expected_len,
            output_width,
            output_height,
            config.output_pipeline_experiment_mode,
            reader_correspondence,
            stdout_reader_waiting_for_thread,
            reader_diagnostics,
            output_tx,
        );
    });
    let stderr_thread = thread::spawn(move || {
        two_real_continuous_ffmpeg_stderr_loop(
            stderr,
            stderr_summary_bytes_for_thread,
            stderr_diagnostics,
            runtime_started_at,
        );
    });
    Ok(TwoRealContinuousDecodeSession {
        ...
        correspondence,
        writer_input_queue_len,
        stdout_reader_waiting,
        stderr_summary_bytes,
        diagnostics,
...
fn two_real_continuous_ffmpeg_writer_loop(
    mut stdin: impl Write,
    input_rx: mpsc::Receiver<TwoRealContinuousDecodeInput>,
    correspondence: Arc<Mutex<VecDeque<TwoRealContinuousDecodeMetadata>>>,
    writer_input_queue_len: Arc<AtomicUsize>,
    diagnostics: Arc<TwoRealContinuousDecodeSharedDiagnostics>,
    output_tx: mpsc::Sender<TwoRealContinuousDecodeEvent>,
) {
    while let Ok(input) = input_rx.recv() {
        writer_input_queue_len.fetch_sub(1, Ordering::Relaxed);
        diagnostics
            .last_input_payload_bytes
            .store(input.encoded_payload.len(), Ordering::Relaxed);
        let mut metadata = input.metadata;
        metadata.correspondence_enqueued_at = Some(Instant::now());
        if let Ok(mut queue) = correspondence.lock() {
            queue.push_back(metadata);
        } else {
            let _ = output_tx.send(TwoRealContinuousDecodeEvent::RuntimeError(
                "continuous_correspondence_lock_poisoned".to_string(),
            ));
            break;
        }
        let write_start = Instant::now();
        match stdin.write_all(&input.encoded_payload) {
            Ok(()) => {
                diagnostics
                    .stdin_write_count
                    .fetch_add(1, Ordering::Relaxed);
                diagnostics
                    .stdin_write_bytes_total
                    .fetch_add(input.encoded_payload.len(), Ordering::Relaxed);
                diagnostics.last_stdin_write_elapsed_ms.store(
                    write_start.elapsed().as_millis().min(usize::MAX as u128) as usize,
                    Ordering::Relaxed,
                );
            }
            Err(error) => {
                ...
                diagnostics
                    .stdin_write_error_count
                    .fetch_add(1, Ordering::Relaxed);
                let error = error.to_string();
                if let Ok(mut last_error) = diagnostics.last_stdin_write_error.lock() {
                    *last_error = Some(error.clone());
                }
                let _ = output_tx.send(TwoRealContinuousDecodeEvent::RuntimeError(format!(
                    "continuous_stdin_write_failed:{}",
                    error
                )));
                break;
            }
...
fn two_real_continuous_ffmpeg_reader_loop(
    ...
    loop {
        ...
        while bytes_read_total < expected_len {
            match stdout.read(&mut pixels[bytes_read_total..expected_len]) {
                Ok(0) => {
        ...
        diagnostics
            .stdout_partial_bytes_read
            .store(0, Ordering::Relaxed);
        let stdout_read_elapsed_ms = read_start.elapsed().as_millis();
        let output_pipe_bytes_len = expected_len;
        let convert_start = Instant::now();
        let (
            pixels,
            pixel_convert_elapsed_ms,
            pixel_convert_count,
            pixel_convert_buffer_reuse_count,
        ...
        let Some(metadata) = metadata else {
            let _ = output_tx.send(TwoRealContinuousDecodeEvent::RuntimeError(
                "continuous_frame_id_mismatch_output_without_input".to_string(),
            ));
            break;
        };
        if output_tx
            .send(TwoRealContinuousDecodeEvent::Decoded(
                TwoRealContinuousDecodeOutput {
                    metadata,
                    frame: SwitcherDecodedFrame {
                        width: output_width,
...
fn convert_bgr24_to_bgra8(pixels: &[u8]) -> Vec<u8> {
    let mut output = vec![0u8; pixels.len() / 3 * 4];
    output[..pixels.len()].copy_from_slice(pixels);
    convert_bgr24_to_bgra8_in_place(&mut output, pixels.len());
    output
}

fn convert_bgr24_to_bgra8_in_place(pixels: &mut [u8], bgr_len: usize) -> usize {
    debug_assert_eq!(bgr_len % 3, 0);
    let pixel_count = bgr_len / 3;
    let bgra_len = pixel_count * 4;
    debug_assert!(pixels.len() >= bgra_len);
    for pixel_index in (0..pixel_count).rev() {
        let source_index = pixel_index * 3;
        let target_index = pixel_index * 4;
        let b = pixels[source_index];
        let g = pixels[source_index + 1];
        let r = pixels[source_index + 2];
...
fn two_real_continuous_ffmpeg_stderr_loop(
    mut stderr: impl Read,
    stderr_summary_bytes: Arc<Mutex<Vec<u8>>>,
    diagnostics: Arc<TwoRealContinuousDecodeSharedDiagnostics>,
    runtime_started_at: Instant,
) {
    diagnostics.stderr_reader_alive.store(1, Ordering::Relaxed);
    ...
    loop {
        match stderr.read(&mut chunk) {
            ...
            Ok(bytes_read) => {
                ...
                diagnostics.last_stderr_at_ms.store(
                    runtime_started_at
                        .elapsed()
                        .as_millis()
                        .saturating_add(1)
                ...
                if let Ok(mut bytes) = stderr_summary_bytes.lock() {
                    bytes.extend_from_slice(&chunk[..bytes_read]);
                    if bytes.len() > TWO_REAL_CONTINUOUS_DECODE_STDERR_SUMMARY_MAX_BYTES {
                        let drain_len =
                            bytes.len() - TWO_REAL_CONTINUOUS_DECODE_STDERR_SUMMARY_MAX_BYTES;
                        bytes.drain(0..drain_len);
                    }
                } else {
                    break;
                }
    ...
    diagnostics.stderr_reader_alive.store(0, Ordering::Relaxed);
}

fn format_continuous_ffmpeg_stderr_summary(
    stderr_summary_bytes: &Arc<Mutex<Vec<u8>>>,
) -> Option<String> {
    let bytes = stderr_summary_bytes.lock().ok()?;
    if bytes.is_empty() {
        return None;
    }
...
impl<'a, Runtime> TimedSwitcherH264DecodeRuntime<'a, Runtime> {
    fn new(
        ...
        });
        {
            ...
            timing.continuous_decode_config_enabled = continuous_slot0.is_some();
            timing.continuous_decode_slot0_enabled = continuous_slot0.is_some();
            timing.continuous_decode_runtime_enabled = continuous_slot0.is_some();
        }
        Self {
            inner,
            timing,
    ...
    fn set_program_first_decode_suppression_active(&self, active: bool) {
        self.program_first_decode_suppression_active.set(active);
        if active && self.program_first_decode_suppression.is_some() {
            self.timing
                .borrow_mut()
    ...
    fn maybe_suppress_program_first_preview_one_shot(
        &self,
        input: &SwitcherH264DecodeInput,
    ) -> bool {
        if !self.program_first_decode_suppression_active.get() {
            return false;
        }
        let Some(config) = &self.program_first_decode_suppression else {
            return false;
        };
        let Some(source_identity) = input.source_identity.as_ref() else {
            return false;
        };
        if source_identity.client_id == config.program_source.client_id
            && source_identity.run_id == config.program_source.run_id
        {
            return false;
        }
    ...
    fn refresh_continuous_timing(&self) {
        if let Some(continuous_slot0) = self.continuous_slot0.borrow().as_ref() {
            continuous_slot0.update_timing_flags(&mut self.timing.borrow_mut());
        }
    }
    ...
    fn feed_slot0_continuous_frame(
        &self,
        ...
        let result = continuous_slot0.enqueue(
            &input,
            &key,
            &mut timing,
            TwoRealContinuousDecodeInputOrigin::Feeder,
...
fn record_slot0_one_shot_suppression(
    timing: &mut TwoRealPreviewLoopRuntimeTiming,
    reason: TwoRealContinuousSlot0OneShotSuppressionReason,
) {
    timing.continuous_decode_slot0_one_shot_suppressed_count = timing
    ...
    timing
        ...
        timing
            ...
            .saturating_add(1);
    match reason {
        TwoRealContinuousSlot0OneShotSuppressionReason::ContinuousNotReady => {
            timing.continuous_decode_slot0_one_shot_suppressed_reason_not_ready_count = timing
                .continuous_decode_slot0_one_shot_suppressed_reason_not_ready_count
                .saturating_add(1);
        }
        TwoRealContinuousSlot0OneShotSuppressionReason::Stale => {
            timing.continuous_decode_slot0_one_shot_suppressed_reason_stale_count = timing
                .continuous_decode_slot0_one_shot_suppressed_reason_stale_count
                .saturating_add(1);
        }
        TwoRealContinuousSlot0OneShotSuppressionReason::Future => {
            timing.continuous_decode_slot0_one_shot_suppressed_reason_future_count = timing
                .continuous_decode_slot0_one_shot_suppressed_reason_future_count
                .saturating_add(1);
        }
    }
}
    ...
    Runtime: SwitcherH264DecodeRuntimeHook,
{
    fn decode_annex_b_h264(&self, input: SwitcherH264DecodeInput) -> SwitcherH264DecodeResult {
        ...
        if let Some(decoded) = self.decoded_cache.borrow().get(&key) {
            ...
            let continuous_cache_hit = self
                .continuous_slot0
                .borrow()
                .as_ref()
                .map(|continuous_slot0| continuous_slot0.is_continuous_decoded_key(&key))
                .unwrap_or(false);
            let mut timing = self.timing.borrow_mut();
            ...
            if continuous_slot0_targeted {
                if continuous_cache_hit {
                    if let Some(continuous_slot0) = self.continuous_slot0.borrow().as_ref() {
                        continuous_slot0.observe_lookup_hit(&mut timing);
                    }
                } else if let Some(continuous_slot0) = self.continuous_slot0.borrow().as_ref() {
        ...
        if continuous_slot0_targeted {
            ...
            let suppression_reason = match bounded_candidate {
                TwoRealContinuousBoundedLookupCandidate::Hit {
                    key: bounded_key,
                    used_frame_id,
                    requested_frame_id,
...
fn two_real_preview_loop_seen_key_from_decode_cache_key(
    key: &TwoRealPreviewLoopDecodeCacheKey,
) -> Option<TwoRealPreviewLoopSeenDecodeCacheKey> {
    let source_identity = key.source_identity.as_ref()?;
    Some(TwoRealPreviewLoopSeenDecodeCacheKey {
        width: key.width,
        height: key.height,
        client_id: source_identity.client_id.clone(),
        run_id: source_identity.run_id.clone(),
        frame_id: source_identity.frame_id,
        source_identity: source_identity.source_identity.clone(),
    })
}

fn record_two_real_decode_attempt_observation(
    timing: &mut TwoRealPreviewLoopRuntimeTiming,
    key: &TwoRealPreviewLoopDecodeCacheKey,
    ...
    timing
        .two_real_decode_attempt_observations
        .push(TwoRealPreviewLoopDecodeAttemptObservation {
            key: two_real_preview_loop_seen_key_from_decode_cache_key(key),
            first_byte_slow: diagnostics.stdout_first_byte_elapsed_ms > slow_threshold_ms,
            output_read_slow: diagnostics.output_read_elapsed_ms > slow_threshold_ms,
            input_write_outlier: diagnostics.input_write_elapsed_ms > slow_threshold_ms,
            input_payload_bytes: diagnostics.input_payload_bytes,
        });
}
...
fn add_decode_runtime_diagnostics(
    ...
    timing.persistent_decode_config_enabled |= diagnostics.persistent_decode_config_enabled;
    ...
    timing.persistent_decode_skipped_by_config_count = timing
        .persistent_decode_skipped_by_config_count
        .saturating_add(diagnostics.persistent_decode_skipped_by_config_count);
    ...
    timing.one_shot_decode_attempt_count = timing
        .one_shot_decode_attempt_count
        .saturating_add(diagnostics.one_shot_decode_attempt_count);
    timing.one_shot_decode_elapsed_ms = timing
        .one_shot_decode_elapsed_ms
        .saturating_add(diagnostics.one_shot_decode_elapsed_ms);
    if timing.continuous_decode_process_running {
        timing.continuous_decode_competing_one_shot_attempt_count = timing
            .continuous_decode_competing_one_shot_attempt_count
            .saturating_add(diagnostics.one_shot_decode_attempt_count);
        timing.continuous_decode_competing_one_shot_decode_elapsed_ms = timing
            .continuous_decode_competing_one_shot_decode_elapsed_ms
            .saturating_add(diagnostics.one_shot_decode_elapsed_ms);
    }
    timing.one_shot_decode_elapsed_ms_max = timing
        .one_shot_decode_elapsed_ms_max
        .max(diagnostics.one_shot_decode_elapsed_ms);
    timing.one_shot_decode_input_write_elapsed_ms = timing
        .one_shot_decode_input_write_elapsed_ms
        .saturating_add(diagnostics.one_shot_decode_input_write_elapsed_ms);
    timing.one_shot_decode_input_write_elapsed_ms_max = timing
        .one_shot_decode_input_write_elapsed_ms_max
        .max(diagnostics.one_shot_decode_input_write_elapsed_ms);
    timing.one_shot_decode_stdin_close_elapsed_ms = timing
        .one_shot_decode_stdin_close_elapsed_ms
        .saturating_add(diagnostics.one_shot_decode_stdin_close_elapsed_ms);
    ...
    timing.one_shot_decode_output_read_elapsed_ms_max = timing
        .one_shot_decode_output_read_elapsed_ms_max
        .max(diagnostics.one_shot_decode_output_read_elapsed_ms);
    timing.one_shot_decode_output_read_exact_elapsed_ms = timing
        .one_shot_decode_output_read_exact_elapsed_ms
        .saturating_add(diagnostics.one_shot_decode_output_read_exact_elapsed_ms);
    timing.one_shot_decode_output_read_exact_elapsed_ms_max = timing
        .one_shot_decode_output_read_exact_elapsed_ms_max
        .max(diagnostics.one_shot_decode_output_read_exact_elapsed_ms);
    timing.one_shot_decode_extra_output_probe_elapsed_ms = timing
    ...
    if diagnostics.one_shot_decode_attempt_count != 0 {
        ...
        timing.one_shot_decode_input_payload_bytes_min =
            Some(match timing.one_shot_decode_input_payload_bytes_min {
                ...
                None => diagnostics.input_payload_bytes,
            });
        timing.one_shot_decode_input_payload_bytes_max = timing
            .one_shot_decode_input_payload_bytes_max
            .max(diagnostics.input_payload_bytes);
        timing.one_shot_decode_input_payload_bytes_total = timing
            .one_shot_decode_input_payload_bytes_total
            .saturating_add(diagnostics.input_payload_bytes);
        timing.one_shot_decode_expected_output_bytes_total = timing
            .one_shot_decode_expected_output_bytes_total
            .saturating_add(diagnostics.output_expected_bytes);
        timing.one_shot_decode_output_width = diagnostics.output_width;
        timing.one_shot_decode_output_height = diagnostics.output_height;
        timing.one_shot_decode_output_pixel_format = diagnostics.output_pixel_format;
        timing.one_shot_decode_scaled_output_enabled = diagnostics.scaled_output_enabled;
        timing.one_shot_decode_scaled_output_reason = diagnostics.scaled_output_reason.clone();
        if diagnostics.input_payload_has_idr {
            timing.one_shot_decode_keyframe_attempt_count = timing
                .one_shot_decode_keyframe_attempt_count
                .saturating_add(diagnostics.one_shot_decode_attempt_count);
            timing.one_shot_decode_keyframe_elapsed_ms = timing
                .one_shot_decode_keyframe_elapsed_ms
                .saturating_add(diagnostics.one_shot_decode_elapsed_ms);
            timing.one_shot_decode_keyframe_input_payload_bytes_total = timing
                .one_shot_decode_keyframe_input_payload_bytes_total
                .saturating_add(diagnostics.input_payload_bytes);
        } else {
            timing.one_shot_decode_non_keyframe_attempt_count = timing
                .one_shot_decode_non_keyframe_attempt_count
                .saturating_add(diagnostics.one_shot_decode_attempt_count);
            timing.one_shot_decode_non_keyframe_elapsed_ms = timing
                .one_shot_decode_non_keyframe_elapsed_ms
                .saturating_add(diagnostics.one_shot_decode_elapsed_ms);
            timing.one_shot_decode_non_keyframe_input_payload_bytes_total = timing
...
fn record_continuous_feed_skip(
    timing: &mut TwoRealPreviewLoopRuntimeTiming,
    reason: TwoRealContinuousFeedSkipReason,
) {
    timing.continuous_feed_skipped_count = timing.continuous_feed_skipped_count.saturating_add(1);
    match reason {
        TwoRealContinuousFeedSkipReason::Duplicate => {
            timing.continuous_feed_skip_reason_duplicate_count = timing
                .continuous_feed_skip_reason_duplicate_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::FutureFrame => {
            timing.continuous_feed_skip_reason_future_frame_count = timing
                .continuous_feed_skip_reason_future_frame_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::RuntimeDisabled => {
            timing.continuous_feed_skip_reason_runtime_disabled_count = timing
                .continuous_feed_skip_reason_runtime_disabled_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::StartupNotReady => {
            timing.continuous_feed_skip_reason_startup_not_ready_count = timing
                .continuous_feed_skip_reason_startup_not_ready_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::InputQueueFull => {
            timing.continuous_feed_skip_reason_input_queue_full_count = timing
                .continuous_feed_skip_reason_input_queue_full_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::SourceMismatch => {
            timing.continuous_feed_skip_reason_source_mismatch_count = timing
                .continuous_feed_skip_reason_source_mismatch_count
                .saturating_add(1);
        }
        TwoRealContinuousFeedSkipReason::ConsumeMismatch => {
            timing.continuous_feed_skip_reason_consume_mismatch_count = timing
                .continuous_feed_skip_reason_consume_mismatch_count
                .saturating_add(1);
        }
    }
}

fn record_continuous_feed_enqueue_result(
    timing: &mut TwoRealPreviewLoopRuntimeTiming,
    frame_id: u64,
    result: TwoRealContinuousDecodeEnqueueResult,
) -> bool {
    match result {
        TwoRealContinuousDecodeEnqueueResult::Enqueued => {
            timing.continuous_feed_enqueued_count =
                timing.continuous_feed_enqueued_count.saturating_add(1);
            timing.continuous_feed_latest_enqueued_frame_id = Some(frame_id);
            true
        }
        TwoRealContinuousDecodeEnqueueResult::SkippedDuplicate => {
            record_continuous_feed_skip(timing, TwoRealContinuousFeedSkipReason::Duplicate);
            false
        }
        TwoRealContinuousDecodeEnqueueResult::SkippedRuntimeDisabled
        | TwoRealContinuousDecodeEnqueueResult::Disconnected => {
            record_continuous_feed_skip(timing, TwoRealContinuousFeedSkipReason::RuntimeDisabled);
            false
        }
        TwoRealContinuousDecodeEnqueueResult::SkippedStartupNotReady => {
            record_continuous_feed_skip(timing, TwoRealContinuousFeedSkipReason::StartupNotReady);
            false
        }
        TwoRealContinuousDecodeEnqueueResult::SkippedSourceMismatch => {
            record_continuous_feed_skip(timing, TwoRealContinuousFeedSkipReason::SourceMismatch);
            false
        }
        TwoRealContinuousDecodeEnqueueResult::DroppedInputQueueFull => {
            timing.continuous_feed_dropped_stale_input_count = timing
                .continuous_feed_dropped_stale_input_count
                .saturating_add(1);
            record_continuous_feed_skip(timing, TwoRealContinuousFeedSkipReason::InputQueueFull);
            false
        }
    }
}

fn run_slot0_continuous_bounded_feed<RealHandoff, DecodeRuntime>(
    handoff: &mut MultiRealAndPlaceholderQueuedFrameHandoff<RealHandoff>,
    timed_decode_runtime: &TimedSwitcherH264DecodeRuntime<'_, DecodeRuntime>,
    timing: &Rc<RefCell<TwoRealPreviewLoopRuntimeTiming>>,
    client_id: &ClientId,
    run_id: &RunId,
    ...
    RealHandoff: PreviewLoopRealHandoff,
{
    if !timed_decode_runtime.slot0_continuous_enabled() {
        return;
    }
    {
        let mut timing = timing.borrow_mut();
        timing.continuous_feed_enabled = true;
        timing.continuous_feed_attempt_count =
            timing.continuous_feed_attempt_count.saturating_add(1);
    }
    for _ in 0..TWO_REAL_CONTINUOUS_FEED_MAX_FRAMES_PER_ATTEMPT {
        {
            let mut timing = timing.borrow_mut();
            timing.continuous_feed_handoff_request_count = timing
                .continuous_feed_handoff_request_count
                .saturating_add(1);
        }
        let preview_result = handoff.read_handoff_frame(SwitcherQueuedFrameHandoffInput {
            client_id: client_id.clone(),
            run_id: run_id.clone(),
            mode: SwitcherSingleClientQueueSourceMode::PreviewOldest,
        });
        let frame = match preview_result {
            SwitcherQueuedFrameHandoffResult::FrameRead { frame, .. } => {
                let mut timing = timing.borrow_mut();
                timing.continuous_feed_frame_received_count = timing
                    .continuous_feed_frame_received_count
                    .saturating_add(1);
                timing.continuous_feed_latest_received_frame_id = Some(frame.frame_id);
                frame
            }
            SwitcherQueuedFrameHandoffResult::NoFrameAvailable { .. } => {
                let mut timing = timing.borrow_mut();
                timing.continuous_feed_no_frame_count =
                    timing.continuous_feed_no_frame_count.saturating_add(1);
                break;
            }
            SwitcherQueuedFrameHandoffResult::HandoffError { .. } => {
                let mut timing = timing.borrow_mut();
                timing.continuous_feed_handoff_error_count =
                    timing.continuous_feed_handoff_error_count.saturating_add(1);
                break;
            }
        };
        if frame.capture_timestamp.0 > target_timestamp.0 {
            record_continuous_feed_skip(
                &mut timing.borrow_mut(),
                TwoRealContinuousFeedSkipReason::FutureFrame,
            );
            break;
        }
        let enqueued = {
            let result = timed_decode_runtime.feed_slot0_continuous_frame(&frame);
            record_continuous_feed_enqueue_result(&mut timing.borrow_mut(), frame.frame_id, result)
        };
        if !enqueued {
            break;
        }
        ...
        match handoff.read_handoff_frame(SwitcherQueuedFrameHandoffInput {
            client_id: client_id.clone(),
            run_id: run_id.clone(),
            ...
            SwitcherQueuedFrameHandoffResult::FrameRead {
                frame: consumed, ..
            } if consumed.frame_id == frame.frame_id => {}
            SwitcherQueuedFrameHandoffResult::FrameRead { .. } => {
                record_continuous_feed_skip(
                    &mut timing.borrow_mut(),
                    TwoRealContinuousFeedSkipReason::ConsumeMismatch,
...
fn run_four_view_two_real_handoff_preview_loop(
    ...
    continuous_decoder_slot0_suppress_one_shot_fallback: bool,
    continuous_decoder_bounded_lookup_allowed_lag_frames: Option<u64>,
    continuous_decoder_output_pipeline_experiment_mode: TwoRealContinuousOutputPipelineExperimentMode,
    program_output_window_enabled: bool,
    program_selected_client_id: Option<ClientId>,
    program_continuous_decode_enabled: bool,
    program_continuous_decode_mode: ProgramContinuousDecodeMode,
    ...
    let render_runtime = SwitcherWindowsGdiPersistentWindowRenderRuntime::default();
    let program_render_runtime = program_output_window_enabled
        .then(SwitcherWindowsGdiPersistentWindowRenderRuntime::default);
    let persistent_decode_runtime =
        SwitcherPersistentFfmpegH264DecodeRuntimeHook::with_config_enabled(
            persistent_decoder_enabled,
        );
...
fn run_four_view_two_real_handoff_preview_loop(
    ...
    _continuous_decoder_output_pipeline_experiment_mode: TwoRealContinuousOutputPipelineExperimentMode,
    ...
    _operator_preview_snapshot_retention_enabled: bool,
) -> Result<SwitcherFourViewTwoRealHandoffPreviewLoopSummary, String> {
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealProgramOutputTickDiagnostics {
    ...
    selected_client_id: Option<ClientId>,
    selected_slot_index: Option<usize>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct TwoRealCachedProgramOutputFrame {
    selection: SwitcherProgramSelection,
...
{
    let selection_mode = if enabled {
        if requested_client_id.is_some() {
            "explicit"
        } else {
            "fallback"
        }
    ...
    if !enabled {
        return TwoRealProgramOutputTickDiagnostics {
            selection_mode,
            requested_client_id: requested_client_id.cloned(),
    ...
    let Some(runtime) = program_render_runtime else {
        return TwoRealProgramOutputTickDiagnostics {
            selection_mode,
            requested_client_id: requested_client_id.cloned(),
            result_kind: "NoRuntime",
            rendered: false,
            missing_selected_source: false,
            missing_selected_source_reason: None,
            reused_previous_frame: false,
    ...
    let (selection, selected_decoded_frame, selected_missing_reason) =
        if let Some(requested_client_id) = requested_client_id {
            requested_program_output_selection_and_frame(
                pre_composition,
                slots,
                real_slot_indices,
                requested_client_id,
    ...
    let result_kind = format_program_output_render_result_kind(&result);
    let render_result_missing_selected_source = matches!(
        result,
        SwitcherProgramOutputRenderResult::MissingSelectedSource {
            reason: SwitcherProgramOutputMissingSelectedSourceReason::NoDecodedFrameForSelection,
            ..
        }
    );
    ...
    TwoRealProgramOutputTickDiagnostics {
        ...
        selected_client_id: Some(selection.selected_client_id),
        selected_slot_index: selection.selected_slot_index,
    }
}

fn cached_program_frame_matches_selection(
    cached: &TwoRealCachedProgramOutputFrame,
    selection: &SwitcherProgramSelection,
) -> bool {
    cached.selection.selected_client_id == selection.selected_client_id
        && selection
            .selected_run_id
            .as_ref()
...
fn fallback_program_output_selection_and_frame<'a>(
    pre_composition: &'a SwitcherFourViewHandoffValidationPreCompositionOutput,
    slots: &[SwitcherFourViewTargetTimeSourceSlotConfig; 4],
    real_slot_indices: [usize; 2],
) -> (
    SwitcherProgramSelection,
    Option<&'a stream_sync_switcher::SwitcherDecodedFrame>,
    Option<&'static str>,
) {
    first_available_program_output_selection_and_frame(pre_composition)
        .map(|(selection, decoded)| (selection, Some(decoded), None))
        .unwrap_or_else(|| {
            (
                fallback_program_selection_from_real_slots(slots, real_slot_indices),
                None,
                Some(format_program_output_missing_selected_source_reason(
                    &SwitcherProgramOutputMissingSelectedSourceReason::NoDecodedFrameForSelection,
                )),
...
fn requested_program_output_selection_and_frame<'a>(
    pre_composition: &'a SwitcherFourViewHandoffValidationPreCompositionOutput,
    slots: &[SwitcherFourViewTargetTimeSourceSlotConfig; 4],
    real_slot_indices: [usize; 2],
    ...
    let requested_selection =
        requested_program_selection_from_real_slots(slots, real_slot_indices, requested_client_id);
    let selected = pre_composition
        .composition_render
        .composition
        .slots
        .iter()
        .filter_map(updated_program_output_selection_and_frame_from_slot)
        .find(|(selection, _)| selection.selected_client_id == *requested_client_id);
    if let Some((decoded_selection, decoded)) = selected {
        return (
            SwitcherProgramSelection {
                selected_client_id: requested_client_id.clone(),
                selected_run_id: requested_selection
                    .selected_run_id
                    .clone()
                    .or(decoded_selection.selected_run_id),
                selected_slot_index: requested_selection.selected_slot_index,
            },
            Some(decoded),
            None,
        );
    }
    let missing_reason = if requested_selection.selected_slot_index.is_some() {
        format_program_output_missing_selected_source_reason(
            &SwitcherProgramOutputMissingSelectedSourceReason::NoDecodedFrameForSelection,
        )
    } else {
        "RequestedClientNotInRealSlots"
    };
    (requested_selection, None, Some(missing_reason))
}

fn first_available_program_output_selection_and_frame(
    pre_composition: &SwitcherFourViewHandoffValidationPreCompositionOutput,
) -> Option<(
    SwitcherProgramSelection,
    &stream_sync_switcher::SwitcherDecodedFrame,
)> {
    pre_composition
        .composition_render
        .composition
        .slots
        .iter()
        .find_map(program_output_selection_and_frame_from_slot)
}

fn program_output_selection_and_frame_from_slot(
    ...
    match slot {
        SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame {
            ...
            selected,
            ..
        } => frame.decoded.as_ref().map(|decoded| {
            (
                SwitcherProgramSelection {
                    selected_client_id: selected.frame.client_id.clone(),
                    selected_run_id: Some(selected.frame.run_id.clone()),
                    selected_slot_index: Some(frame.slot_index),
                },
                decoded,
            )
        }),
...
fn updated_program_output_selection_and_frame_from_slot(
    slot: &SwitcherFourViewHandoffQuadCompositionRenderSlot,
) -> Option<(
    SwitcherProgramSelection,
    &stream_sync_switcher::SwitcherDecodedFrame,
...
fn fallback_program_selection_from_real_slots(
    slots: &[SwitcherFourViewTargetTimeSourceSlotConfig; 4],
    real_slot_indices: [usize; 2],
) -> SwitcherProgramSelection {
    let fallback_slot_index = real_slot_indices[0].min(real_slot_indices[1]);
    let slot = &slots[fallback_slot_index];
    SwitcherProgramSelection {
        selected_client_id: slot.client_id.clone(),
        selected_run_id: Some(slot.run_id.clone()),
        selected_slot_index: Some(slot.slot_index),
    }
}

fn requested_program_selection_from_real_slots(
    slots: &[SwitcherFourViewTargetTimeSourceSlotConfig; 4],
    real_slot_indices: [usize; 2],
    requested_client_id: &ClientId,
) -> SwitcherProgramSelection {
    let matched_slot = real_slot_indices
        .iter()
        .copied()
        .filter_map(|slot_index| slots.get(slot_index))
        .find(|slot| slot.client_id == *requested_client_id);
    if let Some(slot) = matched_slot {
        return SwitcherProgramSelection {
            selected_client_id: requested_client_id.clone(),
            selected_run_id: Some(slot.run_id.clone()),
            selected_slot_index: Some(slot.slot_index),
        };
...
fn program_continuous_decode_source_from_real_slots(
    slots: &[SwitcherFourViewTargetTimeSourceSlotConfig; 4],
    real_slot_indices: [usize; 2],
    requested_client_id: Option<&ClientId>,
) -> Option<TwoRealContinuousDecodeSource> {
    let requested_client_id = requested_client_id?;
    real_slot_indices
        .iter()
        .copied()
        .filter_map(|slot_index| slots.get(slot_index))
        .find(|slot| slot.client_id == *requested_client_id)
        .map(|slot| TwoRealContinuousDecodeSource {
            client_id: slot.client_id.clone(),
            run_id: slot.run_id.clone(),
        })
}
...
fn format_program_output_render_result_kind(
    ...
    match result {
        SwitcherProgramOutputRenderResult::Rendered { .. } => "Rendered",
        SwitcherProgramOutputRenderResult::MissingSelectedSource { .. } => "MissingSelectedSource",
        SwitcherProgramOutputRenderResult::RenderDeferred { .. } => "RenderDeferred",
        SwitcherProgramOutputRenderResult::BackendUnavailable { .. } => "BackendUnavailable",
        SwitcherProgramOutputRenderResult::InvalidSelectedFrame { .. } => "InvalidSelectedFrame",
        SwitcherProgramOutputRenderResult::RenderFailed { .. } => "RenderFailed",
    }
}

fn format_program_output_missing_selected_source_reason(
...
fn count_two_real_preview_renderable_slots(
    pre_composition: &SwitcherFourViewHandoffValidationPreCompositionOutput,
) -> usize {
    pre_composition
        .composition_render
...
fn refresh_two_real_preview_composition_renderability(
    pre_composition: &mut SwitcherFourViewHandoffValidationPreCompositionOutput,
) {
    let renderable_slot_count = count_two_real_preview_renderable_slots(pre_composition);
    pre_composition.composition_render.render = if renderable_slot_count == 0 {
        stream_sync_switcher::SwitcherFourViewHandoffQuadCompositionRenderConnectionRenderResult::NoRenderableQuadView {
            non_renderable_slot_count: pre_composition.composition_render.composition.slots.len(),
        }
    } else {
        stream_sync_switcher::SwitcherFourViewHandoffQuadCompositionRenderConnectionRenderResult::CompositionReady {
...
#[derive(Debug, Default, Clone, PartialEq, Eq)]
struct TwoRealOperatorPreviewSnapshotRetentionDiagnostics {
    reuse_count: u32,
    reuse_slot_counts: [u32; 4],
    placeholder_avoided_by_snapshot_count: u32,
    slot_black_after_snapshot_count: u32,
}

#[derive(Debug, Default, Clone, PartialEq, Eq)]
struct TwoRealOperatorPreviewSnapshotCreationDiagnostics {
    created_count: u32,
    created_slot_counts: [u32; 4],
}

fn four_view_displayed_slot_has_decoded_frame(slot: &SwitcherFourViewDisplayedSlot) -> bool {
    slot.decoded.is_some()
}

fn four_view_render_slot_is_renderable(
    slot: &SwitcherFourViewHandoffQuadCompositionRenderSlot,
) -> bool {
    matches!(
        slot,
        SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame { frame, .. }
            | SwitcherFourViewHandoffQuadCompositionRenderSlot::UseHeldPreviousFrame {
                frame,
                ..
            } if four_view_displayed_slot_has_decoded_frame(frame)
    )
}

fn apply_operator_preview_snapshot_retention(
    pre_composition: &mut SwitcherFourViewHandoffValidationPreCompositionOutput,
    snapshots: &[Option<SwitcherFourViewDisplayedSlot>; 4],
) -> TwoRealOperatorPreviewSnapshotRetentionDiagnostics {
    let mut diagnostics = TwoRealOperatorPreviewSnapshotRetentionDiagnostics::default();
    for slot_index in 0..4 {
        let Some(snapshot) = snapshots[slot_index]
            .as_ref()
            .filter(|snapshot| four_view_displayed_slot_has_decoded_frame(snapshot))
            .cloned()
        else {
            continue;
        };
        let slot = &mut pre_composition.composition_render.composition.slots[slot_index];
        if four_view_render_slot_is_renderable(slot) {
            continue;
        }

        let replacement = match slot.clone() {
            SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame {
                placement,
                selected,
                consumed,
        ...
        if let Some((placement, skipped)) = replacement {
            *slot = SwitcherFourViewHandoffQuadCompositionRenderSlot::UseHeldPreviousFrame {
                placement,
                frame: snapshot,
                skipped,
                hold_duration_micros: 0,
            };
            diagnostics.reuse_count = diagnostics.reuse_count.saturating_add(1);
            diagnostics.reuse_slot_counts[slot_index] =
                diagnostics.reuse_slot_counts[slot_index].saturating_add(1);
            diagnostics.placeholder_avoided_by_snapshot_count = diagnostics
                .placeholder_avoided_by_snapshot_count
                .saturating_add(1);
        }
    }

    if diagnostics.reuse_count > 0 {
        refresh_two_real_preview_composition_renderability(pre_composition);
    }

    for slot_index in 0..4 {
        if snapshots[slot_index]
            .as_ref()
            .is_some_and(four_view_displayed_slot_has_decoded_frame)
            && !four_view_render_slot_is_renderable(
                &pre_composition.composition_render.composition.slots[slot_index],
            )
        {
            diagnostics.slot_black_after_snapshot_count = diagnostics
                .slot_black_after_snapshot_count
                .saturating_add(1);
        }
    }
...
fn operator_preview_snapshot_creation_diagnostics(
    previous_snapshots: &[Option<SwitcherFourViewDisplayedSlot>; 4],
    next_snapshots: &[Option<SwitcherFourViewDisplayedSlot>; 4],
) -> TwoRealOperatorPreviewSnapshotCreationDiagnostics {
    let mut diagnostics = TwoRealOperatorPreviewSnapshotCreationDiagnostics::default();
    for slot_index in 0..4 {
        let Some(next_snapshot) = next_snapshots[slot_index]
            .as_ref()
            .filter(|snapshot| four_view_displayed_slot_has_decoded_frame(snapshot))
        else {
            continue;
        };
        if previous_snapshots[slot_index].as_ref() != Some(next_snapshot) {
            diagnostics.created_count = diagnostics.created_count.saturating_add(1);
            diagnostics.created_slot_counts[slot_index] =
                diagnostics.created_slot_counts[slot_index].saturating_add(1);
        }
    }
    diagnostics
}

fn operator_preview_program_slot_visible(
    decoded_slot_identities: &[Option<TwoRealPreviewLoopDecodedSlotIdentity>; 4],
    program_selection: Option<&SwitcherProgramSelection>,
) -> bool {
    let Some(selection) = program_selection else {
        return false;
    };
    let Some(slot_index) = selection.selected_slot_index else {
        return false;
    };
    let Some(identity) = decoded_slot_identities
        .get(slot_index)
        .and_then(Option::as_ref)
    else {
        return false;
    };
    identity.client_id == selection.selected_client_id
        && selection
            .selected_run_id
            .as_ref()
            .map(|run_id| identity.run_id == *run_id)
            .unwrap_or(true)
}

fn operator_preview_program_reuse_candidate<'a>(
    program_selection: &SwitcherProgramSelection,
    program_continuous_latest_frame: Option<&'a TwoRealProgramContinuousLatestFrame>,
    last_valid_program_frame: Option<&'a TwoRealCachedProgramOutputFrame>,
) -> (
    Option<&'a stream_sync_switcher::SwitcherDecodedFrame>,
    TwoRealOperatorPreviewProgramReuseSource,
) {
    if let Some(latest) = program_continuous_latest_frame
        .filter(|latest| latest_program_frame_matches_selection(latest, program_selection))
    {
        return (
            Some(&latest.decoded_frame),
            TwoRealOperatorPreviewProgramReuseSource::ContinuousLatest,
        );
    }
    if let Some(cached) = last_valid_program_frame
        .filter(|cached| cached_program_frame_matches_selection(cached, program_selection))
    {
        return (
            Some(&cached.decoded_frame),
            TwoRealOperatorPreviewProgramReuseSource::LastValidProgram,
        );
    }
    (None, TwoRealOperatorPreviewProgramReuseSource::None)
}

fn reuse_program_frame_for_operator_preview_slot(
    pre_composition: &mut SwitcherFourViewHandoffValidationPreCompositionOutput,
    program_selection: &SwitcherProgramSelection,
    decoded_frame: &stream_sync_switcher::SwitcherDecodedFrame,
) -> bool {
    let Some(slot_index) = program_selection.selected_slot_index else {
        return false;
    };
    let Some(slot) = pre_composition
        .composition_render
        .composition
        .slots
        .get_mut(slot_index)
    ...
    match slot {
        SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame {
            ...
            selected,
            ..
        } if selected.frame.client_id == program_selection.selected_client_id
            && program_selection
                .selected_run_id
                .as_ref()
                .map(|run_id| selected.frame.run_id == *run_id)
                .unwrap_or(true) =>
        {
            if frame.decoded.is_none() {
                frame.decoded = Some(decoded_frame.clone());
                reused = true;
            }
        }
        SwitcherFourViewHandoffQuadCompositionRenderSlot::UseDecodeDeferredPlaceholder {
                ...
                .unwrap_or(true) =>
        {
            ...
            *slot = SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame {
                placement,
                frame: SwitcherFourViewDisplayedSlot {
                    slot_index: placement.slot_index,
                    selected: Some(selected.clone()),
                    decoded: Some(decoded_frame.clone()),
                    displayed_at: TimestampMicros(selected.frame.capture_timestamp.0),
                },
                selected,
                consumed,
            };
    ...
    TargetTimestampHook: FnMut() -> TimestampMicros,
{
    ...
    let mut clean_output_render_result_kind = "NoRenderableQuadView";
    let mut window_title = SWITCHER_FOUR_VIEW_CLEAN_OUTPUT_WINDOW_TITLE.to_string();
    let mut program_output_selection_mode = if program_output_window_enabled {
        if program_selected_client_id.is_some() {
            "explicit"
        } else {
            "fallback"
        }
    ...
    let mut program_output_requested_client_id = program_selected_client_id.clone();
    let mut program_output_render_count = 0u32;
    let mut program_output_missing_selected_source_count = 0u32;
    let mut program_output_missing_before_first_render_count = 0u32;
    let mut program_output_missing_after_first_render_count = 0u32;
    let mut program_output_reused_previous_frame_count = 0u32;
    let mut program_output_placeholder_render_count = 0u32;
    let mut program_output_black_frame_render_count = 0u32;
    ...
    let mut program_output_selected_client_id = None;
    let mut program_output_selected_slot_index = None;
    let mut preview_compose_skipped_for_program_count = 0u32;
    let mut preview_compose_reused_for_program_count = 0u32;
    let operator_low_cost_preview_enabled =
        program_first_validation_mode && program_first_preview_refresh_interval.is_some();
    let mut operator_preview_refresh_attempt_count = 0u32;
    let mut operator_preview_refresh_success_count = 0u32;
    ...
    let operator_preview_snapshot_retention_active =
        operator_low_cost_preview_enabled && operator_preview_snapshot_retention_enabled;
    let mut operator_preview_decode_refresh_skipped_count = 0u32;
    let mut operator_preview_non_program_visible_count = 0u32;
    let mut operator_preview_reused_program_frame_count = 0u32;
    let mut operator_preview_program_slot_visible_count = 0u32;
    let mut operator_preview_program_slot_black_count = 0u32;
    let mut operator_preview_snapshot_reuse_count = 0u32;
    let mut operator_preview_snapshot_reuse_slot_counts = [0u32; 4];
    let mut operator_preview_snapshot_created_count = 0u32;
    let mut operator_preview_snapshot_created_slot_counts = [0u32; 4];
    let mut operator_preview_placeholder_avoided_by_snapshot_count = 0u32;
    let mut operator_preview_slot_black_after_snapshot_count = 0u32;
    let mut operator_preview_program_slot_reuse_source =
        TwoRealOperatorPreviewProgramReuseSource::None;
    ...
    let program_continuous_decode_source =
        if program_continuous_decode_enabled && program_output_window_enabled {
            program_continuous_decode_source_from_real_slots(
                &slots,
    ...
    let continuous_decode_source = program_continuous_decode_source.clone().or_else(|| {
        continuous_stream_decoder_enabled.then(|| TwoRealContinuousDecodeSource {
            client_id: client0_id.clone(),
            run_id: run0_id.clone(),
        })
    });
    let continuous_decode_config_enabled = continuous_decode_source.is_some();
    let continuous_feed_source = continuous_decode_source.clone();
    let program_first_decode_suppression =
        if program_first_validation_mode && program_output_window_enabled {
            program_continuous_decode_source
                .clone()
                .map(
                    |program_source| TwoRealProgramFirstDecodeSuppressionConfig {
                        program_source,
                        slot0_source: TwoRealContinuousDecodeSource {
                            client_id: client0_id.clone(),
                            run_id: run0_id.clone(),
    ...
    let mut last_valid_program_frame: Option<TwoRealCachedProgramOutputFrame> = None;
    ...
    for frame_index in 0..frames.get() {
        ...
        let operator_preview_refresh_this_tick = should_attempt_operator_low_cost_preview_refresh(
            operator_low_cost_preview_enabled,
            program_first_preview_refresh_interval,
            frame_index,
        );
        ...
        if let Some(continuous_feed_source) = continuous_feed_source.as_ref() {
            run_slot0_continuous_bounded_feed(
                ...
                &timing,
                &continuous_feed_source.client_id,
                &continuous_feed_source.run_id,
                target_timestamp,
            );
        }
        timed_decode_runtime.set_program_first_decode_suppression_active(
            program_first_validation_mode
                && program_output_window_enabled
        ...
        let decode_attempt_observation_delta = &timing_after_validation
            .two_real_decode_attempt_observations[timing_before_validation
            .two_real_decode_attempt_observations
            .len()..];
        ...
        if operator_preview_refresh_this_tick {
            operator_preview_refresh_attempt_count =
                operator_preview_refresh_attempt_count.saturating_add(1);
            let slot0_one_shot_delta = timing_after_validation
                .one_shot_decode_attempt_slot0_count
                .saturating_sub(timing_before_validation.one_shot_decode_attempt_slot0_count);
            let slot1_one_shot_delta = timing_after_validation
                .one_shot_decode_attempt_slot1_count
                .saturating_sub(timing_before_validation.one_shot_decode_attempt_slot1_count);
            let non_program_one_shot_delta =
                if let Some(program_source) = program_continuous_decode_source.as_ref() {
                    let slot0_is_program_source =
                        program_source.client_id == client0_id && program_source.run_id == run0_id;
                    let slot1_is_program_source =
                        program_source.client_id == client1_id && program_source.run_id == run1_id;
                    if slot0_is_program_source {
                        slot1_one_shot_delta
                    } else if slot1_is_program_source {
                        slot0_one_shot_delta
                    } else {
                        slot0_one_shot_delta.saturating_add(slot1_one_shot_delta)
                    }
                } else {
                    slot0_one_shot_delta.saturating_add(slot1_one_shot_delta)
                };
            operator_preview_forced_one_shot_decode_count =
                operator_preview_forced_one_shot_decode_count
                    .saturating_add(non_program_one_shot_delta);
        }
        ...
        if program_output_window_enabled {
            program_render_loop_attempt_count = program_render_loop_attempt_count.saturating_add(1);
        }
        let program_had_first_render = program_output_first_render_attempt_index.is_some();
        if program_output_tick.rendered {
            program_output_render_count = program_output_render_count.saturating_add(1);
            program_window_render_success_count =
                program_window_render_success_count.saturating_add(1);
            if program_output_first_render_attempt_index.is_none() {
                program_output_first_render_attempt_index = Some(frame_index + 1);
                program_output_first_render_elapsed_ms = Some(loop_start.elapsed().as_millis());
            }
        } else if program_output_window_enabled {
            program_window_render_failure_count =
                program_window_render_failure_count.saturating_add(1);
        }
        if program_output_tick.missing_selected_source {
            program_output_missing_selected_source_count =
                program_output_missing_selected_source_count.saturating_add(1);
            if program_had_first_render {
                program_output_missing_after_first_render_count =
                    program_output_missing_after_first_render_count.saturating_add(1);
            } else {
                program_output_missing_before_first_render_count =
                    program_output_missing_before_first_render_count.saturating_add(1);
            }
        }
        ...
        if program_output_tick.placeholder_rendered {
            program_output_placeholder_render_count =
                program_output_placeholder_render_count.saturating_add(1);
        }
        if program_output_tick.black_frame_rendered {
            program_output_black_frame_render_count =
                program_output_black_frame_render_count.saturating_add(1);
        }
        if let Some(latest_valid_frame) = program_output_tick.latest_valid_frame.clone() {
            last_valid_program_frame = Some(latest_valid_frame);
        }
        program_output_last_result_kind = program_output_tick.result_kind;
        program_output_selection_mode = program_output_tick.selection_mode;
        program_output_requested_client_id = program_output_tick.requested_client_id;
        if program_output_tick.missing_selected_source_reason.is_some() {
            program_output_missing_selected_source_reason =
                program_output_tick.missing_selected_source_reason;
        }
        program_output_selected_client_id = program_output_tick.selected_client_id;
        program_output_selected_slot_index = program_output_tick.selected_slot_index;
        let explicit_program_selection = program_selected_client_id.as_ref().map(|client_id| {
            requested_program_selection_from_real_slots(
                &slots,
                [slot0_index, slot1_index],
                client_id,
            )
        });
        let mut preview_pre_composition = pre_composition.clone();
        if operator_low_cost_preview_enabled
            && program_first_validation_mode
            && program_output_window_enabled
        {
            if let Some(program_selection) = explicit_program_selection.as_ref() {
                let (reuse_candidate, reuse_source) = operator_preview_program_reuse_candidate(
                    program_selection,
                    program_continuous_latest_frame.as_ref(),
                    last_valid_program_frame.as_ref(),
                );
                if let Some(decoded_frame) = reuse_candidate {
                    if reuse_program_frame_for_operator_preview_slot(
                        &mut preview_pre_composition,
                        program_selection,
                        decoded_frame,
                    ) {
                        operator_preview_reused_program_frame_count =
                            operator_preview_reused_program_frame_count.saturating_add(1);
                        if reuse_source
                            == TwoRealOperatorPreviewProgramReuseSource::ContinuousLatest
                            || operator_preview_program_slot_reuse_source
                                == TwoRealOperatorPreviewProgramReuseSource::None
                        {
                            operator_preview_program_slot_reuse_source = reuse_source;
                        }
                    }
                }
            }
        }
        if operator_preview_snapshot_retention_active {
            let snapshot_diagnostics = apply_operator_preview_snapshot_retention(
                &mut preview_pre_composition,
                &previous_slots,
            );
            operator_preview_snapshot_reuse_count = operator_preview_snapshot_reuse_count
                .saturating_add(snapshot_diagnostics.reuse_count);
            for slot_index in 0..4 {
                operator_preview_snapshot_reuse_slot_counts[slot_index] =
                    operator_preview_snapshot_reuse_slot_counts[slot_index]
                        .saturating_add(snapshot_diagnostics.reuse_slot_counts[slot_index]);
            }
            operator_preview_placeholder_avoided_by_snapshot_count =
                operator_preview_placeholder_avoided_by_snapshot_count
                    .saturating_add(snapshot_diagnostics.placeholder_avoided_by_snapshot_count);
            operator_preview_slot_black_after_snapshot_count =
                operator_preview_slot_black_after_snapshot_count
                    .saturating_add(snapshot_diagnostics.slot_black_after_snapshot_count);
        }
        let current_slot_identities = update_two_real_decoded_slot_identities(
            ...
            &preview_pre_composition,
        );
        if operator_low_cost_preview_enabled {
            let program_slot_visible = operator_preview_program_slot_visible(
                &current_slot_identities,
                explicit_program_selection.as_ref(),
            );
            operator_preview_program_slot_visible_count = u32::from(program_slot_visible);
            if explicit_program_selection.is_some() && !program_slot_visible {
                operator_preview_program_slot_black_count =
                    operator_preview_program_slot_black_count.saturating_add(1);
            }
        }
        ...
        let current_visual_identities = two_real_preview_loop_visual_identities(
            &current_slot_identities,
            &preview_pre_composition,
        );
        ...
        let can_reuse_preview_for_program = program_first_validation_mode
            && program_output_window_enabled
            && previous_clean_output.is_some()
            && !operator_preview_refresh_this_tick;

        let mut render_facing_for_next: Option<
            stream_sync_switcher::SwitcherFourViewQuadRenderFacingConnectionOutput,
        > = None;
        let clean_output = if can_reuse_preview_for_program {
            preview_compose_skipped_for_program_count =
                preview_compose_skipped_for_program_count.saturating_add(1);
            preview_compose_reused_for_program_count =
                preview_compose_reused_for_program_count.saturating_add(1);
            if operator_low_cost_preview_enabled {
                operator_preview_refresh_skipped_count =
                    operator_preview_refresh_skipped_count.saturating_add(1);
                if previous_clean_output_is_rendered {
                    operator_preview_used_stale_frame_count =
                        operator_preview_used_stale_frame_count.saturating_add(1);
                }
            }
            ...
            previous_clean_output
                .clone()
                .expect("program-first preview reuse should have previous clean output")
        } else if can_reuse_rendered_output {
            if operator_low_cost_preview_enabled && operator_preview_refresh_this_tick {
                operator_preview_used_stale_frame_count =
                    operator_preview_used_stale_frame_count.saturating_add(1);
            }
            render_reuse_frame_count += 1;
                ...
                if let Some(previous_composition) = previous_bgra_composition.clone() {
                    ...
                    (
                        SwitcherFourViewQuadCompositionOutput {
                            ...
                            connection: preview_pre_composition.composition_render.clone(),
            ...
            .saturating_sub(timing_before_validation.decode_attempt_count);
        {
            ...
            record_two_real_decode_attempt_diagnostics(
                ...
                decode_attempt_observation_delta,
                &unchanged_reuse_slots,
                &previous_slot_identities,
        ...
        let tick_diagnostics = four_view_two_real_tick_diagnostics_from_composition_render(
            &slot_result_kinds,
            &preview_pre_composition.composition_render,
            &clean_output,
        );
                ...
                SwitcherFourViewComposedCanvasRenderResult::Rendered { .. } => {
                    ...
                    if operator_preview_refresh_this_tick {
                        operator_preview_refresh_success_count =
                            operator_preview_refresh_success_count.saturating_add(1);
                    }
        ...
        let next_previous_slots = update_four_view_previous_slots_from_composition_render(
            &previous_slots,
            &preview_pre_composition.composition_render,
        );
        if operator_preview_snapshot_retention_active {
            let snapshot_creation_diagnostics = operator_preview_snapshot_creation_diagnostics(
                &previous_slots,
                &next_previous_slots,
            );
            operator_preview_snapshot_created_count = operator_preview_snapshot_created_count
                .saturating_add(snapshot_creation_diagnostics.created_count);
            for slot_index in 0..4 {
                operator_preview_snapshot_created_slot_counts[slot_index] =
                    operator_preview_snapshot_created_slot_counts[slot_index].saturating_add(
                        snapshot_creation_diagnostics.created_slot_counts[slot_index],
                    );
            }
        }
        previous_slots = next_previous_slots;
    ...
    if let Some(program_render_runtime) = program_render_runtime {
        program_render_runtime.close_persistent_window();
    }
    render_runtime.close_persistent_window();
    timed_decode_runtime.refresh_continuous_timing();
    let render_metadata = obs_runtime.metadata_snapshot();
    let lifecycle = render_runtime.lifecycle_snapshot();
    ...
    let effective_program_render_fps =
        format_preview_loop_fps(program_window_render_success_count, loop_total_elapsed_ms);
    let operator_preview_render_effective_fps = if operator_low_cost_preview_enabled {
    ...
    SwitcherFourViewTwoRealHandoffPreviewLoopSummary {
        ...
        persistent_decode_config_enabled: timing.persistent_decode_config_enabled,
        ...
        persistent_decode_disabled_after_failure_count: timing
            .persistent_decode_disabled_after_failure_count,
        persistent_decode_skipped_by_config_count: timing.persistent_decode_skipped_by_config_count,
        ...
        continuous_decode_config_enabled: timing.continuous_decode_config_enabled,
        continuous_decode_runtime_enabled: timing.continuous_decode_runtime_enabled,
        continuous_decode_slot0_enabled: timing.continuous_decode_slot0_enabled,
        continuous_decode_input_frame_count: timing.continuous_decode_input_frame_count,
        continuous_decode_output_frame_count: timing.continuous_decode_output_frame_count,
        continuous_decode_queue_len: timing.continuous_decode_queue_len,
        continuous_decode_dropped_stale_count: timing.continuous_decode_dropped_stale_count,
        continuous_decode_frame_id_lag: timing.continuous_decode_frame_id_lag,
        continuous_decode_stdout_read_elapsed_ms: timing.continuous_decode_stdout_read_elapsed_ms,
        continuous_decode_stall_count: timing.continuous_decode_stall_count,
        continuous_decode_restart_count: timing.continuous_decode_restart_count,
        continuous_decode_runtime_disabled: timing.continuous_decode_runtime_disabled,
        continuous_decode_runtime_disabled_reason: timing
            .continuous_decode_runtime_disabled_reason
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_fallback_to_one_shot_count: timing
            .continuous_decode_fallback_to_one_shot_count,
        continuous_decode_lookup_hit_count: timing.continuous_decode_lookup_hit_count,
        continuous_decode_lookup_miss_count: timing.continuous_decode_lookup_miss_count,
        continuous_decode_lookup_miss_reason_counts:
            format_continuous_decode_lookup_miss_reason_counts(
                timing.continuous_decode_lookup_miss_reason_exact_key_missing_count,
                timing.continuous_decode_lookup_miss_reason_queue_empty_count,
                timing.continuous_decode_lookup_miss_reason_runtime_disabled_count,
                timing.continuous_decode_lookup_miss_reason_output_pending_count,
                timing.continuous_decode_lookup_miss_reason_frame_id_lagging_count,
                timing.continuous_decode_lookup_miss_reason_unknown_count,
            ),
        continuous_decode_requested_frame_id: timing.continuous_decode_requested_frame_id,
        continuous_decode_latest_decoded_frame_id: timing.continuous_decode_latest_decoded_frame_id,
        continuous_decode_requested_minus_latest_lag: timing
            .continuous_decode_requested_minus_latest_lag,
        continuous_decode_queue_oldest_frame_id: timing.continuous_decode_queue_oldest_frame_id,
        continuous_decode_queue_newest_frame_id: timing.continuous_decode_queue_newest_frame_id,
        continuous_decode_input_frame_id_min: timing.continuous_decode_input_frame_id_min,
        continuous_decode_input_frame_id_max: timing.continuous_decode_input_frame_id_max,
        continuous_decode_output_frame_id_min: timing.continuous_decode_output_frame_id_min,
        continuous_decode_output_frame_id_max: timing.continuous_decode_output_frame_id_max,
        continuous_decode_output_pending_correspondence_count: timing
            .continuous_decode_output_pending_correspondence_count,
        continuous_decode_pending_correspondence_count: timing
            .continuous_decode_output_pending_correspondence_count,
        continuous_decode_pending_correspondence_age_ms_max: timing
            .continuous_decode_pending_correspondence_age_ms_max,
        continuous_decode_pending_correspondence_age_ms_avg: format_preview_loop_average_elapsed(
            timing.continuous_decode_pending_correspondence_age_ms_total,
            timing.continuous_decode_pending_correspondence_age_sample_count,
        ),
        continuous_decode_pending_correspondence_oldest_frame_id: timing
        ...
        continuous_decode_exact_match_required_count: timing
            .continuous_decode_exact_match_required_count,
        continuous_decode_stale_frame_available_count: timing
            .continuous_decode_stale_frame_available_count,
        continuous_decode_input_frame_id_gap_max: timing.continuous_decode_input_frame_id_gap_max,
        continuous_decode_input_frame_id_gap_total: timing
            .continuous_decode_input_frame_id_gap_total,
        continuous_decode_input_non_consecutive_count: timing
            .continuous_decode_input_non_consecutive_count,
        continuous_decode_input_keyframe_count: timing.continuous_decode_input_keyframe_count,
        continuous_decode_input_non_keyframe_count: timing
            .continuous_decode_input_non_keyframe_count,
        continuous_decode_input_has_sps_count: timing.continuous_decode_input_has_sps_count,
        continuous_decode_input_has_pps_count: timing.continuous_decode_input_has_pps_count,
        continuous_decode_input_has_idr_count: timing.continuous_decode_input_has_idr_count,
        continuous_decode_input_has_non_idr_vcl_count: timing
            .continuous_decode_input_has_non_idr_vcl_count,
        continuous_decode_last_input_payload_nal_kinds: timing
            .continuous_decode_last_input_payload_nal_kinds
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_ffmpeg_stderr_summary: timing
            .continuous_decode_ffmpeg_stderr_summary
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_stdout_reader_blocked_count: timing
            .continuous_decode_stdout_reader_blocked_count,
        continuous_decode_no_output_after_input_count: timing
            .continuous_decode_no_output_after_input_count,
        continuous_decode_no_output_after_keyframe_count: timing
            .continuous_decode_no_output_after_keyframe_count,
        continuous_decode_bootstrap_input_count: timing.continuous_decode_bootstrap_input_count,
        continuous_decode_bootstrap_output_count: timing.continuous_decode_bootstrap_output_count,
        continuous_decode_last_input_frame_id: timing.continuous_decode_last_input_frame_id,
        continuous_decode_last_output_frame_id: timing.continuous_decode_last_output_frame_id,
        continuous_decode_ffmpeg_args_summary: timing
        ...
        continuous_decode_last_stdin_write_error: timing
            .continuous_decode_last_stdin_write_error
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_process_running: timing.continuous_decode_process_running,
        continuous_decode_process_exit_status: timing
        ...
        continuous_decode_last_input_write_elapsed_ms: timing
            .continuous_decode_last_input_write_elapsed_ms,
        continuous_decode_last_input_payload_bytes: timing
            .continuous_decode_last_input_payload_bytes,
        continuous_decode_first_input_to_first_output_elapsed_ms: timing
        ...
        continuous_decode_latest_input_to_output_frame_gap: timing
            .continuous_decode_latest_input_minus_latest_output_lag,
        ...
        continuous_decode_latest_selected_to_output_frame_gap: timing
            .continuous_decode_output_lag_to_selected_frames,
        ...
        continuous_decode_reader_full_frame_elapsed_ms_max: timing
            .continuous_decode_reader_full_frame_elapsed_ms_max,
        continuous_decode_reader_full_frame_elapsed_ms_avg: format_preview_loop_average_elapsed(
            timing.continuous_decode_stdout_read_elapsed_ms,
            timing.continuous_decode_output_frame_count,
        ),
        continuous_decode_reader_full_frame_slow_count: timing
            .continuous_decode_reader_full_frame_slow_count,
        continuous_decode_reader_full_frame_slow_threshold_ms:
            TWO_REAL_CONTINUOUS_READER_FULL_FRAME_SLOW_THRESHOLD_MS,
        continuous_decode_output_bytes_total: timing.continuous_decode_output_bytes_total,
        continuous_decode_output_bytes_per_sec: format_preview_loop_bytes_per_sec(
            timing.continuous_decode_output_bytes_total,
            timing
                .continuous_decode_first_input_to_now_elapsed_ms
                .unwrap_or(0),
        ),
        continuous_decode_output_frame_interval_ms_avg: format_preview_loop_average_elapsed(
            timing.continuous_decode_output_frame_interval_elapsed_ms,
            timing.continuous_decode_output_frame_interval_count,
        ),
        continuous_decode_output_frame_interval_ms_max: timing
            .continuous_decode_output_frame_interval_ms_max,
        continuous_decode_stdout_read_throughput_bytes_per_ms: format_preview_loop_bytes_per_ms(
            timing.continuous_decode_output_bytes_total,
            timing.continuous_decode_stdout_read_elapsed_ms,
        ),
        continuous_decode_ffmpeg_scale_enabled: timing.continuous_decode_ffmpeg_scale_enabled,
        continuous_decode_ffmpeg_output_pixel_format: timing
            .continuous_decode_ffmpeg_output_pixel_format
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_output_pipeline_experiment_mode: timing
        ...
        continuous_decode_output_pixel_convert_elapsed_ms: timing
            .continuous_decode_output_pixel_convert_elapsed_ms,
        continuous_decode_output_pixel_convert_elapsed_ms_max: timing
            .continuous_decode_output_pixel_convert_elapsed_ms_max,
        continuous_decode_output_pixel_convert_count: timing
            .continuous_decode_output_pixel_convert_count,
        continuous_decode_output_pixel_convert_buffer_reuse_count: timing
            .continuous_decode_output_pixel_convert_buffer_reuse_count,
        continuous_decode_output_pixel_convert_buffer_allocation_count: timing
            .continuous_decode_output_pixel_convert_buffer_allocation_count,
        continuous_decode_output_pixel_convert_bytes_written_total: timing
            .continuous_decode_output_pixel_convert_bytes_written_total,
        continuous_decode_output_pixel_convert_bytes_written_per_frame: if timing
            .continuous_decode_output_pixel_convert_count
            > 0
        {
            timing.continuous_decode_output_pixel_convert_bytes_written_total
                / timing.continuous_decode_output_pixel_convert_count as usize
        } else {
            0
        },
        continuous_decode_output_pixel_convert_mode: timing
            .continuous_decode_output_pixel_convert_mode
            .clone()
            .unwrap_or_else(|| "none".to_string()),
        continuous_decode_competing_one_shot_decode_elapsed_ms: timing
            .continuous_decode_competing_one_shot_decode_elapsed_ms,
        continuous_decode_competing_one_shot_attempt_count: timing
            .continuous_decode_competing_one_shot_attempt_count,
        continuous_decode_slot0_one_shot_suppression_enabled: timing
            .continuous_decode_slot0_one_shot_suppression_enabled,
        continuous_decode_slot0_one_shot_suppressed_count: timing
            .continuous_decode_slot0_one_shot_suppressed_count,
        continuous_decode_slot0_one_shot_suppressed_reason_counts:
            format_continuous_decode_slot0_one_shot_suppressed_reason_counts(
                timing.continuous_decode_slot0_one_shot_suppressed_reason_not_ready_count,
                timing.continuous_decode_slot0_one_shot_suppressed_reason_stale_count,
                timing.continuous_decode_slot0_one_shot_suppressed_reason_future_count,
                timing.continuous_decode_slot0_one_shot_suppressed_reason_unknown_count,
            ),
        continuous_decode_slot0_one_shot_suppressed_render_safety_counts:
            format_continuous_decode_slot0_one_shot_suppressed_render_safety_counts(
                timing
                    .continuous_decode_slot0_one_shot_suppressed_render_safety_decode_deferred_placeholder_count,
                timing.continuous_decode_slot0_one_shot_suppressed_render_safety_unknown_count,
            ),
        continuous_decode_slot0_one_shot_suppressed_continuous_not_ready_count: timing
            .continuous_decode_slot0_one_shot_suppressed_reason_not_ready_count,
        continuous_decode_slot0_one_shot_suppressed_stale_count: timing
            .continuous_decode_slot0_one_shot_suppressed_reason_stale_count,
        ...
        continuous_feed_frame_received_count: timing.continuous_feed_frame_received_count,
        continuous_feed_no_frame_count: timing.continuous_feed_no_frame_count,
        continuous_feed_handoff_error_count: timing.continuous_feed_handoff_error_count,
        continuous_feed_enqueued_count: timing.continuous_feed_enqueued_count,
        continuous_feed_skipped_count: timing.continuous_feed_skipped_count,
        continuous_feed_skip_reason_counts: format_continuous_feed_skip_reason_counts(
            timing.continuous_feed_skip_reason_duplicate_count,
            timing.continuous_feed_skip_reason_future_frame_count,
            timing.continuous_feed_skip_reason_runtime_disabled_count,
            timing.continuous_feed_skip_reason_startup_not_ready_count,
            timing.continuous_feed_skip_reason_input_queue_full_count,
            timing.continuous_feed_skip_reason_source_mismatch_count,
            timing.continuous_feed_skip_reason_consume_mismatch_count,
            timing.continuous_feed_skip_reason_unknown_count,
        ),
        continuous_feed_dropped_stale_input_count: timing.continuous_feed_dropped_stale_input_count,
        continuous_feed_latest_received_frame_id: timing.continuous_feed_latest_received_frame_id,
        continuous_feed_latest_enqueued_frame_id: timing.continuous_feed_latest_enqueued_frame_id,
        continuous_decode_input_from_feeder_count: timing.continuous_decode_input_from_feeder_count,
        continuous_decode_input_from_render_demand_count: timing
            .continuous_decode_input_from_render_demand_count,
        continuous_decode_feeder_lag_to_selected: timing.continuous_decode_feeder_lag_to_selected,
        continuous_decode_render_exact_hit_count: timing.continuous_decode_render_exact_hit_count,
        ...
        continuous_decode_bounded_lookup_rejected_not_ready_count: timing
            .continuous_decode_bounded_lookup_rejected_not_ready_count,
        continuous_decode_output_availability_not_ready_count: timing
            .continuous_decode_output_availability_not_ready_count,
        continuous_decode_output_availability_stale_count: timing
            .continuous_decode_output_availability_stale_count,
        continuous_decode_output_availability_future_count: timing
            .continuous_decode_output_availability_future_count,
        ...
        one_shot_decode_attempt_count: timing.one_shot_decode_attempt_count,
        one_shot_decode_elapsed_ms: timing.one_shot_decode_elapsed_ms,
        one_shot_decode_elapsed_ms_max: timing.one_shot_decode_elapsed_ms_max,
        ...
        one_shot_decode_slow_first_byte_slot_counts: format_two_real_attempt_slot_counts(
            timing.one_shot_decode_slow_first_byte_slot0_count,
            timing.one_shot_decode_slow_first_byte_slot1_count,
        ),
        one_shot_decode_slow_first_byte_source_counts: format_two_real_attempt_source_counts(
            timing.one_shot_decode_slow_first_byte_source_queue_count,
            timing.one_shot_decode_slow_first_byte_source_retained_keyframe_count,
            timing.one_shot_decode_slow_first_byte_source_unknown_count,
        ),
        one_shot_decode_slow_first_byte_reason_counts: format_two_real_attempt_reason_counts(
            timing.one_shot_decode_slow_first_byte_reason_frame_id_changed_count,
            timing.one_shot_decode_slow_first_byte_reason_cache_miss_count,
            timing.one_shot_decode_slow_first_byte_reason_previous_unavailable_count,
            timing.one_shot_decode_slow_first_byte_reason_source_recovered_count,
            timing.one_shot_decode_slow_first_byte_reason_unknown_count,
        ),
        one_shot_decode_slow_output_read_slot_counts: format_two_real_attempt_slot_counts(
            timing.one_shot_decode_slow_output_read_slot0_count,
            timing.one_shot_decode_slow_output_read_slot1_count,
        ),
        one_shot_decode_slow_output_read_source_counts: format_two_real_attempt_source_counts(
            timing.one_shot_decode_slow_output_read_source_queue_count,
            timing.one_shot_decode_slow_output_read_source_retained_keyframe_count,
            timing.one_shot_decode_slow_output_read_source_unknown_count,
        ),
        one_shot_decode_slow_output_read_reason_counts: format_two_real_attempt_reason_counts(
            timing.one_shot_decode_slow_output_read_reason_frame_id_changed_count,
            timing.one_shot_decode_slow_output_read_reason_cache_miss_count,
            timing.one_shot_decode_slow_output_read_reason_previous_unavailable_count,
            timing.one_shot_decode_slow_output_read_reason_source_recovered_count,
            timing.one_shot_decode_slow_output_read_reason_unknown_count,
        ),
        one_shot_decode_input_write_outlier_count: timing.one_shot_decode_input_write_outlier_count,
        one_shot_decode_input_write_outlier_threshold_ms: slow_attempt_threshold_ms,
        one_shot_decode_input_write_outlier_slot_counts: format_two_real_attempt_slot_counts(
            timing.one_shot_decode_input_write_outlier_slot0_count,
            timing.one_shot_decode_input_write_outlier_slot1_count,
        ),
        one_shot_decode_input_write_outlier_source_counts: format_two_real_attempt_source_counts(
            timing.one_shot_decode_input_write_outlier_source_queue_count,
            timing.one_shot_decode_input_write_outlier_source_retained_keyframe_count,
            timing.one_shot_decode_input_write_outlier_source_unknown_count,
        ),
        one_shot_decode_input_write_outlier_reason_counts: format_two_real_attempt_reason_counts(
            timing.one_shot_decode_input_write_outlier_reason_frame_id_changed_count,
            timing.one_shot_decode_input_write_outlier_reason_cache_miss_count,
            timing.one_shot_decode_input_write_outlier_reason_previous_unavailable_count,
            timing.one_shot_decode_input_write_outlier_reason_source_recovered_count,
            timing.one_shot_decode_input_write_outlier_reason_unknown_count,
        ),
        one_shot_decode_input_write_outlier_payload_bytes_total: timing
            .one_shot_decode_input_write_outlier_payload_bytes_total,
        one_shot_decode_input_write_outlier_payload_bytes_avg: format_preview_loop_average_usize(
            timing.one_shot_decode_input_write_outlier_payload_bytes_total,
            timing.one_shot_decode_input_write_outlier_count,
        ),
        ...
        one_shot_decode_input_payload_bytes_avg: format_preview_loop_average_usize(
            ...
            timing.one_shot_decode_attempt_count,
        ),
        one_shot_decode_keyframe_attempt_count: timing.one_shot_decode_keyframe_attempt_count,
        one_shot_decode_non_keyframe_attempt_count: timing
            .one_shot_decode_non_keyframe_attempt_count,
        one_shot_decode_keyframe_elapsed_ms: timing.one_shot_decode_keyframe_elapsed_ms,
        one_shot_decode_non_keyframe_elapsed_ms: timing.one_shot_decode_non_keyframe_elapsed_ms,
        one_shot_decode_keyframe_input_payload_bytes_total: timing
            .one_shot_decode_keyframe_input_payload_bytes_total,
        one_shot_decode_non_keyframe_input_payload_bytes_total: timing
            .one_shot_decode_non_keyframe_input_payload_bytes_total,
        one_shot_decode_expected_output_bytes_per_frame: format_preview_loop_average_usize(
            timing.one_shot_decode_expected_output_bytes_total,
            timing.one_shot_decode_attempt_count,
        ),
        one_shot_decode_output_width: timing
            .one_shot_decode_output_width
            .or(Some(FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_WIDTH / 2)),
        one_shot_decode_output_height: timing
            .one_shot_decode_output_height
            .or(Some(FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_HEIGHT / 2)),
        one_shot_decode_output_pixel_format: match timing.one_shot_decode_output_pixel_format {
            Some(pixel_format) => format!("{pixel_format:?}"),
            None => "Bgra8".to_string(),
        },
        one_shot_decode_scaled_output_enabled: true,
        one_shot_decode_scaled_output_reason: timing
        ...
        one_shot_decode_input_write_elapsed_per_payload_kb:
            format_preview_loop_elapsed_per_payload_kb(
                timing.one_shot_decode_input_write_elapsed_ms,
                timing.one_shot_decode_input_payload_bytes_total,
            ),
        one_shot_decode_read_throughput_bytes_per_ms: format_preview_loop_bytes_per_ms(
            timing.decode_output_bytes_total,
            timing.one_shot_decode_output_read_elapsed_ms,
        ),
        ...
        output_width: render_metadata.output_width,
        output_height: render_metadata.output_height,
        program_output_enabled: program_output_window_enabled,
        ...
        program_output_window_title: program_output_window_enabled
        ...
        program_continuous_decode_lookup_hit_count: if program_continuous_decode_active {
            timing.continuous_decode_lookup_hit_count
        } else {
            0
        },
        program_continuous_decode_lookup_miss_count: if program_continuous_decode_active {
            timing.continuous_decode_lookup_miss_count
        } else {
            0
        },
        ...
        program_decode_fps: if program_continuous_decode_active {
            format_preview_loop_fps(
                ...
                timing
                    .continuous_decode_first_input_to_now_elapsed_ms
                    .unwrap_or(loop_total_elapsed_ms),
            )
        } else {
            "n/a".to_string()
        },
        program_selected_source_frame_lag: if program_continuous_decode_active {
            timing.continuous_decode_requested_minus_latest_lag
        } else {
            None
        },
        program_first_validation_enabled: program_first_validation_mode,
        program_first_suppressed_preview_one_shot_decode_count: timing
            .program_first_suppressed_preview_one_shot_decode_count,
        program_first_suppressed_preview_one_shot_decode_slot_counts:
            format_two_real_attempt_slot_counts(
                timing.program_first_suppressed_preview_one_shot_decode_slot0_count,
                timing.program_first_suppressed_preview_one_shot_decode_slot1_count,
            ),
        program_first_program_only_decode_path_enabled: timing
            .program_first_program_only_decode_path_enabled,
        program_first_remaining_one_shot_decode_count: if program_first_validation_mode {
            timing.one_shot_decode_attempt_count
        } else {
            0
        },
        program_first_preview_visible: program_first_validation_mode && frames_rendered > 0,
        program_first_preview_refresh_interval: program_first_preview_refresh_interval
            .map(NonZeroU32::get),
        program_first_preview_refresh_count: operator_preview_refresh_attempt_count,
        program_first_preview_suppressed_count: if program_first_validation_mode {
            preview_compose_skipped_for_program_count
        } else {
            0
        },
        operator_low_cost_preview_enabled,
        operator_preview_refresh_interval_ticks: program_first_preview_refresh_interval
            .map(NonZeroU32::get),
        operator_preview_refresh_attempt_count,
        operator_preview_refresh_success_count,
        operator_preview_refresh_skipped_count,
        ...
        operator_preview_reused_program_frame_count: if operator_low_cost_preview_enabled {
            operator_preview_reused_program_frame_count
        } else {
            0
        },
        operator_preview_program_slot_visible_count: if operator_low_cost_preview_enabled {
            operator_preview_program_slot_visible_count
        } else {
            0
        },
        operator_preview_program_slot_reuse_source: if operator_low_cost_preview_enabled {
            operator_preview_program_slot_reuse_source.as_summary_value()
        } else {
            "none"
        },
        operator_preview_program_slot_black_count: if operator_low_cost_preview_enabled {
            operator_preview_program_slot_black_count
        } else {
            0
        },
        operator_preview_snapshot_retention_enabled: operator_preview_snapshot_retention_active,
        operator_preview_snapshot_reuse_count: if operator_preview_snapshot_retention_active {
            operator_preview_snapshot_reuse_count
        } else {
            0
        },
        operator_preview_snapshot_reuse_slot_counts: if operator_preview_snapshot_retention_active {
            format_four_view_slot_counts(operator_preview_snapshot_reuse_slot_counts)
        } else {
            format_four_view_slot_counts([0; 4])
        },
        operator_preview_snapshot_created_count: if operator_preview_snapshot_retention_active {
            operator_preview_snapshot_created_count
        } else {
            0
        },
        operator_preview_snapshot_created_slot_counts: if operator_preview_snapshot_retention_active
        {
            format_four_view_slot_counts(operator_preview_snapshot_created_slot_counts)
        } else {
            format_four_view_slot_counts([0; 4])
        },
        operator_preview_placeholder_avoided_by_snapshot_count:
        ...
        operator_preview_slot_black_after_snapshot_count: if operator_preview_snapshot_retention_active
        {
            operator_preview_slot_black_after_snapshot_count
        } else {
            0
        ...
        program_window_render_success_count,
        program_window_render_failure_count,
        program_render_effective_fps: effective_program_render_fps.clone(),
...
fn format_four_view_slot_counts(slot_counts: [u32; 4]) -> String {
    format!(
        "slot0:{}|slot1:{}|slot2:{}|slot3:{}",
        slot_counts[0], slot_counts[1], slot_counts[2], slot_counts[3]
    )
}
...
fn format_continuous_decode_lookup_miss_reason_counts(
    exact_key_missing: u32,
    queue_empty: u32,
    runtime_disabled: u32,
    output_pending: u32,
    frame_id_lagging: u32,
    unknown: u32,
) -> String {
    format!(
        "exact_key_missing:{exact_key_missing}|queue_empty:{queue_empty}|runtime_disabled:{runtime_disabled}|output_pending:{output_pending}|frame_id_lagging:{frame_id_lagging}|unknown:{unknown}"
    )
}

fn format_continuous_feed_skip_reason_counts(
    duplicate: u32,
    future_frame: u32,
    runtime_disabled: u32,
    startup_not_ready: u32,
    input_queue_full: u32,
    source_mismatch: u32,
    consume_mismatch: u32,
    unknown: u32,
) -> String {
    format!(
        "duplicate:{duplicate}|future_frame:{future_frame}|runtime_disabled:{runtime_disabled}|startup_not_ready:{startup_not_ready}|input_queue_full:{input_queue_full}|source_mismatch:{source_mismatch}|consume_mismatch:{consume_mismatch}|unknown:{unknown}"
    )
}
...
fn format_continuous_decode_slot0_one_shot_suppressed_reason_counts(
    continuous_not_ready: u32,
    stale: u32,
    future: u32,
    unknown: u32,
) -> String {
    format!(
        "continuous_not_ready:{continuous_not_ready}|stale:{stale}|future:{future}|unknown:{unknown}"
    )
}

fn format_continuous_decode_slot0_one_shot_suppressed_render_safety_counts(
    decode_deferred_placeholder: u32,
    unknown: u32,
) -> String {
    format!("decode_deferred_placeholder:{decode_deferred_placeholder}|unknown:{unknown}")
}
...
fn record_two_real_attempt_slot_counts(
    ...
    if candidate.slot_index == slot0_index {
        *slot0_count = slot0_count.saturating_add(1);
    } else if candidate.slot_index == slot1_index {
        *slot1_count = slot1_count.saturating_add(1);
    }
}

fn record_two_real_attempt_source_counts(
    candidate: &TwoRealPreviewLoopDecodeAttemptCandidate,
    queue_count: &mut u32,
    retained_keyframe_count: &mut u32,
    ...
    match candidate.selected_frame_source.as_deref() {
        ...
        Some("retained_keyframe") => {
            *retained_keyframe_count = retained_keyframe_count.saturating_add(1);
        }
        _ => {
...
fn record_two_real_attempt_reason_counts(
    candidate: &TwoRealPreviewLoopDecodeAttemptCandidate,
    frame_id_changed_count: &mut u32,
    cache_miss_count: &mut u32,
    previous_unavailable_count: &mut u32,
    source_recovered_count: &mut u32,
    unknown_count: &mut u32,
) {
    match candidate.reason {
        TwoRealPreviewLoopDecodeAttemptReason::FrameIdChanged => {
            *frame_id_changed_count = frame_id_changed_count.saturating_add(1);
        }
        ...
        TwoRealPreviewLoopDecodeAttemptReason::PreviousUnavailable => {
            *previous_unavailable_count = previous_unavailable_count.saturating_add(1);
        }
        TwoRealPreviewLoopDecodeAttemptReason::SourceRecovered => {
            *source_recovered_count = source_recovered_count.saturating_add(1);
        }
        TwoRealPreviewLoopDecodeAttemptReason::Unknown => {
            *unknown_count = unknown_count.saturating_add(1);
        }
...
fn record_two_real_decode_attempt_candidate(
    ...
    record_two_real_attempt_source_counts(
        candidate,
        &mut timing.one_shot_decode_attempt_source_queue_count,
        &mut timing.one_shot_decode_attempt_source_retained_keyframe_count,
        &mut timing.one_shot_decode_attempt_source_unknown_count,
    );
    record_two_real_attempt_slot_counts(
        slot0_index,
        slot1_index,
        candidate,
        &mut timing.one_shot_decode_attempt_slot0_count,
        &mut timing.one_shot_decode_attempt_slot1_count,
    );
    if candidate.slot_index == slot0_index {
    ...
    record_two_real_attempt_reason_counts(
        candidate,
        &mut timing.one_shot_decode_attempt_reason_frame_id_changed_count,
        &mut timing.one_shot_decode_attempt_reason_cache_miss_count,
        &mut timing.one_shot_decode_attempt_reason_previous_unavailable_count,
        &mut timing.one_shot_decode_attempt_reason_source_recovered_count,
        &mut timing.one_shot_decode_attempt_reason_unknown_count,
    );
}
...
fn record_two_real_decode_attempt_diagnostics(
    ...
    decode_attempt_observation_delta: &[TwoRealPreviewLoopDecodeAttemptObservation],
    unchanged_frame_reuse_slots: &[bool; 4],
    ...
    let mut assigned_candidates = Vec::new();
        ...
        .filter(|candidate| candidate.probable_cache_miss)
    {
        ...
        assigned_candidates.push(candidate.clone());
    ...
    record_two_real_decode_attempt_correlation_diagnostics(
        timing,
        slot0_index,
        slot1_index,
        &assigned_candidates,
...
fn record_two_real_decode_attempt_correlation_diagnostics(
    timing: &mut TwoRealPreviewLoopRuntimeTiming,
    slot0_index: usize,
    slot1_index: usize,
    assigned_candidates: &[TwoRealPreviewLoopDecodeAttemptCandidate],
    observations: &[TwoRealPreviewLoopDecodeAttemptObservation],
) {
    let mut matched_candidates = vec![false; assigned_candidates.len()];
    for observation in observations {
        let matched_index = assigned_candidates
            .iter()
            .enumerate()
            .find(|(index, candidate)| {
                !matched_candidates[*index]
                    && observation.key.is_some()
            ...
            .or_else(|| {
                assigned_candidates
                    .iter()
                    .enumerate()
                    .find(|(index, _)| !matched_candidates[*index])
                    .map(|(index, _)| index)
            });
        let Some(matched_index) = matched_index else {
            continue;
        };
        matched_candidates[matched_index] = true;
        let candidate = &assigned_candidates[matched_index];
        if observation.first_byte_slow {
            record_two_real_attempt_slot_counts(
                slot0_index,
                slot1_index,
                candidate,
                &mut timing.one_shot_decode_slow_first_byte_slot0_count,
                &mut timing.one_shot_decode_slow_first_byte_slot1_count,
            );
            record_two_real_attempt_source_counts(
                candidate,
                &mut timing.one_shot_decode_slow_first_byte_source_queue_count,
                &mut timing.one_shot_decode_slow_first_byte_source_retained_keyframe_count,
                &mut timing.one_shot_decode_slow_first_byte_source_unknown_count,
            );
            record_two_real_attempt_reason_counts(
                candidate,
                &mut timing.one_shot_decode_slow_first_byte_reason_frame_id_changed_count,
                &mut timing.one_shot_decode_slow_first_byte_reason_cache_miss_count,
                &mut timing.one_shot_decode_slow_first_byte_reason_previous_unavailable_count,
                &mut timing.one_shot_decode_slow_first_byte_reason_source_recovered_count,
                &mut timing.one_shot_decode_slow_first_byte_reason_unknown_count,
            );
        }
        if observation.output_read_slow {
            record_two_real_attempt_slot_counts(
                slot0_index,
                slot1_index,
                candidate,
                &mut timing.one_shot_decode_slow_output_read_slot0_count,
                &mut timing.one_shot_decode_slow_output_read_slot1_count,
            );
            record_two_real_attempt_source_counts(
                candidate,
                &mut timing.one_shot_decode_slow_output_read_source_queue_count,
                &mut timing.one_shot_decode_slow_output_read_source_retained_keyframe_count,
                &mut timing.one_shot_decode_slow_output_read_source_unknown_count,
            );
            record_two_real_attempt_reason_counts(
                candidate,
                &mut timing.one_shot_decode_slow_output_read_reason_frame_id_changed_count,
                &mut timing.one_shot_decode_slow_output_read_reason_cache_miss_count,
                &mut timing.one_shot_decode_slow_output_read_reason_previous_unavailable_count,
                &mut timing.one_shot_decode_slow_output_read_reason_source_recovered_count,
                &mut timing.one_shot_decode_slow_output_read_reason_unknown_count,
            );
        }
        if observation.input_write_outlier {
            timing.one_shot_decode_input_write_outlier_count = timing
                .one_shot_decode_input_write_outlier_count
                .saturating_add(1);
            timing.one_shot_decode_input_write_outlier_payload_bytes_total = timing
                .one_shot_decode_input_write_outlier_payload_bytes_total
                .saturating_add(observation.input_payload_bytes);
            record_two_real_attempt_slot_counts(
                slot0_index,
                slot1_index,
                candidate,
                &mut timing.one_shot_decode_input_write_outlier_slot0_count,
                &mut timing.one_shot_decode_input_write_outlier_slot1_count,
            );
            record_two_real_attempt_source_counts(
                candidate,
                &mut timing.one_shot_decode_input_write_outlier_source_queue_count,
                &mut timing.one_shot_decode_input_write_outlier_source_retained_keyframe_count,
                &mut timing.one_shot_decode_input_write_outlier_source_unknown_count,
            );
            record_two_real_attempt_reason_counts(
                candidate,
                &mut timing.one_shot_decode_input_write_outlier_reason_frame_id_changed_count,
                &mut timing.one_shot_decode_input_write_outlier_reason_cache_miss_count,
                &mut timing.one_shot_decode_input_write_outlier_reason_previous_unavailable_count,
                &mut timing.one_shot_decode_input_write_outlier_reason_source_recovered_count,
                &mut timing.one_shot_decode_input_write_outlier_reason_unknown_count,
            );
        }
    }
}
...
fn format_preview_loop_average_usize(total: usize, count: u32) -> String {
    if count == 0 {
        return "n/a".to_string();
    }
    format!("{:.3}", total as f64 / count as f64)
}

fn format_preview_loop_bytes_per_ms(total_bytes: usize, elapsed_ms: u128) -> String {
    if elapsed_ms == 0 {
        return "n/a".to_string();
    }
    format!("{:.3}", total_bytes as f64 / elapsed_ms as f64)
}

fn format_preview_loop_bytes_per_sec(total_bytes: usize, elapsed_ms: u128) -> String {
    if elapsed_ms == 0 {
        return "n/a".to_string();
    }
    format!("{:.3}", total_bytes as f64 * 1000.0 / elapsed_ms as f64)
}

fn format_preview_loop_elapsed_per_payload_kb(
    ...
    format!(
        "{:.6}",
        total_elapsed_ms as f64 / (total_bytes as f64 / 1024.0)
    )
}
...
fn format_four_view_two_real_handoff_preview_loop_summary(
    ...
    let summary_line = format!(
        ...
        summary.persistent_decode_config_enabled,
        ...
        summary.persistent_decode_skipped_by_config_count,
        ...
        summary.continuous_decode_config_enabled,
        summary.continuous_decode_runtime_enabled,
        summary.continuous_decode_slot0_enabled,
        summary.continuous_decode_input_frame_count,
        summary.continuous_decode_output_frame_count,
        summary.continuous_decode_queue_len,
        summary.continuous_decode_dropped_stale_count,
        summary.continuous_decode_frame_id_lag,
        summary.continuous_decode_stdout_read_elapsed_ms,
        summary.continuous_decode_stall_count,
        summary.continuous_decode_restart_count,
        summary.continuous_decode_runtime_disabled,
        sanitize_summary_value(&summary.continuous_decode_runtime_disabled_reason),
        summary.continuous_decode_fallback_to_one_shot_count,
        summary.continuous_decode_lookup_hit_count,
        summary.continuous_decode_lookup_miss_count,
        summary.continuous_decode_lookup_miss_reason_counts,
        format_optional_u64(summary.continuous_decode_requested_frame_id),
        format_optional_u64(summary.continuous_decode_latest_decoded_frame_id),
        format_optional_u64(summary.continuous_decode_requested_minus_latest_lag),
        format_optional_u64(summary.continuous_decode_queue_oldest_frame_id),
        format_optional_u64(summary.continuous_decode_queue_newest_frame_id),
        format_optional_u64(summary.continuous_decode_input_frame_id_min),
        format_optional_u64(summary.continuous_decode_input_frame_id_max),
        format_optional_u64(summary.continuous_decode_output_frame_id_min),
        format_optional_u64(summary.continuous_decode_output_frame_id_max),
        summary.continuous_decode_output_pending_correspondence_count,
        summary.continuous_decode_writer_input_queue_len,
        summary.continuous_decode_exact_match_required_count,
        summary.continuous_decode_stale_frame_available_count,
        summary.continuous_decode_input_frame_id_gap_max,
        summary.continuous_decode_input_frame_id_gap_total,
        summary.continuous_decode_input_non_consecutive_count,
        summary.continuous_decode_input_keyframe_count,
        summary.continuous_decode_input_non_keyframe_count,
        summary.continuous_decode_input_has_sps_count,
        summary.continuous_decode_input_has_pps_count,
        summary.continuous_decode_input_has_idr_count,
        summary.continuous_decode_input_has_non_idr_vcl_count,
        sanitize_summary_value(&summary.continuous_decode_last_input_payload_nal_kinds),
        sanitize_summary_value(&summary.continuous_decode_ffmpeg_stderr_summary),
        summary.continuous_decode_stdout_reader_blocked_count,
        summary.continuous_decode_no_output_after_input_count,
        summary.continuous_decode_no_output_after_keyframe_count,
        summary.continuous_decode_bootstrap_input_count,
        summary.continuous_decode_bootstrap_output_count,
        format_optional_u64(summary.continuous_decode_last_input_frame_id),
        format_optional_u64(summary.continuous_decode_last_output_frame_id),
        sanitize_summary_value(&summary.continuous_decode_ffmpeg_args_summary),
        summary.continuous_decode_stdin_write_count,
        summary.continuous_decode_stdin_write_bytes_total,
        summary.continuous_decode_stdin_write_error_count,
        sanitize_summary_value(&summary.continuous_decode_last_stdin_write_error),
        summary.continuous_decode_process_running,
        sanitize_summary_value(&summary.continuous_decode_process_exit_status),
        summary.continuous_decode_stdout_read_attempt_count,
        summary.continuous_decode_stdout_read_in_progress,
        summary.continuous_decode_stderr_reader_alive,
        summary.continuous_decode_stderr_bytes_total,
        format_optional_u128(summary.continuous_decode_last_stderr_at_ms),
        summary.continuous_decode_last_input_write_elapsed_ms,
        summary.continuous_decode_last_input_payload_bytes,
        format_optional_u128(summary.continuous_decode_first_input_to_first_output_elapsed_ms),
        format_optional_u128(summary.continuous_decode_first_input_to_now_elapsed_ms),
        summary.render_used_continuous_decoded_count,
        summary.render_used_one_shot_fallback_count,
        summary.one_shot_decode_fallback_count,
        summary.one_shot_decode_attempt_count,
        summary.one_shot_decode_elapsed_ms,
        summary.one_shot_decode_elapsed_ms_max,
        summary.one_shot_decode_input_write_elapsed_ms,
        ...
        summary.one_shot_decode_slow_first_byte_slot_counts,
        summary.one_shot_decode_slow_first_byte_source_counts,
        summary.one_shot_decode_slow_first_byte_reason_counts,
        summary.one_shot_decode_slow_output_read_slot_counts,
        summary.one_shot_decode_slow_output_read_source_counts,
        summary.one_shot_decode_slow_output_read_reason_counts,
        summary.one_shot_decode_input_write_outlier_count,
        summary.one_shot_decode_input_write_outlier_threshold_ms,
        summary.one_shot_decode_input_write_outlier_slot_counts,
        summary.one_shot_decode_input_write_outlier_source_counts,
        summary.one_shot_decode_input_write_outlier_reason_counts,
        summary.one_shot_decode_input_write_outlier_payload_bytes_total,
        summary.one_shot_decode_input_write_outlier_payload_bytes_avg,
        ...
        format_optional_usize(summary.one_shot_decode_input_payload_bytes_min),
        summary.one_shot_decode_input_payload_bytes_max,
        summary.one_shot_decode_input_payload_bytes_avg,
        summary.one_shot_decode_keyframe_attempt_count,
        summary.one_shot_decode_non_keyframe_attempt_count,
        summary.one_shot_decode_keyframe_elapsed_ms,
        summary.one_shot_decode_non_keyframe_elapsed_ms,
        summary.one_shot_decode_keyframe_input_payload_bytes_total,
        summary.one_shot_decode_non_keyframe_input_payload_bytes_total,
        summary.one_shot_decode_expected_output_bytes_per_frame,
        format_optional_u32(summary.one_shot_decode_output_width),
        format_optional_u32(summary.one_shot_decode_output_height),
        summary.one_shot_decode_output_pixel_format,
        summary.one_shot_decode_scaled_output_enabled,
        sanitize_summary_value(&summary.one_shot_decode_scaled_output_reason),
    ...
    format!(
        ...
        summary.continuous_feed_enabled,
        summary.continuous_feed_attempt_count,
        summary.continuous_feed_handoff_request_count,
        summary.continuous_feed_frame_received_count,
        summary.continuous_feed_no_frame_count,
        summary.continuous_feed_handoff_error_count,
        summary.continuous_feed_enqueued_count,
        summary.continuous_feed_skipped_count,
        summary.continuous_feed_skip_reason_counts,
        summary.continuous_feed_dropped_stale_input_count,
        format_optional_u64(summary.continuous_feed_latest_received_frame_id),
        format_optional_u64(summary.continuous_feed_latest_enqueued_frame_id),
        summary.continuous_decode_input_from_feeder_count,
        summary.continuous_decode_input_from_render_demand_count,
        format_optional_u64(summary.continuous_decode_feeder_lag_to_selected),
        ...
        summary.continuous_decode_output_availability_not_ready_count,
        summary.continuous_decode_output_availability_stale_count,
        summary.continuous_decode_output_availability_future_count,
        ...
        summary.continuous_decode_pending_correspondence_count,
        format_optional_u128(summary.continuous_decode_pending_correspondence_age_ms_max),
        ...
        format_optional_u64(summary.continuous_decode_latest_selected_to_output_frame_gap),
        summary.continuous_decode_output_throughput_fps,
        summary.continuous_decode_reader_full_frame_elapsed_ms_max,
        summary.continuous_decode_reader_full_frame_elapsed_ms_avg,
        summary.continuous_decode_reader_full_frame_slow_count,
        summary.continuous_decode_reader_full_frame_slow_threshold_ms,
        summary.continuous_decode_output_bytes_total,
        summary.continuous_decode_output_bytes_per_sec,
        summary.continuous_decode_output_frame_interval_ms_avg,
        summary.continuous_decode_output_frame_interval_ms_max,
        summary.continuous_decode_stdout_read_throughput_bytes_per_ms,
        ...
        summary.continuous_decode_output_pixel_convert_elapsed_ms_max,
        summary.continuous_decode_output_pixel_convert_count,
        summary.continuous_decode_output_pixel_convert_buffer_reuse_count,
        summary.continuous_decode_output_pixel_convert_buffer_allocation_count,
        summary.continuous_decode_output_pixel_convert_bytes_written_total,
        summary.continuous_decode_output_pixel_convert_bytes_written_per_frame,
        sanitize_summary_value(&summary.continuous_decode_output_pixel_convert_mode),
        summary.continuous_decode_competing_one_shot_decode_elapsed_ms,
        summary.continuous_decode_competing_one_shot_attempt_count,
        summary.continuous_decode_slot0_one_shot_suppression_enabled,
        summary.continuous_decode_slot0_one_shot_suppressed_count,
        summary.continuous_decode_slot0_one_shot_suppressed_reason_counts,
        summary.continuous_decode_slot0_one_shot_suppressed_render_safety_counts,
        summary.continuous_decode_slot0_one_shot_suppressed_continuous_not_ready_count,
        summary.continuous_decode_slot0_one_shot_suppressed_stale_count,
        summary.continuous_decode_queue_drop_reason_counts,
        summary.program_output_enabled,
        summary.program_output_selection_mode,
        format_optional_summary_string(
            summary
                .program_output_requested_client_id
                .as_ref()
                .map(|client_id| client_id.0.as_str()),
        ),
        summary.program_output_render_count,
        summary.program_output_missing_selected_source_count,
        summary.program_output_missing_before_first_render_count,
        summary.program_output_missing_after_first_render_count,
        summary.program_output_reused_previous_frame_count,
        summary.program_output_placeholder_render_count,
        summary.program_output_black_frame_render_count,
        format_optional_u32(summary.program_output_first_render_attempt_index),
        format_optional_u128(summary.program_output_first_render_elapsed_ms),
        format_optional_summary_string(summary.program_output_missing_selected_source_reason),
        summary.program_output_last_result_kind,
        format_optional_summary_string(
            summary
                .program_output_selected_client_id
                .as_ref()
        ...
        format_optional_usize(summary.program_output_selected_slot_index),
        ...
        summary.program_first_validation_enabled,
        summary.program_first_suppressed_preview_one_shot_decode_count,
        summary.program_first_suppressed_preview_one_shot_decode_slot_counts,
        summary.program_first_program_only_decode_path_enabled,
        summary.program_first_remaining_one_shot_decode_count,
        summary.program_first_preview_visible,
        format_optional_u32(summary.program_first_preview_refresh_interval),
        summary.program_first_preview_refresh_count,
        summary.program_first_preview_suppressed_count,
        summary.operator_low_cost_preview_enabled,
        format_optional_u32(summary.operator_preview_refresh_interval_ticks),
        summary.operator_preview_refresh_attempt_count,
        summary.operator_preview_refresh_success_count,
        summary.operator_preview_refresh_skipped_count,
        ...
        summary.operator_preview_reused_program_frame_count,
        summary.operator_preview_program_slot_visible_count,
        summary.operator_preview_program_slot_reuse_source,
        summary.operator_preview_program_slot_black_count,
        summary.operator_preview_snapshot_retention_enabled,
        summary.operator_preview_snapshot_reuse_count,
        summary.operator_preview_snapshot_reuse_slot_counts,
        summary.operator_preview_snapshot_created_count,
        summary.operator_preview_snapshot_created_slot_counts,
        summary.operator_preview_placeholder_avoided_by_snapshot_count,
        summary.operator_preview_slot_black_after_snapshot_count,
        ...
        summary.program_window_render_success_count,
        summary.program_window_render_failure_count,
        summary.program_render_effective_fps,
        summary.effective_program_render_fps,
    )
}
...
fn format_optional_summary_string(value: Option<&str>) -> String {
    value
...
mod tests {
    ...
    use super::{
        ...
        SwitcherSingleClientQueueSourceMode, TimedSwitcherH264DecodeRuntime,
        ...
        TwoRealPreviewLoopRuntimeTiming, TwoRealProgramFirstDecodeSuppressionConfig,
        FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_HEIGHT,
        FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_WIDTH, FOUR_VIEW_CLEAN_OUTPUT_LOOP_SCALE_MODE,
        REUSABLE_OBS_RENDER_BUFFER,
    };
    use stream_sync_switcher::{
        SWITCHER_FOUR_VIEW_CLEAN_OUTPUT_WINDOW_TITLE, SWITCHER_PROGRAM_OUTPUT_WINDOW_TITLE,
    };
    ...
    #[test]
    fn switcher_two_real_handoff_preview_options_default_to_persistent_enabled() {
        let options = super::parse_two_real_handoff_preview_options_or_exit(Vec::new());
        assert_eq!(
            options.read_mode,
            SwitcherSingleClientQueueSourceMode::PreviewLatest
        );
        ...
        assert_eq!(
            ...
            super::TwoRealContinuousOutputPipelineExperimentMode::Default
        );
        ...
        assert!(!options.operator_preview_snapshot_retention_enabled);
    }

    #[test]
    fn switcher_two_real_handoff_preview_options_accept_disable_persistent_decoder_flag() {
    ...
    #[test]
    fn switcher_two_real_handoff_preview_options_accept_enable_continuous_stream_decoder_flag() {
        let options = super::parse_two_real_handoff_preview_options_or_exit(vec![
            "preview-latest-decodable".to_string(),
        ...
        let options = super::parse_two_real_handoff_preview_options_or_exit(vec![
            ...
            "60".to_string(),
            "--operator-preview-snapshot-retention".to_string(),
            "preview-latest-decodable".to_string(),
        ]);
        assert_eq!(
            options.read_mode,
    ...
    #[test]
    fn switcher_operator_low_cost_preview_refresh_decision_uses_interval_ticks() {
        assert!(!super::should_attempt_operator_low_cost_preview_refresh(
            false,
            NonZeroU32::new(2),
            0
        ));
    ...
    #[test]
    fn switcher_two_real_program_continuous_decode_source_maps_explicit_real_slot() {
        let slots = super::default_four_view_two_real_handoff_preview_slots(
            0,
        ...
        assert_eq!(
            super::program_continuous_decode_source_from_real_slots(
                &slots,
                [0, 2],
                Some(&ClientId("real-client-1".to_string()))
            ),
            Some(super::TwoRealContinuousDecodeSource {
    ...
    #[test]
    fn switcher_operator_preview_reuses_program_continuous_latest_for_program_slot() {
        let slots = super::default_four_view_two_real_handoff_preview_slots(
            0,
            ClientId("real-client-0".to_string()),
            RunId("real-run-0".to_string()),
        ...
        let selection = super::requested_program_selection_from_real_slots(
            &slots,
            [0, 2],
            &ClientId("real-client-1".to_string()),
        );
        let latest = super::TwoRealProgramContinuousLatestFrame {
            ...
            decoded_frame: fixture_decoded_frame(2, 2, [0x90, 0x91, 0x92, 255]),
            frame_id: 99,
            selected_frame_lag: Some(0),
            output_age_ms: Some(3),
        };
        let mut handoff = super::MultiRealAndPlaceholderQueuedFrameHandoff::new(
            slots.clone(),
            vec![
                (
                    ClientId("real-client-0".to_string()),
        ...
        let input = SwitcherFourViewHandoffValidationInput {
            slots,
            target_timestamp: TimestampMicros(1_000_004),
            previous_slots: std::array::from_fn(|_| None),
            display_current_time: TimestampMicros(1_000_005),
            layout_policy: SwitcherFourViewQuadLayoutPolicy::default(),
            composed_window_title: "StreamSync 4-view".to_string(),
            composed_render_hold_millis: 0,
        };
        let mut pre_composition = SwitcherFourViewHandoffValidationBoundary::default()
            .run_from_handoff_to_composition_render_with_runtimes_and_mode(
                &mut handoff,
                &input,
                super::preview_target_time_mode_from_switcher_mode(
                    SwitcherSingleClientQueueSourceMode::PreviewLatest,
                ),
                &AlwaysSuppressPreviewDecodeRuntime,
            );

        assert!(matches!(
            pre_composition.composition_render.render,
            SwitcherFourViewHandoffQuadCompositionRenderConnectionRenderResult::NoRenderableQuadView {
                ..
            }
        ));
        let (candidate, reuse_source) =
            super::operator_preview_program_reuse_candidate(&selection, Some(&latest), None);
        assert_eq!(
            reuse_source,
            super::TwoRealOperatorPreviewProgramReuseSource::ContinuousLatest
        );
        assert!(super::reuse_program_frame_for_operator_preview_slot(
            &mut pre_composition,
            &selection,
            candidate.expect("continuous latest should be reusable"),
        ));
        ...
        let SwitcherFourViewHandoffQuadCompositionRenderSlot::UseUpdatedFrame {
            ...
            selected,
            ..
        } = &pre_composition.composition_render.composition.slots[2]
        else {
            panic!("program slot should become a renderable updated frame");
        };
        assert_eq!(selected.frame.client_id, selection.selected_client_id);
        assert_eq!(frame.decoded.as_ref(), Some(&latest.decoded_frame));
    }

    #[test]
    fn switcher_operator_preview_program_reuse_can_fall_back_to_last_valid_program_frame() {
        let selection = SwitcherProgramSelection {
            selected_client_id: ClientId("real-client-1".to_string()),
            selected_run_id: Some(RunId("real-run-1".to_string())),
            selected_slot_index: Some(2),
        };
        let cached = super::TwoRealCachedProgramOutputFrame {
            selection: selection.clone(),
            decoded_frame: fixture_decoded_frame(2, 2, [0x50, 0x51, 0x52, 255]),
        };

        let (candidate, reuse_source) =
            super::operator_preview_program_reuse_candidate(&selection, None, Some(&cached));

        assert_eq!(
            reuse_source,
            super::TwoRealOperatorPreviewProgramReuseSource::LastValidProgram
        );
        assert_eq!(candidate, Some(&cached.decoded_frame));
    }

    #[test]
    fn switcher_operator_preview_snapshot_retention_replaces_decode_deferred_slot() {
        let slots = super::default_four_view_two_real_handoff_preview_slots(
            0,
            ClientId("real-client-0".to_string()),
            RunId("real-run-0".to_string()),
        ...
        let snapshot = SwitcherFourViewDisplayedSlot {
            slot_index: 2,
            selected: None,
            decoded: Some(fixture_decoded_frame(2, 2, [0x31, 0x32, 0x33, 255])),
            displayed_at: TimestampMicros(900_000),
        };
        let mut snapshots: [Option<SwitcherFourViewDisplayedSlot>; 4] =
            std::array::from_fn(|_| None);
        snapshots[2] = Some(snapshot.clone());

        let diagnostics =
            super::apply_operator_preview_snapshot_retention(&mut pre_composition, &snapshots);

        assert_eq!(diagnostics.reuse_count, 1);
        assert_eq!(diagnostics.reuse_slot_counts, [0, 0, 1, 0]);
        assert_eq!(diagnostics.placeholder_avoided_by_snapshot_count, 1);
        assert_eq!(diagnostics.slot_black_after_snapshot_count, 0);
        assert!(matches!(
            pre_composition.composition_render.render,
            SwitcherFourViewHandoffQuadCompositionRenderConnectionRenderResult::CompositionReady {
                renderable_slot_count: 1
        ...
        let SwitcherFourViewHandoffQuadCompositionRenderSlot::UseHeldPreviousFrame {
            frame, ..
        } = &pre_composition.composition_render.composition.slots[2]
        else {
            panic!("decode-deferred slot should be replaced by a held snapshot");
        };
        assert_eq!(frame, &snapshot);
    }

    #[test]
    fn switcher_operator_preview_snapshot_retention_replaces_no_display_placeholder_slot() {
        let slots = super::default_four_view_two_real_handoff_preview_slots(
            0,
            ClientId("real-client-0".to_string()),
            RunId("real-run-0".to_string()),
        ...
        assert!(matches!(
            pre_composition.composition_render.composition.slots[1],
            SwitcherFourViewHandoffQuadCompositionRenderSlot::UseNoDisplayPlaceholder { .. }
        ));
        ...
        let snapshot = SwitcherFourViewDisplayedSlot {
            slot_index: 1,
            selected: None,
            decoded: Some(fixture_decoded_frame(2, 2, [0x41, 0x42, 0x43, 255])),
            displayed_at: TimestampMicros(900_001),
        };
        let mut snapshots: [Option<SwitcherFourViewDisplayedSlot>; 4] =
            std::array::from_fn(|_| None);
        snapshots[1] = Some(snapshot.clone());

        let diagnostics =
            super::apply_operator_preview_snapshot_retention(&mut pre_composition, &snapshots);

        assert_eq!(diagnostics.reuse_count, 1);
        assert_eq!(diagnostics.reuse_slot_counts, [0, 1, 0, 0]);
        assert_eq!(diagnostics.placeholder_avoided_by_snapshot_count, 1);
        let SwitcherFourViewHandoffQuadCompositionRenderSlot::UseHeldPreviousFrame {
            frame, ..
        } = &pre_composition.composition_render.composition.slots[1]
        else {
            panic!("no-display placeholder should be replaced by a held snapshot");
        };
        assert_eq!(frame, &snapshot);
    }

    #[test]
    fn switcher_operator_preview_snapshot_creation_counts_new_visible_snapshots() {
        let previous: [Option<SwitcherFourViewDisplayedSlot>; 4] = std::array::from_fn(|_| None);
        let mut next: [Option<SwitcherFourViewDisplayedSlot>; 4] = std::array::from_fn(|_| None);
        next[0] = Some(SwitcherFourViewDisplayedSlot {
            slot_index: 0,
            selected: None,
            decoded: Some(fixture_decoded_frame(2, 2, [0x51, 0x52, 0x53, 255])),
            displayed_at: TimestampMicros(900_002),
        });

        let diagnostics = super::operator_preview_snapshot_creation_diagnostics(&previous, &next);

        assert_eq!(diagnostics.created_count, 1);
        assert_eq!(diagnostics.created_slot_counts, [1, 0, 0, 0]);
    }
    ...
    #[test]
    fn switcher_two_real_continuous_ffmpeg_args_support_scaled_bgr24_experiment() {
        let config = super::TwoRealContinuousDecodeConfig {
            output_pipeline_experiment_mode:
                super::TwoRealContinuousOutputPipelineExperimentMode::ScaledBgr24,
            ..super::TwoRealContinuousDecodeConfig::default()
        };
        let args = super::build_two_real_continuous_ffmpeg_args(640, 360, true, config);
    ...
    #[test]
    fn switcher_two_real_continuous_bgr24_output_converts_to_bgra() {
        assert_eq!(
            super::convert_bgr24_to_bgra8(&[10, 20, 30, 40, 50, 60]),
            vec![10, 20, 30, 255, 40, 50, 60, 255]
        );
    }

    #[test]
    fn switcher_two_real_continuous_bgr24_output_converts_in_place() {
        let mut pixels = vec![10, 20, 30, 40, 50, 60, 0, 0];
        let bytes_written = super::convert_bgr24_to_bgra8_in_place(&mut pixels, 6);
    ...
    #[test]
    fn switcher_two_real_continuous_bgr24_output_converts_expected_scaled_frame_size() {
        let bgr_len = 640 * 360 * 3;
        let bgra_len = 640 * 360 * 4;
        let mut pixels = vec![0u8; bgra_len];
        pixels[0..3].copy_from_slice(&[1, 2, 3]);
        pixels[bgr_len - 3..bgr_len].copy_from_slice(&[4, 5, 6]);

        let bytes_written = super::convert_bgr24_to_bgra8_in_place(&mut pixels, bgr_len);

        assert_eq!(bgr_len, 691_200);
        assert_eq!(bgra_len, 921_600);
        assert_eq!(bytes_written, bgra_len);
        assert_eq!(&pixels[0..4], &[1, 2, 3, 255]);
        assert_eq!(&pixels[bgra_len - 4..bgra_len], &[4, 5, 6, 255]);
    }
    ...
    #[derive(Debug, Default)]
    struct AlwaysSuppressPreviewDecodeRuntime;

    impl SwitcherH264DecodeRuntimeHook for AlwaysSuppressPreviewDecodeRuntime {
    ...
    fn switcher_four_view_real_handoff_preview_summary_formats_expected_fields() {
        ...
        assert!(formatted.contains("program_output_enabled=false"));
        assert!(formatted.contains("program_output_render_count=0"));
        assert!(formatted.contains("program_output_missing_selected_source_count=0"));
        assert!(formatted.contains("program_output_last_result_kind=Disabled"));
        assert!(formatted.contains("program_output_selected_client_id=none"));
        assert!(formatted.contains("program_output_selected_slot_index=none"));
    ...
    #[test]
    fn switcher_four_view_two_real_handoff_preview_loop_can_opt_in_program_output_window() {
        let preview_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let program_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let summary =
        ...
        assert!(summary.program_output_enabled);
        assert_eq!(summary.program_output_selection_mode, "fallback");
        assert_eq!(summary.program_output_requested_client_id, None);
        assert_eq!(summary.program_output_render_count, 1);
        assert_eq!(summary.program_output_missing_selected_source_count, 0);
        assert_eq!(summary.program_output_missing_selected_source_reason, None);
        assert_eq!(summary.program_output_last_result_kind, "Rendered");
        assert_eq!(
            summary.program_output_selected_client_id,
            Some(ClientId("real-client-0".to_string()))
        );
        assert_eq!(summary.program_output_selected_slot_index, Some(0));
        assert_eq!(
            summary.program_output_window_title.as_deref(),
            Some(SWITCHER_PROGRAM_OUTPUT_WINDOW_TITLE)
        );

        let preview_requests = preview_runtime.requests.borrow();
        let program_requests = program_runtime.requests.borrow();
        assert_eq!(preview_requests.len(), 1);
        assert_eq!(program_requests.len(), 1);
        assert_eq!(
            preview_requests[0].title,
            SWITCHER_FOUR_VIEW_CLEAN_OUTPUT_WINDOW_TITLE
        );
        assert_eq!(
            program_requests[0].title,
            SWITCHER_PROGRAM_OUTPUT_WINDOW_TITLE
        );
        assert_eq!(
            program_requests[0].width,
            FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_WIDTH / 2
        );
        assert_eq!(
            program_requests[0].height,
            FOUR_VIEW_CLEAN_OUTPUT_LOOP_OBS_OUTPUT_HEIGHT / 2
        );
        assert_eq!(program_requests[0].unique_corner_count(), 1);
    }

    #[test]
    fn switcher_four_view_two_real_handoff_preview_loop_can_explicitly_select_program_client() {
        let preview_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let program_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let summary =
            super::run_four_view_two_real_handoff_preview_loop_with_handoff_runtime_target_timestamp_hook_and_sleep(
        ...
        assert_eq!(summary.program_output_selection_mode, "explicit");
        assert_eq!(
            summary.program_output_requested_client_id,
            Some(ClientId("real-client-1".to_string()))
        );
        assert_eq!(summary.program_output_render_count, 1);
        assert_eq!(summary.program_output_missing_selected_source_count, 0);
        ...
        assert_eq!(summary.program_output_selected_slot_index, Some(2));
        assert_eq!(program_runtime.requests.borrow().len(), 1);
        assert_eq!(preview_runtime.requests.borrow().len(), 1);
    }

    #[test]
    fn switcher_four_view_two_real_handoff_preview_loop_reports_missing_explicit_program_client() {
        let preview_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let program_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let summary =
            super::run_four_view_two_real_handoff_preview_loop_with_handoff_runtime_target_timestamp_hook_and_sleep(
        ...
        assert_eq!(summary.program_output_render_count, 0);
        assert_eq!(summary.program_output_missing_selected_source_count, 1);
        assert_eq!(
            summary.program_output_missing_selected_source_reason,
            Some("RequestedClientNotInRealSlots")
        );
        assert_eq!(
            summary.program_output_last_result_kind,
        ...
        assert_eq!(summary.program_output_selected_slot_index, None);
        assert_eq!(program_runtime.requests.borrow().len(), 0);
        assert_eq!(preview_runtime.requests.borrow().len(), 1);
    }

    #[test]
    fn switcher_four_view_two_real_handoff_preview_loop_reuses_previous_program_frame_on_missing_tick(
    ) {
        let preview_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let program_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let summary =
        ...
        assert_eq!(summary.program_output_render_count, 2);
        assert_eq!(summary.program_output_missing_selected_source_count, 1);
        assert_eq!(summary.program_output_missing_before_first_render_count, 0);
        assert_eq!(summary.program_output_missing_after_first_render_count, 1);
        assert_eq!(summary.program_output_reused_previous_frame_count, 1);
        assert_eq!(summary.program_output_placeholder_render_count, 0);
        assert_eq!(summary.program_output_black_frame_render_count, 0);
        assert_eq!(summary.program_output_first_render_attempt_index, Some(1));
        assert!(summary.program_output_first_render_elapsed_ms.is_some());
        assert_eq!(
            summary.program_output_missing_selected_source_reason,
            Some("NoDecodedFrameForSelection")
        );
        assert_eq!(summary.program_output_last_result_kind, "Rendered");

        let program_requests = program_runtime.requests.borrow();
        assert_eq!(program_requests.len(), 2);
        assert_eq!(program_requests[0].top_left, [0x10, 0x11, 0x12, 255]);
        assert_eq!(program_requests[1].top_left, program_requests[0].top_left);
        assert_eq!(
            program_requests[1].bottom_right,
            program_requests[0].bottom_right
        );
    }

    #[test]
    fn switcher_four_view_two_real_handoff_preview_loop_program_first_reuses_preview_after_first_render(
    ) {
        let preview_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let program_runtime = RecordingPersistentFixtureRenderedWindowRuntime::default();
        let summary =
        ...
        assert!(summary.program_first_validation_enabled);
        assert_eq!(summary.program_render_loop_attempt_count, 2);
        assert_eq!(summary.program_window_render_success_count, 2);
        assert_eq!(summary.program_window_render_failure_count, 0);
        assert!(summary.program_first_preview_visible);
        assert_eq!(summary.program_first_preview_refresh_interval, None);
        assert_eq!(summary.program_first_preview_refresh_count, 0);
        assert_eq!(summary.program_first_preview_suppressed_count, 1);
        assert!(!summary.operator_low_cost_preview_enabled);
        assert_eq!(summary.operator_preview_refresh_interval_ticks, None);
        assert_eq!(summary.operator_preview_refresh_attempt_count, 0);
        assert_eq!(summary.operator_preview_refresh_success_count, 0);
        assert_eq!(summary.operator_preview_refresh_skipped_count, 0);
        assert_eq!(summary.operator_preview_used_stale_frame_count, 0);
        assert_eq!(summary.preview_compose_skipped_for_program_count, 1);
        assert_eq!(summary.preview_compose_reused_for_program_count, 1);
        assert_eq!(preview_runtime.requests.borrow().len(), 1);
        assert_eq!(program_runtime.requests.borrow().len(), 2);
    }

    #[test]
    fn switcher_program_first_suppresses_non_program_preview_one_shot_decode() {
        let timing = Rc::new(RefCell::new(TwoRealPreviewLoopRuntimeTiming::default()));
        ...
        ));
        {
            ...
            assert!(timing.program_first_program_only_decode_path_enabled);
            assert_eq!(
            ...
            assert_eq!(timing.decode_attempt_count, 0);
            assert_eq!(timing.one_shot_decode_attempt_count, 0);
        }

        let program_result = runtime.decode_annex_b_h264(SwitcherH264DecodeInput {
    ...
    fn switcher_four_view_two_real_handoff_preview_loop_redecodes_different_frame_ids() {
        ...
        assert_eq!(
            summary.one_shot_decode_slow_first_byte_source_counts,
            "queue:0|retained_keyframe:0|unknown:0"
        );
        assert_eq!(
            summary.one_shot_decode_slow_first_byte_reason_counts,
            "frame_id_changed:0|cache_miss:0|previous_unavailable:0|source_recovered:0|unknown:0"
        );
        assert_eq!(
            summary.one_shot_decode_slow_output_read_slot_counts,
            "slot0:0|slot1:0"
        ...
        assert_eq!(summary.one_shot_decode_input_write_outlier_count, 0);
        assert_eq!(summary.one_shot_decode_input_write_outlier_threshold_ms, 66);
        assert_eq!(
            summary.one_shot_decode_input_write_outlier_slot_counts,
            "slot0:0|slot1:0"
        );
    ...
    fn switcher_four_view_two_real_handoff_preview_summary_formats_expected_fields() {
        ...
        assert!(formatted.contains("persistent_decode_config_enabled=false"));
        ...
        assert!(formatted.contains("persistent_decode_skipped_by_config_count=0"));
        ...
        assert!(formatted.contains("continuous_decode_config_enabled=false"));
        assert!(formatted.contains("continuous_decode_runtime_enabled=false"));
        assert!(formatted.contains("continuous_decode_slot0_enabled=false"));
        assert!(formatted.contains("continuous_decode_input_frame_count=0"));
        assert!(formatted.contains("continuous_decode_output_frame_count=0"));
        assert!(formatted.contains("continuous_decode_queue_len=0"));
        assert!(formatted.contains("continuous_decode_dropped_stale_count=0"));
        assert!(formatted.contains("continuous_decode_frame_id_lag=0"));
        assert!(formatted.contains("continuous_decode_stdout_read_elapsed_ms=0"));
        assert!(formatted.contains("continuous_decode_stall_count=0"));
        assert!(formatted.contains("continuous_decode_restart_count=0"));
        assert!(formatted.contains("continuous_decode_runtime_disabled=false"));
        assert!(formatted.contains("continuous_decode_runtime_disabled_reason=none"));
        assert!(formatted.contains("continuous_decode_fallback_to_one_shot_count=0"));
        assert!(formatted.contains("continuous_decode_lookup_hit_count=0"));
        assert!(formatted.contains("continuous_decode_lookup_miss_count=0"));
        assert!(formatted.contains("continuous_decode_lookup_miss_reason_counts=exact_key_missing:0|queue_empty:0|runtime_disabled:0|output_pending:0|frame_id_lagging:0|unknown:0"));
        assert!(formatted.contains("continuous_decode_requested_frame_id=none"));
        assert!(formatted.contains("continuous_decode_latest_decoded_frame_id=none"));
        assert!(formatted.contains("continuous_decode_requested_minus_latest_lag=none"));
        assert!(formatted.contains("continuous_decode_queue_oldest_frame_id=none"));
        assert!(formatted.contains("continuous_decode_queue_newest_frame_id=none"));
        assert!(formatted.contains("continuous_decode_input_frame_id_min=none"));
        assert!(formatted.contains("continuous_decode_input_frame_id_max=none"));
        assert!(formatted.contains("continuous_decode_output_frame_id_min=none"));
        assert!(formatted.contains("continuous_decode_output_frame_id_max=none"));
        assert!(formatted.contains("continuous_decode_output_pending_correspondence_count=0"));
        assert!(formatted.contains("continuous_decode_pending_correspondence_count=0"));
        assert!(formatted.contains("continuous_decode_pending_correspondence_age_ms_max=none"));
        ...
        assert!(formatted.contains("continuous_decode_input_frame_id_gap_max=0"));
        assert!(formatted.contains("continuous_decode_input_frame_id_gap_total=0"));
        assert!(formatted.contains("continuous_decode_input_non_consecutive_count=0"));
        assert!(formatted.contains("continuous_decode_input_keyframe_count=0"));
        assert!(formatted.contains("continuous_decode_input_non_keyframe_count=0"));
        assert!(formatted.contains("continuous_decode_input_has_sps_count=0"));
        assert!(formatted.contains("continuous_decode_input_has_pps_count=0"));
        assert!(formatted.contains("continuous_decode_input_has_idr_count=0"));
        assert!(formatted.contains("continuous_decode_input_has_non_idr_vcl_count=0"));
        assert!(formatted.contains("continuous_decode_last_input_payload_nal_kinds=none"));
        assert!(formatted.contains("continuous_decode_ffmpeg_stderr_summary=none"));
        assert!(formatted.contains("continuous_decode_stdout_reader_blocked_count=0"));
        assert!(formatted.contains("continuous_decode_no_output_after_input_count=0"));
        assert!(formatted.contains("continuous_decode_no_output_after_keyframe_count=0"));
        assert!(formatted.contains("continuous_decode_bootstrap_input_count=0"));
        assert!(formatted.contains("continuous_decode_bootstrap_output_count=0"));
        ...
        assert!(formatted.contains("continuous_decode_last_stdin_write_error=none"));
        assert!(formatted.contains("continuous_decode_process_running=false"));
        ...
        assert!(formatted.contains("continuous_decode_last_input_write_elapsed_ms=0"));
        assert!(formatted.contains("continuous_decode_last_input_payload_bytes=0"));
        assert!(formatted.contains("continuous_decode_first_input_to_first_output_elapsed_ms=none"));
        assert!(formatted.contains("continuous_decode_first_input_to_now_elapsed_ms=none"));
        assert!(formatted.contains("continuous_feed_enabled=false"));
        assert!(formatted.contains("continuous_feed_attempt_count=0"));
        assert!(formatted.contains("continuous_feed_handoff_request_count=0"));
        assert!(formatted.contains("continuous_feed_frame_received_count=0"));
        assert!(formatted.contains("continuous_feed_no_frame_count=0"));
        assert!(formatted.contains("continuous_feed_handoff_error_count=0"));
        assert!(formatted.contains("continuous_feed_enqueued_count=0"));
        assert!(formatted.contains("continuous_feed_skipped_count=0"));
        assert!(formatted.contains("continuous_feed_skip_reason_counts=duplicate:0|future_frame:0|runtime_disabled:0|startup_not_ready:0|input_queue_full:0|source_mismatch:0|consume_mismatch:0|unknown:0"));
        assert!(formatted.contains("continuous_feed_dropped_stale_input_count=0"));
        assert!(formatted.contains("continuous_feed_latest_received_frame_id=none"));
        assert!(formatted.contains("continuous_feed_latest_enqueued_frame_id=none"));
        assert!(formatted.contains("continuous_decode_input_from_feeder_count=0"));
        assert!(formatted.contains("continuous_decode_input_from_render_demand_count=0"));
        assert!(formatted.contains("continuous_decode_feeder_lag_to_selected=none"));
        ...
        assert!(formatted.contains("continuous_decode_output_availability_not_ready_count=0"));
        assert!(formatted.contains("continuous_decode_output_availability_stale_count=0"));
        assert!(formatted.contains("continuous_decode_output_availability_future_count=0"));
        ...
        assert!(formatted.contains("continuous_decode_latest_input_to_output_frame_gap=none"));
        ...
        assert!(formatted.contains("continuous_decode_latest_selected_to_output_frame_gap=none"));
        ...
        assert!(formatted.contains("continuous_decode_reader_full_frame_elapsed_ms_avg=n/a"));
        assert!(formatted.contains("continuous_decode_reader_full_frame_slow_count=0"));
        assert!(formatted.contains("continuous_decode_reader_full_frame_slow_threshold_ms=66"));
        assert!(formatted.contains("continuous_decode_output_bytes_total=0"));
        assert!(formatted.contains("continuous_decode_output_bytes_per_sec=n/a"));
        assert!(formatted.contains("continuous_decode_output_frame_interval_ms_avg=n/a"));
        assert!(formatted.contains("continuous_decode_output_frame_interval_ms_max=0"));
        assert!(formatted.contains("continuous_decode_stdout_read_throughput_bytes_per_ms=n/a"));
        ...
        assert!(formatted.contains("continuous_decode_output_pixel_convert_elapsed_ms_max=0"));
        assert!(formatted.contains("continuous_decode_output_pixel_convert_count=0"));
        assert!(formatted.contains("continuous_decode_output_pixel_convert_buffer_reuse_count=0"));
        assert!(
            formatted.contains("continuous_decode_output_pixel_convert_buffer_allocation_count=0")
        );
        assert!(formatted.contains("continuous_decode_output_pixel_convert_bytes_written_total=0"));
        assert!(
            formatted.contains("continuous_decode_output_pixel_convert_bytes_written_per_frame=0")
        );
        assert!(formatted.contains("continuous_decode_output_pixel_convert_mode=none"));
        assert!(formatted.contains("continuous_decode_competing_one_shot_decode_elapsed_ms=0"));
        assert!(formatted.contains("continuous_decode_competing_one_shot_attempt_count=0"));
        assert!(formatted.contains("continuous_decode_slot0_one_shot_suppression_enabled=false"));
        assert!(formatted.contains("continuous_decode_slot0_one_shot_suppressed_count=0"));
        assert!(formatted.contains("continuous_decode_slot0_one_shot_suppressed_reason_counts=continuous_not_ready:0|stale:0|future:0|unknown:0"));
        assert!(formatted.contains("continuous_decode_slot0_one_shot_suppressed_render_safety_counts=decode_deferred_placeholder:0|unknown:0"));
        assert!(formatted
            .contains("continuous_decode_slot0_one_shot_suppressed_continuous_not_ready_count=0"));
        assert!(formatted.contains("continuous_decode_slot0_one_shot_suppressed_stale_count=0"));
        assert!(formatted.contains(
        ...
        assert!(formatted.contains("one_shot_decode_attempt_count=0"));
        assert!(formatted.contains("one_shot_decode_elapsed_ms=0"));
        assert!(formatted.contains("one_shot_decode_elapsed_ms_max=0"));
        assert!(formatted.contains("one_shot_decode_input_write_elapsed_ms=0"));
        ...
        assert!(formatted.contains("one_shot_decode_slow_first_byte_slot_counts=slot0:0|slot1:0"));
        assert!(formatted.contains(
            "one_shot_decode_slow_first_byte_source_counts=queue:0|retained_keyframe:0|unknown:0"
        ));
        assert!(formatted.contains("one_shot_decode_slow_first_byte_reason_counts=frame_id_changed:0|cache_miss:0|previous_unavailable:0|source_recovered:0|unknown:0"));
        assert!(formatted.contains("one_shot_decode_slow_output_read_slot_counts=slot0:0|slot1:0"));
        assert!(formatted.contains(
            "one_shot_decode_slow_output_read_source_counts=queue:0|retained_keyframe:0|unknown:0"
        ));
        assert!(formatted.contains("one_shot_decode_slow_output_read_reason_counts=frame_id_changed:0|cache_miss:0|previous_unavailable:0|source_recovered:0|unknown:0"));
        assert!(formatted.contains("one_shot_decode_input_write_outlier_count=0"));
        assert!(formatted.contains("one_shot_decode_input_write_outlier_threshold_ms=66"));
        assert!(
            formatted.contains("one_shot_decode_input_write_outlier_slot_counts=slot0:0|slot1:0")
        );
        assert!(formatted.contains("one_shot_decode_input_write_outlier_source_counts=queue:0|retained_keyframe:0|unknown:0"));
        assert!(formatted.contains("one_shot_decode_input_write_outlier_reason_counts=frame_id_changed:0|cache_miss:0|previous_unavailable:0|source_recovered:0|unknown:0"));
        assert!(formatted.contains("one_shot_decode_input_write_outlier_payload_bytes_total=0"));
        assert!(formatted.contains("one_shot_decode_input_write_outlier_payload_bytes_avg=n/a"));
        ...
        assert!(formatted.contains("one_shot_decode_input_payload_bytes_min=none"));
        assert!(formatted.contains("one_shot_decode_input_payload_bytes_max=0"));
        assert!(formatted.contains("one_shot_decode_input_payload_bytes_avg=n/a"));
        assert!(formatted.contains("one_shot_decode_keyframe_attempt_count=0"));
        assert!(formatted.contains("one_shot_decode_non_keyframe_attempt_count=0"));
        assert!(formatted.contains("one_shot_decode_keyframe_elapsed_ms=0"));
        assert!(formatted.contains("one_shot_decode_non_keyframe_elapsed_ms=0"));
        assert!(formatted.contains("one_shot_decode_keyframe_input_payload_bytes_total=0"));
        assert!(formatted.contains("one_shot_decode_non_keyframe_input_payload_bytes_total=0"));
        assert!(formatted.contains("one_shot_decode_expected_output_bytes_per_frame=n/a"));
        assert!(formatted.contains("one_shot_decode_output_width=640"));
        assert!(formatted.contains("one_shot_decode_output_height=360"));
        assert!(formatted.contains("one_shot_decode_output_pixel_format=Bgra8"));
        assert!(formatted.contains("one_shot_decode_scaled_output_enabled=true"));
        assert!(formatted.contains("one_shot_decode_scaled_output_reason=two_real_slot_size"));
        ...
        assert!(formatted.contains("program_output_enabled=false"));
        assert!(formatted.contains("program_output_selection_mode=disabled"));
        assert!(formatted.contains("program_output_requested_client_id=none"));
        assert!(formatted.contains("program_output_render_count=0"));
        assert!(formatted.contains("program_output_missing_selected_source_count=0"));
        assert!(formatted.contains("program_output_missing_before_first_render_count=0"));
        assert!(formatted.contains("program_output_missing_after_first_render_count=0"));
        assert!(formatted.contains("program_output_reused_previous_frame_count=0"));
        assert!(formatted.contains("program_output_last_result_kind=Disabled"));
        assert!(formatted.contains("program_output_selected_client_id=none"));
        assert!(formatted.contains("program_output_selected_slot_index=none"));
        assert!(formatted.contains("program_output_window_title=none"));
        ...
        assert!(formatted.contains("program_first_validation_enabled=false"));
        assert!(formatted.contains("program_first_suppressed_preview_one_shot_decode_count=0"));
        assert!(formatted.contains(
            "program_first_suppressed_preview_one_shot_decode_slot_counts=slot0:0|slot1:0"
        ));
        assert!(formatted.contains("program_first_program_only_decode_path_enabled=false"));
        assert!(formatted.contains("program_first_remaining_one_shot_decode_count=0"));
        assert!(formatted.contains("program_first_preview_visible=false"));
        assert!(formatted.contains("program_first_preview_refresh_interval=none"));
        assert!(formatted.contains("program_first_preview_refresh_count=0"));
        assert!(formatted.contains("program_first_preview_suppressed_count=0"));
        assert!(formatted.contains("operator_low_cost_preview_enabled=false"));
        assert!(formatted.contains("operator_preview_refresh_interval_ticks=none"));
        assert!(formatted.contains("operator_preview_refresh_attempt_count=0"));
        assert!(formatted.contains("operator_preview_refresh_success_count=0"));
        assert!(formatted.contains("operator_preview_refresh_skipped_count=0"));
        ...
        assert!(formatted.contains("operator_preview_reused_program_frame_count=0"));
        assert!(formatted.contains("operator_preview_program_slot_visible_count=0"));
        assert!(formatted.contains("operator_preview_program_slot_reuse_source=none"));
        assert!(formatted.contains("operator_preview_program_slot_black_count=0"));
        assert!(formatted.contains("operator_preview_snapshot_retention_enabled=false"));
        assert!(formatted.contains("operator_preview_snapshot_reuse_count=0"));
        assert!(formatted.contains(
            "operator_preview_snapshot_reuse_slot_counts=slot0:0|slot1:0|slot2:0|slot3:0"
        ));
        assert!(formatted.contains("operator_preview_snapshot_created_count=0"));
        assert!(formatted.contains(
            "operator_preview_snapshot_created_slot_counts=slot0:0|slot1:0|slot2:0|slot3:0"
        ));
        assert!(formatted.contains("operator_preview_placeholder_avoided_by_snapshot_count=0"));
        assert!(formatted.contains("operator_preview_slot_black_after_snapshot_count=0"));
        ...
        assert!(formatted.contains("program_window_render_success_count=0"));
        assert!(formatted.contains("program_window_render_failure_count=0"));
        assert!(formatted.contains("program_render_effective_fps="));
        assert!(formatted.contains("effective_program_render_fps="));
    }
    ...
    #[test]
    fn continuous_decode_output_availability_counts_bounded_lookup_rejections() {
        let runtime = two_real_continuous_test_runtime();
        let mut timing = TwoRealPreviewLoopRuntimeTiming::default();

        runtime.observe_bounded_lookup_rejection(
            &mut timing,
            &TwoRealContinuousBoundedLookupCandidate::RejectedStale,
        );
        runtime.observe_bounded_lookup_rejection(
        ...
        assert_eq!(
            timing.continuous_decode_bounded_lookup_rejected_stale_count,
            1
        );
        assert_eq!(
            timing.continuous_decode_bounded_lookup_rejected_future_count,
            1
        );
        assert_eq!(
            timing.continuous_decode_bounded_lookup_rejected_not_ready_count,
            1
        );
        assert_eq!(timing.continuous_decode_output_availability_stale_count, 1);
        assert_eq!(timing.continuous_decode_output_availability_future_count, 1);
        assert_eq!(
