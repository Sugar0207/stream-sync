...
impl ServerReceiveAuthVideoQueueOnceManualPolicy {
    pub fn expected_reassembled_frames_enabled(self) -> bool {
        self.stop_after_expected_reassembled_frames && self.expected_reassembled_frames > 0
    }

    pub fn expected_reassembled_clients_enabled(self) -> bool {
        self.expected_reassembled_clients > 0
    }

    pub fn expected_reassembled_frames_per_client_enabled(self) -> bool {
        self.expected_reassembled_frames_per_client > 0
    }
}
...
fn record_queued_frame_progress(
    summary: &mut ServerReceiveAuthVideoQueueOnceVideoSummary,
    client_id: &ClientId,
    run_id: &RunId,
) {
    let key = format!("{}/{}", client_id.0, run_id.0);
    *summary.per_client_queued_frames.entry(key).or_insert(0) += 1;
    summary.observed_queued_clients = summary.per_client_queued_frames.len();
}

fn record_direct_frame_progress(
    summary: &mut ServerReceiveAuthVideoQueueOnceVideoSummary,
    client_id: &ClientId,
    run_id: &RunId,
) {
...
fn evaluate_reassembled_stop_reason(
    ...
    let frame_threshold_enabled = policy.expected_reassembled_frames_enabled();
    let client_aware_enabled = policy.expected_reassembled_clients_enabled();

    if !frame_threshold_enabled && !client_aware_enabled {
        return None;
    }

    let frame_threshold_met =
        !frame_threshold_enabled || summary.frames_queued >= policy.expected_reassembled_frames;
...
pub struct ServerReceiveSendRuntimeBoundedSummary {
    ...
    pub timeout_iterations: usize,
    pub timeout_only_run: bool,
...
pub enum ServerReceiveSendRuntimeBoundedStartupError {
    ...
    Runtime {
        ...
        summary: ServerReceiveSendRuntimeBoundedSummary,
    },
}
...
impl ServerReceiveSendRuntimeBoundedLauncher {
    ...
    > {
        ...
        for iteration_index in 0..policy.max_iterations {
            ...
            let result = match self.runtime.run_once(
                ...
                Err(error) => {
                    ...
                    self.observe_runtime_error(&error, &mut partial_summary);
                    partial_summary.outbound_queue_len = queue_collection.len();
                    partial_summary.registered_clients = registry.entries().count();
                    return Err(ServerReceiveSendRuntimeBoundedStartupError::Runtime {
                        iteration_index,
                        error,
        ...
        summary.timeout_only_run = matches!(
            summary.stop_reason,
            ServerReceiveSendRuntimeBoundedStopReason::ReceiveTimedOut
        ) && summary.accepted_packets == 0
            && summary.rejected_packets == 0
            && summary.auth_requests_received == 0
            && summary.heartbeats_received == 0
            && summary.client_stats_received == 0;
                        ...
                        return Some(match error_kind {
                            io::ErrorKind::TimedOut | io::ErrorKind::WouldBlock => {
                                summary.timeout_iterations =
                                    summary.timeout_iterations.saturating_add(1);
                                ServerReceiveSendRuntimeBoundedStopReason::ReceiveTimedOut
                            }
                            other => {
...
pub struct ServerReceiveAuthVideoQueueHandoffContinuousRuntimeSummary {
    ...
    pub decodable_source_retained_keyframe_count: usize,
    pub io_error_count: usize,
    pub expected_reassembled_frames_enabled: bool,
    pub expected_clients_enabled: bool,
    pub expected_per_client_frames_enabled: bool,
...
impl ServerReceiveAuthVideoQueueHandoffContinuousRuntimeLauncher {
        ...
        ReadyFn: FnOnce(&ServerReceiveAuthVideoQueueHandoffContinuousReadyState),
    {
        ...
        let summary = ServerReceiveAuthVideoQueueHandoffContinuousRuntimeSummary {
            ...
            expected_reassembled_frames_enabled: policy.expected_reassembled_frames_enabled(),
            expected_clients_enabled: policy.expected_reassembled_clients_enabled(),
            expected_per_client_frames_enabled: policy
                .expected_reassembled_frames_per_client_enabled(),
            stop_reason,
            receive_stop_reason,
            handoff_stop_reason,
            runtime_duration_ms,
...
"#,
    ...
    fn receive_send_runtime_bounded_launcher_stops_at_max_iterations() {
        ...
        assert_eq!(outcome.summary.timeout_iterations, 0);
    ...
    fn receive_send_runtime_bounded_launcher_keeps_registry_across_auth_iterations() {
        ...
        assert_eq!(outcome.summary.timeout_iterations, 1);
        assert!(matches!(
            outcome.summary.last_receive_error,
    ...
    fn receive_send_runtime_bounded_launcher_uses_existing_registry_for_heartbeat() {
        ...
        assert!(outcome.summary.auth_requests_received >= 1);
        assert!(outcome.summary.auth_responses_sent >= 1);
    ...
    #[test]
    fn receive_send_runtime_bounded_launcher_marks_timeout_only_run_when_no_packets_arrive() {
        let launcher = ServerReceiveSendRuntimeBoundedLauncher::default();
        let bind_port = reserve_localhost_udp_port();
        let startup_config = launcher
            .load_startup_config_from_str(&receive_send_runtime_test_config(bind_port))
        ...
        let outcome = launcher
            .run_bounded_with_writers_and_policy(
                startup_config,
                Vec::new(),
                Vec::new(),
                Vec::new(),
            ...
            .expect("timeout-only bounded runtime should complete");
    ...
    #[test]
    fn receive_send_runtime_bounded_launcher_records_auth_rejection_reason() {
        let launcher = ServerReceiveSendRuntimeBoundedLauncher::default();
        let bind_port = reserve_localhost_udp_port();
        let startup_config = launcher
            .load_startup_config_from_str(&receive_send_runtime_test_config(bind_port))
        ...
        client_socket
            ...
            .expect("rejected auth request should send");
        client_socket
            .send_to(
                auth_request_packet("client-1", "wrong-secret").as_slice(),
                server_address,
            )
            .expect("second rejected auth request should send");
        ...
        assert!(outcome.summary.auth_requests_received >= 1);
        assert_eq!(outcome.summary.auth_responses_sent, 0);
        assert_eq!(
    ...
    #[test]
    fn receive_send_runtime_bounded_launcher_records_gate_rejection_reason() {
        let launcher = ServerReceiveSendRuntimeBoundedLauncher::default();
        let bind_port = reserve_localhost_udp_port();
        let startup_config = launcher
            .load_startup_config_from_str(&receive_send_runtime_test_config(bind_port))
        ...
        assert_eq!(outcome.summary.decode_errors, 0);
        assert_eq!(
            outcome.summary.last_rejected_reason.as_deref(),
...
"#,
    ...
    #[test]
    fn concurrent_shared_state_receive_helper_keeps_receiving_video_after_auth() {
        let launcher = ServerReceiveAuthVideoQueueOnceLauncher::default();
        let socket = UdpSocket::bind("127.0.0.1:0").expect("socket should bind");
        let source = packet_source();
        let mut shared = ServerReceiveAuthVideoQueueHandoffContinuousSharedState {
        ...
        launcher
            ...
            .expect("first video frame should enqueue through shared-state receive helper");
        launcher
            .process_received_packet_for_video_queue(
                &socket,
                &auth_config(Some("secret")),
                ...
                ProtocolVersion(2),
                source,
                &video_frame_packet("client-1", 43),
                TimestampMicros(10_000),
                &mut shared.receive_summary,
            )
            .expect("second video frame should enqueue through shared-state receive helper");

        assert_eq!(shared.receive_summary.packets_received, 2);
        assert_eq!(shared.receive_summary.frames_queued, 2);
        assert_eq!(shared.receive_summary.keyframes_queued, 2);
        assert_eq!(
            shared.receive_summary.per_client_queued_frames,
            BTreeMap::from([("client-1/run-1".to_string(), 2)])
        );
        assert_eq!(shared.video_queue_state.total_len(), 2);
        assert_eq!(shared.video_queue_state.retained_keyframe_clients(), 1);
    }
    ...
    #[test]
    fn receive_auth_video_queue_once_direct_frame_timeout_does_not_stop_early() {
        let launcher = ServerReceiveAuthVideoQueueOnceLauncher::default();
        let bind_port = reserve_localhost_udp_port();
        let startup_config = auth_video_queue_startup_config(
            bind_port,
            auth_config_for_clients(&["client-1"], Some("presented-secret")),
        );
        ...
        let server_address = SocketAddr::from(([127, 0, 0, 1], bind_port));
        let handle = std::thread::spawn(move || {
            launcher.run_once_with_writers_and_policy(
                startup_config,
                Vec::new(),
                Vec::new(),
                Vec::new(),
        ...
        let outcome = handle
            .join()
            .expect("receive launcher thread should join")
            .expect("receive launcher should complete");
        let ServerReceiveAuthVideoQueueOnceVideoOutcome::Received { summary, .. } = outcome.video
        else {
            panic!("video summary should be present");
        };
    ...
    #[test]
    fn receive_auth_video_queue_once_accepts_second_client_after_first_direct_frame() {
        let launcher = ServerReceiveAuthVideoQueueOnceLauncher::default();
        let bind_port = reserve_localhost_udp_port();
        let startup_config = auth_video_queue_startup_config(
            bind_port,
            auth_config_for_clients(&["client-1", "client-2"], Some("presented-secret")),
        );
        let policy = ServerReceiveAuthVideoQueueOnceManualPolicy {
            max_video_packets: 12,
            receive_timeout: Duration::from_millis(500),
            expected_reassembled_frames: 2,
            stop_after_expected_reassembled_frames: true,
            receive_buffer_bytes: SERVER_DEFAULT_RECEIVE_BUFFER_BYTES,
        ...
        let client_one = UdpSocket::bind("127.0.0.1:0").expect("client one socket should bind");
        let client_two = UdpSocket::bind("127.0.0.1:0").expect("client two socket should bind");
        client_one
            .set_read_timeout(Some(Duration::from_secs(1)))
            .expect("client one read timeout should be set");
        client_two
            .set_read_timeout(Some(Duration::from_secs(1)))
            .expect("client two read timeout should be set");
        std::thread::sleep(Duration::from_millis(50));
        ...
        client_one
            ...
            .expect("client one auth request should send");
        let mut response_buffer = vec![0_u8; 1024];
        client_one
            .recv_from(&mut response_buffer)
            .expect("client one auth response should receive");
        client_one
            .send_to(
                video_frame_packet("client-1", 42).as_slice(),
                server_address,
            )
            .expect("client one direct frame should send");

        client_two
            .send_to(
                auth_request_packet("client-2", "presented-secret").as_slice(),
                server_address,
            )
            .expect("client two auth request should send");
        client_two
            .recv_from(&mut response_buffer)
            .expect("client two auth response should receive");
        client_two
            .send_to(
                video_frame_packet("client-2", 43).as_slice(),
                server_address,
            )
            .expect("client two direct frame should send");
        ...
        assert_eq!(outcome.registry.entries().count(), 2);
        ...
        assert_eq!(
            summary.per_client_queued_frames.get("client-2/run-1"),
            Some(&1)
        );
        assert_eq!(
            summary.per_client_direct_frames.get("client-1/run-1"),
...
"#
    ...
    fn auth_config_for_clients(
        ...
        ServerAuthConfig {
            allowed_clients: client_ids
                .iter()
                .map(|client_id| AllowedClientConfig {
    ...
    fn auth_video_queue_startup_config(
        bind_port: u16,
        auth_config: ServerAuthConfig,
    ) -> ServerAuthResponsePocStartupConfig {
        ServerAuthResponsePocStartupConfig {
            bind_address: SocketAddr::from(([127, 0, 0, 1], bind_port)),
            expected_protocol_version: ProtocolVersion(2),
            auth_config,
        }
    }
    ...
    #[test]
    fn disabled_expected_thresholds_do_not_stop_receive() {
        let policy = ServerReceiveAuthVideoQueueOnceManualPolicy {
            expected_reassembled_frames: 0,
            stop_after_expected_reassembled_frames: true,
            expected_reassembled_clients: 0,
        ...
        let mut summary = ServerReceiveAuthVideoQueueOnceVideoSummary::default();
        summary.frames_queued = 1;
        summary.frames_reassembled = 1;
        record_queued_frame_progress(
            &mut summary,
            &ClientId("player1".to_string()),
            &RunId("run-1".to_string()),
    ...
    #[test]
    fn positive_reassembled_threshold_still_stops_receive() {
        let policy = ServerReceiveAuthVideoQueueOnceManualPolicy {
            expected_reassembled_frames: 2,
            stop_after_expected_reassembled_frames: true,
            expected_reassembled_clients: 0,
        ...
        assert_eq!(
            evaluate_reassembled_stop_reason(policy, &summary),
            Some(ServerReceiveAuthVideoQueueStopReason::ReassembledFramesThresholdReached)
        );
    }
    ...
    #[test]
    fn client_aware_stop_counts_direct_and_reassembled_queued_frames_together() {
        let policy = ServerReceiveAuthVideoQueueOnceManualPolicy {
            expected_reassembled_frames: 4,
            expected_reassembled_clients: 2,
            expected_reassembled_frames_per_client: 2,
        ...
        summary.frames_queued = 1;
        summary.direct_frames_queued = 1;
        record_direct_frame_progress(
            &mut summary,
            &ClientId("player1".to_string()),
            &RunId("run-1".to_string()),
        );
        ...
        summary.frames_queued = summary.frames_queued.saturating_add(1);
        summary.direct_frames_queued = summary.direct_frames_queued.saturating_add(1);
        record_direct_frame_progress(
            &mut summary,
            &ClientId("player2".to_string()),
            &RunId("run-1".to_string()),
