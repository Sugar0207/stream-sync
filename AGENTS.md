...
## 概要
このリポジトリは、4人のゲーム映像を受信し、共通時間軸で同期し、OBS に載せられる多視点スイッチング配信ツール `StreamSync` を開発するためのものです。

目的は、4人の映像をできるだけ高精度に同期し、配信で実用的に使える基盤を作ることです。
...
- 1080p / 60fps は将来拡張かつ条件付き上位運用モードとする
